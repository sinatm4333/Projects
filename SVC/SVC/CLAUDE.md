...
**Where this now lives:** the user uploaded `res_v2`'s own full export (bot id 304, `.tybot.html`, 42 attachments) — `src/res_v2_attachments/template_report.html` in this repo is `2/res_v2`'s own `template_report.html` with guideline-140 CSS (Peyda font, `#16509D` palette, font sizes), the sort/filter script, and a full-width `#16509D` top banner with the white brand-140 mark (static HTML, first child of `section[data-name="{{_bot_name}}"]`, so it renders before the header/filter-form/table — not JS-injected, no timing dependency; white mark because the banner itself is the `#16509D` colored surface, per the background-pairing rule in `assets/brand140/README.md`), scoped via the template's own `{{_bot_name}}` placeholder so it applies **per rendered bot, automatically, for every RES-based bot on the platform** — not just the 3 in this repo — once uploaded to bot 304. See `src/res_v2_attachments/README.md` for exactly what changed and the upload caveat (this is a shared/platform-wide file — verify visually before/after uploading).
