-- botName = CustomersWhoDoNotHaveASalesAgent
...
function getData(queryType , pageFrom , perPage , pageTo )
    ---- init Query
    local dataQuery = {
        query = teamyar.get_attachment("query_customers_dont_have_sales_agent.txt") ,
        params = {}
    };
...
function customerAcl(data)
    local query = teamyar.get_attachment("query_customer_dont_have_sales_agent_acl.txt");
