...
.PARAMETER ResultFormat
  Output format for new/update bot. Default: html (نوع خروجی = HTML, result_type=0)
...
$categories = "${CatId}_1"
# نوع خروجی = HTML (Teamyar UI) → result_type=0 (پیش‌فرض)؛ با -ResultTypeValue قابل بازنویسی است
$resultTypeValue = if ($ResultTypeValue -ge 0) { [string]$ResultTypeValue } else { '0' }
