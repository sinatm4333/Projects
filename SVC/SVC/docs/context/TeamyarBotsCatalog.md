...
| `crm_rfm_dashboard_report_bot.lua` (جدید، هنوز مستقر نشده) | HTML/JSON | `from_date`, `to_date` (شمسی، پیش‌فرض سال مالی جاری)، `org_id` (اختیاری)، `format=json`، `action=send_sms\|send_sms_bulk` (+`crm_id`/`crm_ids`، `segment`، `text`) | نسخهٔ داشبوردی بات «RFM CRM» (بات ۵۰۱/۶۰۰) — همان منطق داده/کوئری/فیلتر تاریخ/پیامک `crm_rfm_report_bot.lua` (دست‌نخورده)، ولی به‌جای جدول تخت: کارت‌های KPI (تعداد مشتری، مجموع خرید، میانگین امتیاز RFM، میانگین بازگشت، Champion/At Risk با درصد و کلیک‌پذیر برای فیلتر جدول) + ۴ نمودار Clickable بدون کتابخانهٔ خارجی به سبک `crm_geo_sales_dashboard_bot.lua` (توزیع سگمنت، سهم مبلغ از سگمنت به‌صورت دونات، میانگین R/F/M هر سگمنت، ۱۰ مشتری برتر) که با کلیک، جدول کامل مشتریان زیرشان را فیلتر/هایلایت می‌کنند، به‌علاوهٔ جعبهٔ جستجوی نام/موبایل. جدول کامل + ارسال پیامک تکی/گروهی حفظ شده. **هنوز روی Teamyar مستقر نشده** — این سشن دسترسی شبکه به `erp.bimehland.com` ندارد؛ برای استقرار، `scripts/deploy_teamyar_bot.ps1` را با `-RunPath crm_rfm_dashboard` و `-BotId 0` (ساخت بات جدید در `cat_id=79`) اجرا کنید |
