...
if ctype == 3 then 
 local has_club_config=getQueryResponse("select count(*) count  from admin_mem_user_cache_value where user_id="..user_id,{});
    ...
    profile_mobile m on p.id=m.user_id left join profile_email e on e.user_id=p.id  left join  admin_user u  on  u.id=p.id where p.id=]]..user_id..[[ order by m.id,e.id desc limit 1) tmp]],{})

  if res_data==nil then 
    res_data=""
  end
  local position=getQueryResponse([[select coalesce(op.name,"--")name   from hr_personnel_order hor inner join org_position op  on hor.position_id=op.id
  inner join hr_personnels r on r.personnel_id=hor.personnel_id where r.profile_id=]]..user_id..[[ order by hor.id desc limit 1 ]],{})
  if position==nill then 
    position="--";
  end

      local listdata = {data=res_data, has_club_config=has_club_config,position=position};
  teamyar.write_result(json.encode(listdata));
	...
	if userinfo.lang_id == 4 then
 	...
 	local srlang = "<script src='/bot/run/2/p_2/"..lang..".js'></script>";
	res_data = [[
		<div id='myDiv'></div>
 		 ]]..srlang..[[
       <link href='/bot/run/2/p_2/main.css' rel='stylesheet' /> 
		<script src='/bot/run/2/p_2/main.js'></script>
