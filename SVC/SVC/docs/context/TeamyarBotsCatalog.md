...
| `external_sms_gateway_bot.lua` (جدید v01، هنوز مستقر نشده) | JSON [api] | تکی: `secret_key`, `mobile`, `message` — صف: `secret_key`, `items:[{mobile,message},...]` (حداکثر ۲۰۰ آیتم) | SMS Gateway برای فراخوانی از نرم‌افزار خارجی (External Software → این بات → `teamyar.call_api(16,'/api/sms/send',...)` → SMS API). ارسال مستقیم به شماره موبایل با `send_to.mobile_numbers=[{value,country}]` (country=364 ایران) — طبق مستندات رسمی پورتال در `TeamyarInternalApiReference.md` («ماژول پیامک — module_id 16»)، نه حدسی؛ در `messages[].send_to.profile_ids` که بات‌های قدیمی (`crm_rfm_bot.lua`) استفاده می‌کردند تفاوت دارد. احراز هویت با `secret-key` HTTP Header (`teamyar.get_http_header`، الگوی زندهٔ `call_api_1_bot.lua`)، مقایسه با `secret_key` تب پیکربندی بات (fallback: ثابت در ابتدای کد). خطاها: `mobile_required`, `message_required`, `invalid_secret_key`. `message_id` فقط اگر واقعاً در پاسخ API بود برگردانده می‌شود. هر خطا با `pcall` کنترل می‌شود؛ Log ورودی بدون `secret_key`/متن کامل پیامک (فقط طول پیام) + Log کامل پاسخ API |
