...
- **Responsive design — mandatory for all `result_type=HTML` bots (added 1405/05/26).** Full standard (mobile-first, container/grid system, table strategies, breakpoints, RTL, QA checklist): `.cursor/rules/responsive-design.mdc`. It preserves the `white-space: nowrap` / `table { width: auto }` / `td,th { width: 1% }` rules above unchanged — tables scroll inside their own wrapper, never wrap/shrink cells. **Exception: RES-based bots are out of scope** for this standard — their table is rendered by Teamyar's core `$.Teamyar.table` widget, which this project does not control (see "RES-framework bots" below).
