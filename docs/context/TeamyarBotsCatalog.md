...
| 582 | تسویه خودکار (بدون UI/پیوست، بر اساس کانفیگ بات؛ منبع اصلی — دست‌نخورده نگه داشته شود) | (سورس محلی نداریم — کد از کاربر دریافت شد، فایل محلی مربوطه ساخته نشده تا 582 دست‌نخورده بماند) | نامشخص |
| 612 | تسویه خودکار — نسخه صف‌بندی‌شده (کپی بات ۵۸۲؛ ۱۴۰۵/۰۵/۳۱). هر اجرا حداکثر ۲۰ فاکتور (`_SETTLE_BATCH_SIZE`) از صف تسویه می‌کند — فیلتر/کوئری عیناً از ۵۸۲، فقط `order by i.RUN_DATE asc limit 20` اضافه شده. فاکتورهای ناموفق/باقی‌مانده در صف باقی می‌مانند و با اجرای بعدی (دستی یا زمان‌بندی‌شده) دوباره پردازش می‌شوند تا کل صف — از هر تعدادی — تسویه بخورد. رفع باگ همراه: ثبت `update_invoice_history` اکنون فقط روی تسویه‌ی واقعاً موفق انجام می‌شود (نسخه‌ی ۵۸۲ آن را بدون شرط صدا می‌زد) + `pcall` روی هر دو فراخوانی API. **هنوز روی پنل Teamyar دیپلوی نشده — کاربر دستی دیپلوی می‌کند.** | `sales_settlement_auto_queue_bot.lua` | نامشخص (بعد از دیپلوی دستی تکمیل شود) |
