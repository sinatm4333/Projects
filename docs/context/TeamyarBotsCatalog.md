...
| `finance_activity_discovery_json_bot.lua` | JSON | `mode` (`all`/`group_members`/`sales_probe`/`purchase_probe`/`accounting_probe`/`dimdate_probe`/`rawp`), `group_id` (default 36390), `from_jdate`/`to_jdate` or `from_raw`/`to_raw` | Discovery-only probe for the finance-unit activity dashboard (`finance_activity_dashboard_v01`, not yet built): group membership (`profile_group_member`+`profile_main`+`admin_user`), sales (`sales_invoice`+`sales_history`), purchase (`purchase_invoice`+`purchase_history`), accounting (`pa_voucher`+`pa_voucher_signs`) — counts + real CONTENT/NOTE samples to prove TYPE/STATUS/SIGN meaning before writing the final dashboard bot; no ACTIVE/DISABLE user-status column exists on `admin_user`/`profile_main` (confirmed absent, not guessed) |
