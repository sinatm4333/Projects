...
function getQueryResponse(data, from, count)
    ...
    box_clause = [[ AND ConnectedLineProfileId = ]]..user;
    ...
    teamyar.write_log(query_total)

    db.query(params) 
    while db.query_fetch(record) do 
        if(string.len(record[1]) >0) then
...
local res = teamyar.run_command("2/res_bot",{
    id = "voip_inbound_calls_details",
    tpl_name = "table",
    lang=1,
    path="2/voip_inbound_calls_details",
    title = "INBOUND_CALL_DETAILS",
    ...
    ajax = [[{url:'bot/run/2/voip_inbound_calls_details',
        data:()=>{
            return  ty__main.botOnClickApplyInCall();
        }
    }]],
    script=[[
        (function(){
...
elseif input.type == 4 then
    ...
    -- local queresult =  getQueryResponse(input.from,input.count);
