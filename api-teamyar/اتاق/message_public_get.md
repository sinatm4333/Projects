# دریافت پیغام های گفتگوی عمومی

## آدرس

```
/api/message/public/get
```
...
| `data.messages[].type` | integer (int32) | نوع |
| `data.messages[].content` | string | متن پیام |
| `data.messages[].user_id` | integer (int64) | شناسه کاربر |
| `data.messages[].reply_id` | integer (int64) | شناسه پیام پاسخ داده شده |
...
| `data.messages[].create_date` | integer (date) | تاریخ ایجاد پیام |
...
| `data.messages[].related_user_id` | integer (int64) | شناسه کاربر مرتبط |
