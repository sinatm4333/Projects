...
function getQueryResponse(query,query_params)
  ...
  username = res_text[1]
  password = res_text[2]
  teamyar.write_log("u_..."..username.."--p--"..password)
end
    ...
    if curl:sendRequest(paramsc) then
      if curl:getStatus() == 200 then
        if curl:getResponse() ~= nil then
          local response=curl:getResponse()
            teamyar.write_log("msg---"..json.encode(response))
      --    tocken = json.decode(response).access_token
...
function changeStatus(tocken, status, user_id)
  ...
  if curl:connect({domain="voip.bimehland.com", port = 443, ssl = true, secure = false}) then
    ...
    if curl:sendRequest(params) then
        teamyar.write_log("curl:getStatus() change---"..json.encode(curl:getStatus()))
...
function getStatus(tocken)
  ...
  if curl:connect({domain = "voip.bimehland.com", port = 443, ssl = true, secure = false}) then
    ...
    if curl:sendRequest(params2) then
      ...
      teamyar.write_log("ssss--"..status_url)
...
elseif type_input == 3 then 
 ...
 -- local srlang = "<script src='/bot/run/2/change_alovoip_status_1/"..lang..".js'></script>";
    local srlang = "<script src='/bot/run/2/change_alovoip_status/"..lang..".js'></script>";
  res_data = [[
                    <div id='myDiv'></div>
                    ]]..srlang..[[
                    <link href='/bot/run/2/change_alovoip_status_1/main.css' rel='stylesheet' /> 
                    <script src='/bot/run/2/change_alovoip_status_1/main.js'></script>
                    ]];
