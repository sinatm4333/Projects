...
-- version = v01
-- خواستهٔ کاربر: داشبورد فروش (نه CRM/جغرافیا) که فروش را به تفکیک «مرکز درآمد» فاکتور نشان دهد، شامل
-- نمودار قابل‌کلیک پرفروش‌ترین/کم‌فروش‌ترین کالا، ۱۰ مشتری برتر B2B و ۱۰ مشتری برتر B2C، همیشه محدود به
-- بازهٔ تاریخ داده‌شده در فیلتر، و چند حالت (تب) «کل فروش / B2B / B2C».
--
-- خواهر همین بات: crm_geo_sales_dashboard_bot.lua (محور CRM/جغرافیا). این بات محور فروش/مرکز درآمد است؛
-- Query ها، فرمول مبلغ فاکتور، و Rule تفکیک B2B/B2C عیناً از همان بات (نسخهٔ زندهٔ تأییدشده، v03) به ارث
-- رسیده‌اند تا اعداد دو بات با هم ناسازگار نشوند:
--
-- فاکتور معتبر = sales_invoice: DELETED=0 AND CANCELED=0 AND PRE_INVOICE=0
-- مبلغ فاکتور = فرمول واقعی Production (نه RECEPTION_AMOUNT+REMAINED_AMOUNT) — SUM روی
-- sales_invoice_product با علامت منفی برای TYPE=3 (برگشت از فروش)؛ منبع: دو بات مستقل تولیدی
-- (id=433/440، ر.ک. docs/context/SalesReportBotsReference.md یافتهٔ ۳) + crm_geo_sales_dashboard_bot.lua
-- (INVOICE_AMOUNT_JOIN، به تأیید صریح کاربر 1405/05/24). عیناً کپی شده — تغییر داده نشود.
-- مرکز درآمد = sales_invoice.SALES_CENTER -> pa_center.NAME
-- تفکیک B2B/B2C = بر اساس نام مرکز درآمد فاکتور (LIKE «همکار»/«مصرف»)؛ تأییدشده با Query زنده روی
-- دادهٔ واقعی در crm_geo_sales_dashboard_bot.lua (1405/05/24): «فروش همکار»+«فروش همکار آفلاین» => B2B،
-- «فروش مصرف کننده» => B2C؛ بقیه (فروش فروشگاه/هئیت مدیره، حجم ناچیز) => «سایر». همان محدودیت شناخته‌شده
-- آن‌جا هم اینجا صادق است: این یک Heuristic بر مبنای نام است، نه یک فیلد دستهٔ مستقل.
-- تاریخ فاکتور = sales_invoice.RUN_DATE (FILETIME) — تبدیل شمسی فقط از طریق report_dimdate.
-- نکتهٔ حیاتی (طبق همان پروژه): مقادیر FILETIME همیشه Opaque بین Query ها رد و بدل می‌شوند؛ هرگونه
-- جمع/تفریق/باقیمانده روی آنها فقط در SQL انجام می‌شود، هرگز در Lua.
--
-- تفاوت کلیدی با crm_geo_sales_dashboard_bot.lua: این بات یک بازهٔ تاریخ کامل (از/تا) دارد، نه فقط «از
-- تاریخ X تا امروز» — طبق درخواست صریح کاربر («حتما بازه‌ای که در فیلتر تاریخ می‌دهم رو بیاره»).
...
local CONFIG = {
 DAY_TICKS = 864000000000, -- یک روز بر حسب تیک FILETIME — فقط در SQL استفاده می‌شود
 TOP_PRODUCTS_COUNT = 12,
 BOTTOM_PRODUCTS_COUNT = 12,
 TOP_CUSTOMERS_COUNT = 10,
 OTHER_SLICE_THRESHOLD_PCT = 2.0, -- مراکز زیر این سهم در نمودار Donut داخل «سایر» گروه می‌شوند
 CRM_EDIT_URL_PREFIX = "/?page=/crm/client/edit/",
 -- پیشگیرانه با Concatenation ساخته می‌شود (نه Literal «&section=2») — روی بات خواهر
 -- (crm_geo_sales_dashboard_bot.lua) تأیید زندهٔ 1405/05/26: همین رشتهٔ Literal هنگام ذخیرهٔ command
 -- توسط Teamyar به «§ion=2» تبدیل شده بود (Decode حریصانهٔ «&sect» به Entity «§»، دقیقاً همان خانوادهٔ
...
-- پیاده‌سازی مرجع escape_html (طبق CLAUDE.md، الزامی — هر Entity باید با Concatenation ساخته شود، نه
-- Literal مستقیم؛ رشتهٔ Entity پیوستهٔ Literal هنگام ذخیرهٔ command توسط Teamyar بی‌صدا Decode می‌شود و
-- کامپایل را می‌شکند — تأیید زندهٔ 1405/05/23).
...
-- ============================================================
-- DATE HELPERS (فقط report_dimdate — بدون REPORT_FN_JDATE و بدون محاسبهٔ دستی Lua روی FILETIME)
-- ============================================================
...
-- ابتدای روز جاری (DATEKEY) — همیشه محاسبه در SQL، هرگز در Lua (ر.ک. یادداشت بالای فایل دربارهٔ FILETIME)
local function fetch_today_datekey()
 local rows, err = fetch_rows(
 "SELECT ((UNIX_TIMESTAMP() + 11644473600) * 10000000) - MOD((UNIX_TIMESTAMP() + 11644473600) * 10000000, " ..
 CONFIG.DAY_TICKS .. ")", {})
 if rows == nil or #rows == 0 then return nil, err end
 return rows[1][1]
end
...
-- بازهٔ تاریخ فعال (از/تا) — طبق درخواست صریح کاربر، برخلاف crm_geo_sales_dashboard_bot.lua (که فقط
-- «از تاریخ X تا امروز» دارد)، اینجا هر دو سر بازه از فیلتر کاربر خوانده می‌شوند و همیشه اعمال می‌شوند.
-- پیش‌فرض: از = ابتدای سال مالی جاری، تا = امروز (هر دو در نبود ورودی معتبر کاربر).
local function resolve_active_date_range(input)
 input = input or {}
 local warnings = {}
 ...
 local from_key = nil
 local from_label = date_from_text
 if date_from_text ~= nil then
 from_key = resolve_date_from_text(date_from_text)
 if from_key == nil then
 table.insert(warnings, "تاریخ شروع واردشده («" .. date_from_text .. "») در تقویم سیستم یافت نشد؛ به ابتدای سال مالی جاری بازگردانده شد.")
 end
 end
 if from_key == nil then
 from_key = fy_key
 from_label = fy_jndate
 end

 local date_to_text = input["date_to"]
 if date_to_text == "" then date_to_text = nil end
 local to_key = nil
 local to_label = date_to_text
 if date_to_text ~= nil then
 to_key = resolve_date_from_text(date_to_text)
 if to_key == nil then
 table.insert(warnings, "تاریخ پایان واردشده («" .. date_to_text .. "») در تقویم سیستم یافت نشد؛ به امروز بازگردانده شد.")
 end
 end
 if to_key == nil then
 local today_key = fetch_today_datekey()
 to_key = today_key
 if today_key ~= nil then
 local jr = fetch_rows("SELECT JNDATE FROM report_dimdate WHERE DATEKEY = ? LIMIT 1", { today_key })
 to_label = (jr ~= nil and jr[1] ~= nil) and jr[1][1] or nil
 else
 to_label = nil
 end
 end
 ...
 return {
 from_key = from_key, from_label = from_label,
 to_key = to_key, to_label = to_label,
...
local function parse_filters(input)
 ...
 local center = input["center"]
 if center == "" then center = nil end
 return { channel = channel, center = center }
end

-- ============================================================
-- CHANNEL (B2B/B2C) / CENTER FILTER CLAUSES
-- ============================================================
-- هر دو Clause خودکفا و مستقل از JOIN های Query هستند (فقط به alias «si» = sales_invoice نیاز دارند)،
-- دقیقاً همان معماری build_channel_clause در crm_geo_sales_dashboard_bot.lua — طبق همان دلیل مستند آن‌جا:
-- بدون وابستگی به ترتیب JOIN، هم در WHERE بیرونی و هم داخل ON یک LEFT JOIN قابل استفاده است.

-- نکتهٔ حیاتی (کشف‌شده 1405/05/26 با تست مستقیم و ایزوله روی bot 589/schema_probe_v2 — نه حدس):
-- پارامتری‌کردن LIKE با «?» در لایهٔ db.query این پلتفرم Teamyar شکست می‌خورد — پارامتر رسیده بی‌صدا
-- نادیده گرفته می‌شود (تعداد Placeholder با تعداد Param واقعی که به query اجرا می‌رسد جور درنمی‌آید) و
-- «sql error» برمی‌گرداند؛ درحالی‌که «= ?» پارامتری (ر.ک. build_center_clause) کاملاً درست کار می‌کند.
-- تأیید شد با تست ایزولهٔ `WHERE col = ?` (موفق) در برابر `WHERE col LIKE ?` (شکست، حتی بدون هیچ
...
-- ============================================================
-- INVOICE AMOUNT (فرمول واقعی Production) — کپی عیناً از crm_geo_sales_dashboard_bot.lua (تغییر نکند)
-- ============================================================
...
-- تجمیع شده و برای Group By در سطح کالا قابل استفاده نیست. عبارت ریاضی عیناً همان بالاست.
local DIGIT_FEE_SUBQUERY = [[
(
 SELECT po.ORG_ID,
 (CASE WHEN COALESCE(ps.FEE_DECIMAL, 0) = 0 THEN COALESCE(ps.DECIMAL_COUNT, 0) ELSE COALESCE(ps.FEE_DECIMAL, 0) END) AS digit_fee
 FROM pa_organizations po
...
local PRODUCT_LINE_QTY_SQL =
 "SUM((CASE WHEN si.TYPE = 3 THEN -1 ELSE 1 END) * COALESCE(ip.QUANTITY / POW(10, COALESCE(sc.DECIMAL_NUM, 0)), 0)) AS total_quantity"

local PRODUCT_LINE_AMOUNT_SQL = [[
SUM(
 (CASE WHEN si.TYPE = 3 THEN -1 ELSE 1 END) *
 (
 COALESCE((ip.QUANTITY / POW(10, COALESCE(sc.DECIMAL_NUM, 0))) * (ip.FEE / POW(10, COALESCE(dd.digit_fee, 0))), 0)
 - COALESCE(ip.DISCOUNT, 0)
 + COALESCE(ip.VALUE_ADDED, 0)
...
local function fetch_center_aggregation(date_range, filters)
 ...
 local rows, err = fetch_rows([[
SELECT
 COALESCE((SELECT pcc_s.NAME FROM pa_center pcc_s WHERE pcc_s.ID = si.SALES_CENTER), 'بدون مرکز درآمد') AS center_name,
...
ORDER BY total_sales_amount DESC
 ...
 table.insert(list, {
 center_name = r[1],
 invoice_count = tonumber(r[2]) or 0,
 distinct_clients = tonumber(r[3]) or 0,
 total_sales_amount = tonumber(r[4]) or 0,
 })
 end
 return list
end

local function fetch_center_options()
 local rows, err = fetch_rows(
 "SELECT DISTINCT NAME FROM pa_center WHERE NAME IS NOT NULL AND NAME <> '' ORDER BY NAME", {})
 if rows == nil then return {} end
 local list = {}
 for _, r in ipairs(rows) do table.insert(list, r[1]) end
 return list
end
...
-- طبق درخواست کاربر همیشه هر دو نمایش داده می‌شوند، مستقل از این‌که کدام تب فعال است.
local function fetch_top_customers(date_range, filters, forced_channel, limit)
 ...
 local rows, err = fetch_rows([[
SELECT
 ci.ID AS crm_id, pm.FULLNAME AS customer_name, ui.USER_TYPE AS user_type,
 COUNT(si.ID) AS invoice_count, COALESCE(SUM(ia.amount), 0) AS total_amount
FROM sales_invoice si
...
GROUP BY ci.ID, pm.FULLNAME, ui.USER_TYPE
...
LIMIT ?
 ...
 for _, r in ipairs(rows) do
 table.insert(list, {
 crm_id = r[1], customer_name = r[2], user_type = r[3],
 invoice_count = tonumber(r[4]) or 0, total_amount = tonumber(r[5]) or 0,
 })
 end
 return list
end
...
local function fetch_product_invoices(product_code, date_range, filters)
 local channel_extra, channel_params = build_channel_clause(filters)
 local center_extra, center_params = build_center_clause(filters)
 local params = { product_code, date_range.from_key, date_range.to_key }
 for _, p in ipairs(channel_params) do table.insert(params, p) end
 for _, p in ipairs(center_params) do table.insert(params, p) end

 local rows, err = fetch_rows([[
SELECT si.ID, si.INVOICE_CODE, rd.JNDATE, pm.FULLNAME, ]] .. PRODUCT_LINE_QTY_SQL .. [[, ]] .. PRODUCT_LINE_AMOUNT_SQL .. [[
...
WHERE wp.FULL_CODE = ? AND si.DELETED = 0 AND si.CANCELED = 0 AND si.PRE_INVOICE = 0
 AND si.RUN_DATE >= ? AND si.RUN_DATE < (? + ]] .. CONFIG.DAY_TICKS .. [[)
]] .. channel_extra .. center_extra .. [[
GROUP BY si.ID, si.INVOICE_CODE, rd.JNDATE, pm.FULLNAME
...
LIMIT 300
 ...
 for _, r in ipairs(rows) do
 table.insert(list, {
 invoice_id = r[1], invoice_code = r[2], run_jdate = r[3], customer_name = r[4],
 quantity = tonumber(r[5]) or 0, amount = tonumber(r[6]) or 0,
 })
 end
 return list
end

-- ------------- Drill-down: فاکتورهای یک مشتری (Lazy — هنگام کلیک روی ردیف ۱۰ مشتری برتر) -------------
local function fetch_customer_invoices(crm_id, date_range, filters)
 ...
 local rows, err = fetch_rows([[
SELECT si.ID, si.INVOICE_CODE, rd.JNDATE, COALESCE(ia.amount, 0)
FROM sales_invoice si
INNER JOIN pa_client pc ON pc.ID = si.CLIENT_ID
LEFT JOIN report_dimdate rd ON rd.DATEKEY = (si.RUN_DATE - MOD(si.RUN_DATE, ]] .. CONFIG.DAY_TICKS .. [[))
]] .. INVOICE_AMOUNT_JOIN .. [[
...
LIMIT 300
 ...
 for _, r in ipairs(rows) do
 table.insert(list, {
 invoice_id = r[1], invoice_code = r[2], run_jdate = r[3], invoice_amount = tonumber(r[4]) or 0,
 })
 end
 return list
end
...
local function enrich_center_list(centers, total_sales)
 for _, c in ipairs(centers) do
 c.avg_invoice_amount = (c.invoice_count > 0) and (c.total_sales_amount / c.invoice_count) or 0
 c.sales_share_pct = fmt_pct(c.total_sales_amount, total_sales)
 end
 return centers
end

local function build_center_pie_slices(centers)
 local total = 0
 for _, c in ipairs(centers) do total = total + c.total_sales_amount end
 local slices = {}
 local other_total = 0
 for _, c in ipairs(centers) do
 local pct = fmt_pct(c.total_sales_amount, total)
 if pct >= CONFIG.OTHER_SLICE_THRESHOLD_PCT then
 table.insert(slices, { name = c.center_name, value = c.total_sales_amount, pct = pct })
 else
 other_total = other_total + c.total_sales_amount
 end
 end
...
/* تمام‌صفحه = CSS-only (نه Fullscreen API واقعی) — طبق تأیید زندهٔ 1405/05/26 روی بات خواهر
 (crm_geo_sales_dashboard_bot.lua): روی موبایل/iframe پنل Teamyar، Element.requestFullscreen()
 بازمی‌شد ولی اسکرول نمی‌شد؛ به‌جایش فقط یک کلاس CSS با position:fixed اضافه/حذف می‌شود. */
...
.minmax-grid{ display:grid; grid-template-columns:repeat(auto-fit,minmax(240px,1fr)); gap:12px; margin-bottom:16px; }
...
.modal-content{ background:#fff; border-radius:14px; max-width:720px; width:92%; max-height:82vh; overflow-y:auto; padding:0; }
...
.bar-row, .combo-row{ display:grid; grid-template-columns:130px 1fr 130px; align-items:center; gap:8px; cursor:pointer; padding:2px 0; }
.combo-row{ grid-template-columns:130px 1fr; }
...
local function render_top_customers_rows(list)
 ...
 local crm_url = CONFIG.CRM_EDIT_URL_PREFIX .. tostring(c.crm_id) .. CONFIG.CRM_EDIT_URL_SUFFIX
 table.insert(html, '<tr class="clickable" data-crmid="' .. tostring(c.crm_id) .. '" data-name="' ..
 escape_html(c.customer_name) .. '" onclick="openCustomerInvoicesFromRow(this)"><td>' .. tostring(i) ..
 '</td><td>' .. escape_html(c.customer_name) .. '</td><td>' .. fmt_num(c.invoice_count) .. '</td><td>' ..
 fmt_num(c.total_amount) .. '</td><td onclick="event.stopPropagation();"><a class="link" target="_blank" href="' ..
 crm_url .. '">مشاهده CRM</a></td></tr>')
 end
 if #list == 0 then
 table.insert(html, '<tr><td colspan="5" class="empty-row">در این بازه/فیلتر مشتری‌ای یافت نشد</td></tr>')
 end
...
local function render_filter_bar(filters, date_range, center_options)
 local html = {}
 ...
 table.insert(html, '<div class="filter-field"><label>از تاریخ (شمسی)</label><input type="text" name="date_from" id="dateFromInput" placeholder="1405/01/01" value="' ..
 escape_html(date_range.from_label) .. '"></div>')
 table.insert(html, '<div class="filter-field"><label>تا تاریخ (شمسی)</label><input type="text" name="date_to" id="dateToInput" placeholder="1405/05/26" value="' ..
 escape_html(date_range.to_label) .. '"></div>')
 table.insert(html, '<div class="filter-field"><label>مرکز درآمد</label><select name="center" id="centerFilterInput"><option value="">همهٔ مراکز</option>')
 for _, cname in ipairs(center_options) do
 local sel = (filters.center == cname) and ' selected' or ''
 table.insert(html, '<option value="' .. escape_html(cname) .. '"' .. sel .. '>' .. escape_html(cname) .. '</option>')
 end
...
function setCenterFilterFromRow(tr){ setCenterFilter(tr.getAttribute('data-center')); }
function escapeHtml(v){
 if (v === null || v === undefined) return '';
 /* طبق قانون CLAUDE.md دربارهٔ Quirk شناخته‌شدهٔ تیم‌یار: امپرسند با String.fromCharCode(38) در زمان
 اجرا ساخته می‌شود، نه به‌صورت رشتهٔ Entity کامل پیوسته در سورس (که هنگام ذخیرهٔ command بی‌صدا
 Decode می‌شود و اسکریپت صفحه را می‌شکند). */
...
function renderBarChart(containerId, items, opts){
 ...
 if (!el) return;
 if (!items || items.length === 0) { el.innerHTML = '<div class="empty-row">داده‌ای برای نمایش نیست</div>'; return; }
 var max = 0;
...
/* ---------------- Combo chart: دو معیار در کنار هم برای هر مرکز درآمد ---------------- */
function renderComboChart(containerId, items, optsA, optsB, labelKey){
 ...
 showTip(e, it[labelKey] + ' — ' + optsA.label + ': ' + fmtNum(it[optsA.key]) + ' | ' + optsB.label + ': ' + fmtNum(it[optsB.key]));
 });
 row.addEventListener('mouseleave', hideTip);
 });
}
...
function renderDonutChart(containerId, slices, onClick){
 var el = document.getElementById(containerId);
 if (!el) return;
 if (!slices || slices.length === 0) { el.innerHTML = '<div class="empty-row">داده‌ای برای نمایش نیست</div>'; return; }
...
function exportToExcel(){
 var table = document.getElementById('centerTable'); if (!table) return;
 ...
 downloadCsv(lines, 'گزارش-فروش-مراکز-درآمد.csv');
}

/* ---------------- داده تعبیه‌شده ---------------- */
var DASH = JSON.parse(document.getElementById('dashData').textContent);

/* ---------------- Drill-down: فاکتورهای یک کالای خاص (Lazy AJAX، در Modal) ---------------- */
function openProductInvoices(productCode, productName){
 var modal = document.getElementById('invoiceModal');
 var body = document.getElementById('invoiceModalBody');
 document.getElementById('invoiceModalTitle').textContent = 'فاکتورهای کالا: ' + (productName || '');
 body.innerHTML = '<div class="loading-row">در حال بارگذاری...</div>';
 modal.classList.add('active');
 var fd = new FormData();
 fd.append('action', 'product_invoices');
 ...
 var rowsHtml = rows.length === 0 ? '<tr><td colspan="5" class="empty-row">فاکتوری یافت نشد</td></tr>' : rows.map(function(inv){
 ...
 escapeHtml(inv.invoice_code) + '</td><td>' + escapeHtml(inv.run_jdate || '—') + '</td><td>' +
 escapeHtml(inv.customer_name) + '</td><td>' + fmtNum(inv.amount) + '</td></tr>';
 }).join('');
 body.innerHTML = '<div class="table-scroll"><table class="data-table"><thead><tr><th>شناسه فاکتور</th><th>کد فاکتور</th><th>تاریخ</th><th>مشتری</th><th>مبلغ</th></tr></thead><tbody>' + rowsHtml + '</tbody></table></div>';
...
/* ---------------- Drill-down: فاکتورهای یک مشتری (Lazy AJAX، در Modal) ---------------- */
function openCustomerInvoicesFromRow(tr){ openCustomerInvoices(tr.getAttribute('data-crmid'), tr.getAttribute('data-name')); }
function openCustomerInvoices(crmId, customerName){
 var modal = document.getElementById('invoiceModal');
 var body = document.getElementById('invoiceModalBody');
 document.getElementById('invoiceModalTitle').textContent = 'فاکتورهای ' + (customerName || '');
 body.innerHTML = '<div class="loading-row">در حال بارگذاری...</div>';
 modal.classList.add('active');
 var fd = new FormData();
 fd.append('action', 'customer_invoices');
...
function resetFilters(){
 ...
 document.getElementById('centerFilterInput').value = '';
...
local function render_html(args)
 ...
 table.insert(html, '<title>داشبورد فروش به تفکیک مرکز درآمد</title>\n')
 ...
 '<h1>داشبورد فروش به تفکیک مرکز درآمد</h1>' ..
 '<p class="sub" id="headerSubText">' .. escape_html(args.header_sub_text) .. '</p></header>\n')
...
<div class="section" id="centerSection">
 <div class="section-head"><h2><span class="num">۱</span>فروش به تفکیک مرکز درآمد</h2><p>روی هر میله/برش نمودار یا ردیف جدول کلیک کنید تا داشبورد به همان مرکز درآمد محدود شود</p></div>
 <div class="grid-2">
 <div class="card"><h3>مبلغ فروش هر مرکز درآمد</h3><div class="desc">مرتب‌شده نزولی بر اساس مبلغ فروش</div><div class="chart-box" id="salesByCenterChart"></div></div>
 <div class="card"><h3>سهم هر مرکز از کل فروش</h3><div class="desc">مراکز کوچک زیر ]] .. fmt_dec1(CONFIG.OTHER_SLICE_THRESHOLD_PCT) .. [[٪ در «سایر» گروه شده‌اند — دادهٔ خام همچنان در جدول کامل موجود است</div><div class="chart-box" id="salesCenterDonutChart"></div></div>
 </div>
...
<div class="section" id="topCustomersSection">
 ...
 <tbody id="topB2BTbody">]] .. args.top_customers_b2b_html .. [[</tbody></table></div>
 </div>
 <div class="card"><h3>۱۰ مشتری برتر B2C (مصرف‌کننده)</h3>
 <div class="table-scroll"><table class="data-table" id="topB2CTable"><thead><tr><th>#</th><th>نام مشتری</th><th>تعداد فاکتور</th><th>مبلغ خرید</th><th>CRM</th></tr></thead>
 <tbody id="topB2CTbody">]] .. args.top_customers_b2c_html .. [[</tbody></table></div>
 </div>
 </div>
</div>

<footer id="footerText">]] .. render_footer_html(args.footer_text) .. [[</footer>
]])
...
<div id="helpModal" class="modal"><div class="modal-content">
 ...
 <p>این داشبورد فروش سازمان را بر پایهٔ دادهٔ زندهٔ Teamyar، به تفکیک «مرکز درآمد» فاکتور (sales_invoice.SALES_CENTER) نشان می‌دهد.</p>
 <h4>KPIهای بالای صفحه</h4>
 <ul>
 <li><b>مبلغ کل فروش:</b> جمع مبلغ خالص فاکتورهای معتبر (فاکتورهای برگشتی با علامت منفی کسر شده‌اند)، در بازهٔ تاریخ و فیلترهای فعال.</li>
 <li><b>تعداد مراکز درآمد فعال:</b> تعداد مراکز درآمد متمایزی که در این بازه حداقل یک فاکتور داشته‌اند.</li>
 </ul>
...
local function main()
 ...
 local header_sub_text = "بازه: از " .. tostring(date_range.from_label) .. " تا " .. tostring(date_range.to_label) ..
 ...
 local filter_bar_html = render_filter_bar(filters, date_range, center_options)
