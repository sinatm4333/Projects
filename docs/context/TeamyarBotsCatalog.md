...
| `document_content_access_probe_json_bot.lua` (بات **تشخیصی موقت** — کارش تمام شد، مکانیزم نهایی استخراج و در `vat_quarterly_dashboard_report_bot.lua` استفاده شد؛ نگه‌داشته شده فقط برای مرجع/تکرار روی سند دیگر) | JSON | بدون ورودی الزامی (`document_id` ثابت `TEST_DOCUMENT_ID=19025` در کد؛ اگر ورودی `document_id`/`id`/`doc_id` داده شود همان استفاده می‌شود) | **نتیجهٔ نهایی (v06، تأییدشدهٔ زنده 1405/06/07، فایل واقعی id=19025 «فصل بهار.csv»، ۱۳٬۳۱۶٬۱۲۲ بایت)**: محتوای یک سند از ماژول «اسناد» فقط از طریق سه تابع مستندنشدهٔ روی شیء `teamyar` قابل خواندن است — `documents_main` فقط متادیتا دارد، `io`/`os` در Sandbox موجود نیستند. مکانیزم: `local fm = teamyar.create_file_manager(7, document_id)` (۷=module_id ماژول اسناد؛ بدون آرگومان دوم خطای «invalid module id» می‌دهد) → `fm:readFile(document_id)` (**امضای دوآرگومانی الزامی** — `fn(self, document_id)`؛ `readFile(self)` تک‌آرگومانی خطای «integer type required» می‌دهد) → رشتهٔ کامل فایل → `teamyar.csv_to_json(csv_text)` رشتهٔ JSON آرایه‌ای برمی‌گرداند (هر ردیف = آبجکت به کلید سرستون). `fm:getInfo(document_id)` هم با همان امضا متادیتای کامل (نام/حجم/module_id/...) می‌دهد. متدهای نوشتنی `updateFile`/`updateFolder` کشف شدند ولی هرگز فراخوانی نشدند (امضای ناشناخته + ریسک). روند کشف (۶ دور تست زنده روی سرور واقعی، چون این محیط توسعه به `erp.bimehland.com` دسترسی شبکه ندارد) در تاریخچهٔ کامیت‌های همین فایل مستند است. |
| `vat_quarterly_dashboard_report_bot.lua` (جدید، هنوز مستقر نشده) | HTML | ۸ شناسهٔ سند ثابت و مستقل — `sale_q1`..`sale_q4`, `purchase_q1`..`purchase_q4` (هرکدام اختیاری؛ در Query String آدرس اجرا داده می‌شوند، مثال: `?sale_q1=19025&purchase_q1=19024`) | داشبورد ارزش‌افزودهٔ فصلی که **در هر اجرا** (نه فقط یک‌بار آپلود) هر ۸ فایل CSV صورتحساب فروش/خرید سامانهٔ مودیان را مستقیماً از «اسناد» تیم‌یار (با مکانیزم کشف‌شده در `document_content_access_probe_json_bot.lua`) می‌خواند و داشبورد را با دادهٔ تازه می‌سازد. منطق محاسباتی/نمودار/جدول/Drill-down/تسویه عیناً از نسخهٔ اولیهٔ HTML کاربر (که با آپلود دستی فایل در مرورگر کار می‌کرد) گرفته شده — فقط منبع داده (سروری به‌جای آپلود) و ظاهر (پالت اجباری فقط #16509D/سفید/خاکستری/مشکی به‌جای طلایی/سبز/قرمز/کهربایی نسخهٔ اصلی، فونت Peyda کامل Embed، لوگو ۱۴۰ نسخهٔ White در هدر، دکمهٔ راهنما، تمام‌صفحهٔ CSS-only) عوض شده. رنگ چندسری نمودارها (فروش/خرید) از سایه‌های آبی Accent (الگوی `sales_revenue_center_dashboard_report_bot.lua`) استفاده می‌کند، نه رنگ‌های جدید. تب اول («وضعیت داده‌ها») جایگزین تب آپلود قدیمی است — برای هر یک از ۸ اسلات نشان می‌دهد کدام شناسه داده شده، نام فایل واقعی خوانده‌شده، و وضعیت. **نکتهٔ کارایی مهم برای دیپلوی**: فایل‌های واقعی می‌توانند حجیم باشند (نمونهٔ تست‌شده ۱۳ مگابایت) و تا ۸ فایل ممکن است در یک اجرا خوانده شوند — `max_execute_time` را در دیپلوی بالاتر از پیش‌فرض ۱۰۰ ثانیه تنظیم کنید. **تست محلی کامل انجام شد (1405/06/07)** — چون این محیط توسعه به `erp.bimehland.com` دسترسی شبکه ندارد، HTML واقعی خروجی این بات با اجرای مستقیم فایل Lua (روی `lua5.3`، با شبیه‌ساز سبک `teamyar`/`json` — بدون تغییر منطق واقعی بات) تولید و در Chromium واقعی (headless، ویوپورت موبایل ۳۹۰px) بازشد؛ کلیک روی هر ۵ تب، تب‌های فصلی، ردیف جدول گزارش (باز شدن Drawer)، دکمهٔ راهنما، تمام‌صفحه، و خروجی Excel همه تأیید شدند، بدون Overflow افقی صفحه. **دو باگ واقعی همین‌طور پیدا و رفع شد**: ۱) تگ بستهٔ `</script>` بلوک اصلی جاوااسکریپت (شامل تمام منطق تب‌ها/رندر) از قلم افتاده بود — کل اسکریپت هرگز اجرا نمی‌شد (تأیید با شمارش `addEventListener` صفر روی کل صفحه)؛ ۲) CSS دسکتاپ‌محور بود، نه موبایل‌محور — بازنویسی به الگوی mobile-first (تک‌ستونی و نوار تب اسکرول‌پذیر افقی به‌صورت پیش‌فرض، `grid-2`/`grid-4` و هدر با `min-width` برای صفحه‌های بزرگ‌تر توسعه پیدا می‌کنند، نه برعکس). **هنوز روی مکانیزم واقعی خواندن سند (`create_file_manager`/`readFile`/`csv_to_json` زنده روی سرور) تست نشده** — فقط منطق فرانت‌اند/HTML با دادهٔ شبیه‌سازی‌شده تأیید شد؛ بعد از دیپلوی و اجرای واقعی با شناسه‌های سند واقعی، این یادداشت باید به‌روزرسانی شود. |
