-- تحلیل و ایجاد توسط سینا مقدم 09121011778
-- Last Edit = 1405/06/09 20:30

-- botName = action_module  («ماژول اقدام»)
-- بازطراحی ماژول «اقدام» (BPMN/گردش‌کار سازمانی) — یک بات HTML یکپارچه با ساید‌بار داخلی
--
-- هدف (طبق درخواست کاربر): همان ساختار ماژول بومی Teamyar، فقط با گایدلاین ظاهری جدید این ریپو
--   (پالت #16509D، فونت Peyda جاسازی‌شده، لوگو ۱۴۰، هدر مرتب‌سازی‌پذیر، تمام‌صفحه/Excel/راهنما).
--   صفحهٔ اول = داشبورد اقدام‌های «کاربری که بات را اجرا می‌کند».
--
-- ============================================================
-- نگاشت داده (از DatabaseSchema.md — schema 0000000)
-- ============================================================
--   اقدام        = todo_task        (AUTHOR_ID=مؤلف, OWNER_ID=مالک, CATEGORY_ID, TOPIC_ID,
--                                    WORK_FLOW_ID, STATUS 1=باز/2=بسته, T_START_DATE, T_END_DATE=مهلت,
--                                    T_REAL_END_DATE=پایان واقعی, ARCHIVE_FLAG)
--   مرحله        = todo_task_steps  (TASK_ID, STEP_ID, RESPONSIBLE_ID, STATUS 0=جاری/1=تایید/2=کامل/3=رد,
--                                    FLAG_LAST_STEP, DATE_CREATE)
--   تعریف مرحله  = todo_step        (STEP_NAME, duration + time_unit = SLA)
--   رده/موضوع/بخش = todo_category (CATEGORY_TITLE) / todo_topic (NAME) / todo_section (SECTION_NAME)
--   جریان کار    = todo_workflow    (WF_TITLE, CATEGORY_ID, wf_disabled)
--   مطلعین       = todo_notify      (TASK_ID, USER_ID)
--   کامنت‌ها     = todo_task_comments (TASK_ID, AUTHOR_ID)
--   نام افراد    = profile_main.fullname
--   تاریخ‌ها     = Windows FILETIME (تیک ۱۰۰ns از ۱۶۰۱)
--   لینک جزئیات  = /?page=/todo/report/{id}  (لینک بومی Teamyar — تاییدشده در بات ۵۹۲)
--
-- ============================================================
-- تعریف «دامنهٔ کاربر» (تصمیم کاربر، ۱۴۰۵/۰۶/۰۹)
-- ============================================================
--   مسئولم  = مسئولِ مرحلهٔ باز فعلی        -> todo_task_steps.STATUS=0 AND RESPONSIBLE_ID=me
--   مؤلفم   = ایجادکننده/مالک              -> todo_task.AUTHOR_ID=me OR todo_task.OWNER_ID=me
--   عضوم    = مطلع یا درگیر شده             -> todo_notify.USER_ID=me OR todo_task_comments.AUTHOR_ID=me
--   همه     = اجتماع هر سه مورد بالا
--   («تاریخچهٔ کامل مسئولیت» طبق انتخاب کاربر جزو تعریف عضویت نیست.)
--
-- ============================================================
-- ملاحظات «ماژول ساز» (استخراج‌شده از بات مودیان — بازنویسی بات ۵۷۸ «[Module]»)
-- ============================================================
--   ۱) وقتی بات از ماژول‌ساز باز می‌شود، `location.pathname` دیگر `/bot/run/...` نیست (صفحهٔ میزبان است).
--      پس هر فراخوانی AJAX به «خودِ بات» باید RUN_URL را با fallback بسازد:
--          var RUN_URL = (location.pathname.indexOf('/bot/run/') === 0) ? location.pathname : RUN_PATH_FALLBACK;
--      این تنها راه تاییدشده در این کدبیس برای بات‌های داخل ماژول است.
--   ۲) پارامترهای فراخوانی خودِ بات با کلید `customform` (JSON) POST می‌شوند، نه query string:
--          data: { customform: JSON.stringify({ action: "list", ... }) }
--      و سمت Lua با همان `teamyar.get_input()` خوانده می‌شوند.
--   ۳) از `$.Teamyar.ajax` (ویجت هسته‌ای پلتفرم) استفاده می‌شود — داخل ماژول در دسترس است؛
--      با fallback به fetch() اگر بارگذاری نشده باشد (بات ممکن است بیرون از پورتال هم اجرا شود).
--   ۴) همهٔ CSS/JS داخل همین فایل جاسازی شده — بدون پیوست (attachment) و بدون main.css/main.js،
--      چون در ماژول‌ساز مسیر پیوست‌ها قابل اتکا نیست (همان تصمیم بات مودیان).
--   ۵) دکمهٔ «تمام صفحه» با toggle کلاس CSS پیاده شده، نه Fullscreen API — داخل iframe ماژول
--      (بدون allow="fullscreen") محتوا فول‌اسکرین می‌شود ولی اسکرول نمی‌خورد (باگ تاییدشدهٔ بات ۶۰۴).
--   ۶) هیچ ناوبری‌ای صفحهٔ میزبان را reload نمی‌کند؛ تعویض تب/منو کاملاً client-side + AJAX است.
--
-- ============================================================
-- نکات پلتفرمی رعایت‌شده (از CLAUDE.md — همگی باگ‌های تاییدشدهٔ زنده)
-- ============================================================
--   * `LIKE ?` پارامتری در لایهٔ db.query این پلتفرم بی‌صدا شکست می‌خورد -> به‌جایش `LOCATE(?, col) > 0`
--     که هم پارامتری (ضدInjection) است و هم از باگ LIKE عبور می‌کند.
--   * رشته‌های شبیه Entity (نمونه: امپرسند + amp + سمی‌کالن، و حتی امپرسند + sect...) هنگام ذخیرهٔ
--     command توسط Teamyar بی‌صدا decode/مخدوش می‌شوند
--     -> همهٔ آن‌ها با concatenation ساخته شده‌اند.
--   * REPORT_FN_JDATE روی این نشست خطا می‌دهد -> تقویم شمسی کاملاً سمت Lua پیاده شده (کپی از بات ۵۹۲).
--
-- ============================================================
-- نیازمند تایید روی دادهٔ زنده (یک‌بار اجرا، سپس نهایی می‌شود)
-- ============================================================
--   الف) شناسهٔ «کاربر جاری»: در این کدبیس فقط `teamyar.get_user_info().timezone` استفاده شده و شکل کامل
--        خروجی get_user_info مستند نیست. resolve_current_user() چند کلید محتمل را کاوش می‌کند و در
--        نبود همه، ورودی صریح `user_id` را می‌پذیرد. با `action=whoami` می‌توان خروجی خام را دید و
--        کلید واقعی را در CONFIG.USER_ID_KEYS قفل کرد.
--   ب) چهار مرحلهٔ نوار بالای ماژول بومی (پیش‌نویس/بررسی/اجرا/کامل) از ستون‌های todo_task.state /
--        todo_task.progress / todo_step.state می‌آیند که مقادیرشان مستند نیست. برای همین در این نسخه
--        نوار وضعیت با معیارهای ۱۰۰٪ قابل‌اتکا ساخته شده (باز / در جریان / معوق / کامل‌شده) و
--        `action=state_probe` مقادیر مشاهده‌شدهٔ آن سه ستون را برمی‌گرداند تا نگاشت دقیق ۱:۱ در پاس بعدی
--        اضافه شود. حدس‌زدن نگاشت عمداً انجام نشده.
--   ج) تب «منتخب» ماژول بومی: هیچ جدول todo_favorite در schema نیست (برخلاف crm_favorite/project_favorite)
--        -> مکانیزمش ناشناخته است و در این فاز پیاده نشده.

-- ============================================================
-- CONFIG
-- ============================================================

local CONFIG = {
    RUN_PATH_FALLBACK = "/bot/run/443/action_module",
    PAGE_SIZE = 50,
    MAX_PAGE_SIZE = 200,
    ID_CHUNK = 200,
    RECENT_ACTIVITY_LIMIT = 8,
    DUE_SOON_DAYS = 3,
    DAY_TICKS = 10000000 * 86400,
    -- کلیدهای محتملِ شناسهٔ کاربر در خروجی teamyar.get_user_info() — به ترتیب اولویت کاوش می‌شوند.
    -- بعد از اجرای action=whoami روی سرور، کلید واقعی را اول این لیست بگذارید.
    USER_ID_KEYS = { "id", "user_id", "profile_id", "uid", "person_id", "profile_main_id", "USER_ID", "ID" },
    ACCENT = "#16509D",
}

local TASK_STATUS_OPEN = 1
local TASK_STATUS_CLOSE = 2
local STEP_STATUS_CURRENT = 0

local SCOPE_ALL = "all"
local SCOPE_RESPONSIBLE = "responsible"
local SCOPE_AUTHOR = "author"
local SCOPE_MEMBER = "member"

local STATE_ALL = "all"
local STATE_OPEN = "open"
local STATE_RUNNING = "running"
local STATE_OVERDUE = "overdue"
local STATE_DONE = "done"

-- ============================================================
-- ASSETS (base64 — طبق قانون CLAUDE.md هر بات نسخهٔ خودش را حمل می‌کند؛
-- هاست استاتیک مشترکی روی این پلتفرم وجود ندارد)
-- ============================================================

local PEYDA_REGULAR_B64 = ""
local PEYDA_BOLD_B64 = ""
-- لوگو روی نوار #16509D می‌نشیند -> طبق قانون Background-pairing گایدلاین برند ۱۴۰ نسخهٔ White
local LOGO140_WHITE_B64 = ""

-- ============================================================
-- HELPERS
-- ============================================================

-- شکل canonical و اجباری CLAUDE.md: هر Entity با concatenation ساخته می‌شود، چون فرم Literal پیوسته
-- هنگام ذخیرهٔ command توسط Teamyar خطا می‌دهد (تاییدشدهٔ زنده ۱۴۰۵/۰۵/۲۳).
local function escape_html(value)
    if value == nil then return "" end
    local amp_entity = "&" .. "amp;"
    local lt_entity = "&" .. "lt;"
    local gt_entity = "&" .. "gt;"
    local quot_entity = "&" .. "quot;"
    local apos_entity = "&" .. "#39;"
    local out = tostring(value)
        :gsub("&", amp_entity)
        :gsub("<", lt_entity)
        :gsub(">", gt_entity)
        :gsub('"', quot_entity)
        :gsub("'", apos_entity)
    return out
end

-- escape برای درج داخل رشتهٔ JSON که خودش داخل تگ <script> رندر می‌شود
local function escape_js(value)
    if value == nil then return "" end
    local out = tostring(value)
        :gsub("\\", "\\\\")
        :gsub('"', '\\"')
        :gsub("'", "\\'")
        :gsub("\n", "\\n")
        :gsub("\r", "")
        :gsub("<", "\\u003C")
        :gsub(">", "\\u003E")
        :gsub("&", "\\u0026")
    return out
end

local function fmt_num(value)
    local n = tonumber(value)
    if n == nil then return "0" end
    local s = tostring(math.floor(n + 0.5))
    local sign = ""
    if s:sub(1, 1) == "-" then
        sign = "-"
        s = s:sub(2)
    end
    local grouped = s:reverse():gsub("(%d%d%d)", "%1,"):reverse():gsub("^,", "")
    return sign .. grouped
end

local function fmt_dec1(value)
    local n = tonumber(value)
    if n == nil then return "0.0" end
    return string.format("%.1f", n)
end

local function safe_str(v, default_v)
    if v == nil or v == "" then return default_v or "—" end
    return tostring(v)
end

local function trim(s)
    if s == nil then return "" end
    local out = tostring(s):gsub("^%s+", ""):gsub("%s+$", "")
    return out
end

local function to_positive_int(v)
    local n = tonumber(v)
    if n == nil or n <= 0 then return nil end
    return math.floor(n)
end

-- ============================================================
-- DATE HELPERS — تقویم شمسی کاملاً سمت Lua
-- (کپی از action_management_dashboard_report_bot.lua؛ REPORT_FN_JDATE روی این نشست SQL خطا می‌دهد)
-- ============================================================

local function days_from_civil(y, m, d)
    if m <= 2 then y = y - 1 end
    local era
    if y >= 0 then era = math.floor(y / 400) else era = math.floor((y - 399) / 400) end
    local yoe = y - era * 400
    local mm = m + (m > 2 and -3 or 9)
    local doy = math.floor((153 * mm + 2) / 5) + d - 1
    local doe = yoe * 365 + math.floor(yoe / 4) - math.floor(yoe / 100) + doy
    return era * 146097 + doe - 719468
end

local function civil_from_civil_days(z)
    z = z + 719468
    local era
    if z >= 0 then era = math.floor(z / 146097) else era = math.floor((z - 146096) / 146097) end
    local doe = z - era * 146097
    local yoe = math.floor((doe - math.floor(doe / 1460) + math.floor(doe / 36524) - math.floor(doe / 146096)) / 365)
    local y = yoe + era * 400
    local doy = doe - (365 * yoe + math.floor(yoe / 4) - math.floor(yoe / 100))
    local mp = math.floor((5 * doy + 2) / 153)
    local d = doy - math.floor((153 * mp + 2) / 5) + 1
    local m = mp + ((mp < 10) and 3 or (-9))
    y = y + ((m <= 2) and 1 or 0)
    return y, m, d
end

local function gregorian_to_jalali(gy, gm, gd)
    local g_days_in_month = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 }
    local gy2 = (gm > 2) and (gy + 1) or gy
    local days = 355666 + (365 * gy) + math.floor((gy2 + 3) / 4) - math.floor((gy2 + 99) / 100) +
        math.floor((gy2 + 399) / 400) + gd
    for i = 1, gm - 1 do
        days = days + g_days_in_month[i]
    end
    local jy = -1595 + (33 * math.floor(days / 12053))
    days = days % 12053
    jy = jy + 4 * math.floor(days / 1461)
    days = days % 1461
    if days > 365 then
        jy = jy + math.floor((days - 1) / 365)
        days = (days - 1) % 365
    end
    local jm, jd
    if days < 186 then
        jm = 1 + math.floor(days / 31)
        jd = 1 + (days % 31)
    else
        jm = 7 + math.floor((days - 186) / 30)
        jd = 1 + ((days - 186) % 30)
    end
    return jy, jm, jd
end

local function jalali_to_gregorian(jy, jm, jd)
    jy = jy + 1595
    local days = -355668 + (365 * jy) + (math.floor(jy / 33) * 8) + math.floor(((jy % 33) + 3) / 4) + jd
    if jm < 7 then
        days = days + (jm - 1) * 31
    else
        days = days + ((jm - 7) * 30) + 186
    end
    local gy = 400 * math.floor(days / 146097)
    days = days % 146097
    if days > 36524 then
        days = days - 1
        gy = gy + 100 * math.floor(days / 36524)
        days = days % 36524
        if days >= 365 then days = days + 1 end
    end
    gy = gy + 4 * math.floor(days / 1461)
    days = days % 1461
    if days > 365 then
        gy = gy + math.floor((days - 1) / 365)
        days = (days - 1) % 365
    end
    local gd = days + 1
    local is_leap = (gy % 4 == 0 and gy % 100 ~= 0) or (gy % 400 == 0)
    local g_days_in_month = { 31, is_leap and 29 or 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 }
    local gm = 1
    while gm <= 12 and gd > g_days_in_month[gm] do
        gd = gd - g_days_in_month[gm]
        gm = gm + 1
    end
    return gy, gm, gd
end

local function filetime_to_jalali(raw)
    raw = tonumber(raw)
    if raw == nil or raw <= 0 then return nil end
    local unix_seconds = raw / 10000000 - 11644473600
    local days = math.floor(unix_seconds / 86400)
    local sec_of_day = unix_seconds - days * 86400
    local gy, gm, gd = civil_from_civil_days(days)
    local jy, jm, jd = gregorian_to_jalali(gy, gm, gd)
    return {
        y = jy, m = jm, d = jd,
        hh = math.floor(sec_of_day / 3600),
        mi = math.floor((sec_of_day % 3600) / 60),
    }
end

local function jalali_date_str(raw)
    local j = filetime_to_jalali(raw)
    if j == nil then return nil end
    return string.format("%04d/%02d/%02d", j.y, j.m, j.d)
end

local function jalali_datetime_str(raw)
    local j = filetime_to_jalali(raw)
    if j == nil then return nil end
    return string.format("%04d/%02d/%02d %02d:%02d", j.y, j.m, j.d, j.hh, j.mi)
end

local function jalali_to_filetime(jy, jm, jd)
    if jy == nil or jm == nil or jd == nil then return nil end
    local gy, gm, gd = jalali_to_gregorian(jy, jm, jd)
    return (days_from_civil(gy, gm, gd) * 86400 + 11644473600) * 10000000
end

-- ورودی تاریخ به شکل «1405/06/09» یا «1405-06-09» -> FILETIME ابتدای همان روز
local function parse_jalali_input(text)
    if text == nil then return nil end
    local s = trim(text)
    if s == "" then return nil end
    local jy, jm, jd = s:match("^(%d%d%d%d)[/-](%d%d?)[/-](%d%d?)$")
    if jy == nil then return nil end
    return jalali_to_filetime(tonumber(jy), tonumber(jm), tonumber(jd))
end

local function now_filetime()
    local ok, value = pcall(function()
        local t = time.current()
        return time.get_filetime(string.format(
            [[{"year":%d,"month":%d,"day":%d,"hour":%d,"minute":%d,"second":%d}]],
            time.get_year(t), time.get_month(t), time.get_day(t),
            time.get_hour(t), time.get_minute(t), time.get_second(t)))
    end)
    if ok and tonumber(value) then return tonumber(value) end
    return nil
end

-- ============================================================
-- DATABASE HELPERS
-- ============================================================

local function fetch_rows(query, params)
    pcall(function() db.use_db("0000000") end)
    local ok, err = pcall(function()
        db.query({ query = query, params = params or {} })
    end)
    if not ok then return nil, tostring(err) end
    local rows = {}
    local record = {}
    while db.query_fetch(record) do
        local row = {}
        for i = 1, #record do row[i] = record[i] end
        table.insert(rows, row)
    end
    db.query_free()
    return rows
end

local function placeholders(count)
    local parts = {}
    for _ = 1, count do table.insert(parts, "?") end
    return table.concat(parts, ",")
end

-- اجرای یک کوئری روی فهرست شناسه‌ها به‌صورت تکه‌تکه (پرهیز از IN بسیار بزرگ — قانون CLAUDE.md)
local function fetch_rows_chunked(build_query, ids, extra_params)
    local all = {}
    local chunk = {}
    local function flush()
        if #chunk == 0 then return true end
        local params = {}
        for _, v in ipairs(extra_params or {}) do table.insert(params, v) end
        for _, v in ipairs(chunk) do table.insert(params, v) end
        local rows, err = fetch_rows(build_query(#chunk), params)
        if rows == nil then return false, err end
        for _, r in ipairs(rows) do table.insert(all, r) end
        chunk = {}
        return true
    end
    for _, id in ipairs(ids) do
        table.insert(chunk, id)
        if #chunk >= CONFIG.ID_CHUNK then
            local ok, err = flush()
            if not ok then return nil, err end
        end
    end
    local ok, err = flush()
    if not ok then return nil, err end
    return all
end

-- ============================================================
-- CURRENT USER
-- ============================================================

-- شکل کامل خروجی teamyar.get_user_info() در این کدبیس مستند نیست (فقط .timezone جایی استفاده شده)،
-- پس کلیدهای محتمل کاوش می‌شوند و در نهایت ورودی صریح user_id پذیرفته می‌شود.
-- برای قفل‌کردن کلید واقعی: بات را با action=whoami اجرا کنید و خروجی خام را ببینید.
local function raw_user_info()
    local ok, info = pcall(function() return teamyar.get_user_info() end)
    if ok and type(info) == "table" then return info end
    return nil
end

local function resolve_current_user(input)
    local explicit = to_positive_int(input["user_id"])
    if explicit then
        return explicit, "input:user_id"
    end
    local info = raw_user_info()
    if info ~= nil then
        for _, key in ipairs(CONFIG.USER_ID_KEYS) do
            local candidate = to_positive_int(info[key])
            if candidate then
                return candidate, "get_user_info:" .. key
            end
        end
    end
    return nil, "unresolved"
end

local function fetch_user_name(user_id)
    if user_id == nil then return nil end
    local rows = fetch_rows("SELECT fullname FROM profile_main WHERE id = ? LIMIT 1", { user_id })
    if rows and rows[1] then return rows[1][1] end
    return nil
end

-- ============================================================
-- FILTERS
-- ============================================================

local function parse_filters(input)
    local scope = trim(input["scope"])
    if scope ~= SCOPE_RESPONSIBLE and scope ~= SCOPE_AUTHOR and scope ~= SCOPE_MEMBER then
        scope = SCOPE_ALL
    end

    local state = trim(input["state"])
    if state ~= STATE_OPEN and state ~= STATE_RUNNING and state ~= STATE_OVERDUE and state ~= STATE_DONE then
        state = STATE_ALL
    end

    local page_size = to_positive_int(input["page_size"]) or CONFIG.PAGE_SIZE
    if page_size > CONFIG.MAX_PAGE_SIZE then page_size = CONFIG.MAX_PAGE_SIZE end

    return {
        scope = scope,
        state = state,
        search = trim(input["search"]),
        category_id = to_positive_int(input["category_id"]),
        topic_id = to_positive_int(input["topic_id"]),
        workflow_id = to_positive_int(input["workflow_id"]),
        section_id = to_positive_int(input["section_id"]),
        from_raw = parse_jalali_input(input["from_date"]),
        to_raw = parse_jalali_input(input["to_date"]),
        page = to_positive_int(input["page"]) or 1,
        page_size = page_size,
    }
end

-- شرط «دامنهٔ کاربر» — همیشه پارامتری، هرگز concatenation ورودی کاربر
local function build_scope_clause(scope, user_id, params)
    local responsible_sql =
        "EXISTS (SELECT 1 FROM todo_task_steps sc WHERE sc.TASK_ID = tt.ID AND sc.STATUS = " ..
        STEP_STATUS_CURRENT .. " AND sc.RESPONSIBLE_ID = ?)"
    local author_sql = "(tt.AUTHOR_ID = ? OR tt.OWNER_ID = ?)"
    local member_sql =
        "(EXISTS (SELECT 1 FROM todo_notify nf WHERE nf.TASK_ID = tt.ID AND nf.USER_ID = ?)" ..
        " OR EXISTS (SELECT 1 FROM todo_task_comments cm WHERE cm.TASK_ID = tt.ID AND cm.AUTHOR_ID = ?))"

    if scope == SCOPE_RESPONSIBLE then
        table.insert(params, user_id)
        return responsible_sql
    elseif scope == SCOPE_AUTHOR then
        table.insert(params, user_id)
        table.insert(params, user_id)
        return author_sql
    elseif scope == SCOPE_MEMBER then
        table.insert(params, user_id)
        table.insert(params, user_id)
        return member_sql
    end

    -- SCOPE_ALL = اجتماع هر سه
    table.insert(params, user_id)
    table.insert(params, user_id)
    table.insert(params, user_id)
    table.insert(params, user_id)
    table.insert(params, user_id)
    return "(" .. responsible_sql .. " OR " .. author_sql .. " OR " .. member_sql .. ")"
end

local function build_where(filters, user_id, now_raw)
    local clauses = { "tt.ARCHIVE_FLAG = 0" }
    local params = {}

    table.insert(clauses, build_scope_clause(filters.scope, user_id, params))

    if filters.state == STATE_OPEN then
        table.insert(clauses, "tt.STATUS = " .. TASK_STATUS_OPEN)
    elseif filters.state == STATE_DONE then
        table.insert(clauses, "(tt.STATUS = " .. TASK_STATUS_CLOSE .. " OR tt.T_REAL_END_DATE > 0)")
    elseif filters.state == STATE_RUNNING then
        -- باز، و دست‌کم یک مرحله‌اش تمام شده (یعنی واقعاً در جریان است، نه تازه‌ثبت‌شده)
        table.insert(clauses, "tt.STATUS = " .. TASK_STATUS_OPEN)
        table.insert(clauses,
            "EXISTS (SELECT 1 FROM todo_task_steps sr WHERE sr.TASK_ID = tt.ID AND sr.STATUS IN (1, 2))")
    elseif filters.state == STATE_OVERDUE then
        table.insert(clauses, "tt.STATUS = " .. TASK_STATUS_OPEN)
        table.insert(clauses, "tt.T_END_DATE > 0")
        table.insert(clauses, "tt.T_END_DATE < ?")
        table.insert(params, now_raw)
    end

    if filters.category_id then
        table.insert(clauses, "tt.CATEGORY_ID = ?")
        table.insert(params, filters.category_id)
    end
    if filters.topic_id then
        table.insert(clauses, "tt.TOPIC_ID = ?")
        table.insert(params, filters.topic_id)
    end
    if filters.workflow_id then
        table.insert(clauses, "tt.WORK_FLOW_ID = ?")
        table.insert(params, filters.workflow_id)
    end
    if filters.section_id then
        table.insert(clauses, "tc.SECTION_ID = ?")
        table.insert(params, filters.section_id)
    end
    if filters.from_raw then
        table.insert(clauses, "tt.T_START_DATE >= ?")
        table.insert(params, filters.from_raw)
    end
    if filters.to_raw then
        table.insert(clauses, "tt.T_START_DATE < ?")
        table.insert(params, filters.to_raw + CONFIG.DAY_TICKS)
    end
    if filters.search ~= "" then
        -- توجه: `LIKE ?` پارامتری روی این پلتفرم بی‌صدا شکست می‌خورد (باگ تاییدشدهٔ زنده).
        -- LOCATE هم پارامتری است (ضدInjection) و هم از آن باگ عبور می‌کند.
        table.insert(clauses, "(LOCATE(?, tt.TASK_TITLE) > 0 OR CAST(tt.ID AS CHAR) = ?)")
        table.insert(params, filters.search)
        table.insert(params, filters.search)
    end

    return table.concat(clauses, " AND "), params
end

local ACTION_BASE_FROM = [[
FROM todo_task tt
LEFT JOIN todo_category tc ON tc.ID = tt.CATEGORY_ID
LEFT JOIN todo_topic tp ON tp.ID = tt.TOPIC_ID
LEFT JOIN todo_workflow tw ON tw.ID = tt.WORK_FLOW_ID
LEFT JOIN todo_section ts ON ts.ID = tc.SECTION_ID
LEFT JOIN profile_main pa ON pa.id = tt.AUTHOR_ID
]]

local function count_actions(where_sql, params)
    local rows, err = fetch_rows("SELECT COUNT(*) " .. ACTION_BASE_FROM .. " WHERE " .. where_sql, params)
    if rows == nil then return nil, err end
    return tonumber(rows[1] and rows[1][1]) or 0
end

local function fetch_actions(where_sql, params, filters)
    local offset = (filters.page - 1) * filters.page_size
    local query = [[
SELECT
    tt.ID,
    tt.TASK_TITLE,
    tt.STATUS,
    tt.T_START_DATE,
    tt.T_END_DATE,
    tt.T_REAL_END_DATE,
    tt.AUTHOR_ID,
    COALESCE(pa.fullname, '') AS author_name,
    COALESCE(ts.SECTION_NAME, '') AS section_name,
    COALESCE(tc.CATEGORY_TITLE, '') AS category_title,
    COALESCE(tp.NAME, '') AS topic_name,
    COALESCE(tw.WF_TITLE, '') AS wf_title
]] .. ACTION_BASE_FROM .. [[
WHERE ]] .. where_sql .. [[
ORDER BY tt.T_START_DATE DESC, tt.ID DESC
LIMIT ]] .. math.floor(filters.page_size) .. [[ OFFSET ]] .. math.floor(offset)

    local rows, err = fetch_rows(query, params)
    if rows == nil then return nil, err end

    local actions = {}
    local ids = {}
    for _, r in ipairs(rows) do
        local id = tonumber(r[1])
        table.insert(actions, {
            id = id,
            title = safe_str(r[2], "بدون عنوان"),
            status = tonumber(r[3]) or 0,
            start_raw = tonumber(r[4]) or 0,
            end_raw = tonumber(r[5]) or 0,
            real_end_raw = tonumber(r[6]) or 0,
            author_id = tonumber(r[7]),
            author_name = r[8],
            section_name = r[9],
            category_title = r[10],
            topic_name = r[11],
            wf_title = r[12],
        })
        table.insert(ids, id)
    end
    return actions, nil, ids
end

-- مرحلهٔ جاری هر اقدام — یک کوئری دسته‌ای برای کل صفحه (نه N+1)
local function attach_current_steps(actions, ids)
    if #ids == 0 then return true end
    local function build(n)
        return [[
SELECT
    tsx.TASK_ID,
    COALESCE(st.STEP_NAME, '') AS step_name,
    COALESCE(pr.fullname, '') AS responsible_name,
    tsx.RESPONSIBLE_ID,
    tsx.DATE_CREATE
FROM todo_task_steps tsx
LEFT JOIN todo_step st ON st.ID = tsx.STEP_ID
LEFT JOIN profile_main pr ON pr.id = tsx.RESPONSIBLE_ID
WHERE tsx.STATUS = ]] .. STEP_STATUS_CURRENT .. [[ AND tsx.TASK_ID IN (]] .. placeholders(n) .. [[)
ORDER BY tsx.TASK_ID, tsx.DATE_CREATE DESC, tsx.ID DESC
]]
    end

    local rows, err = fetch_rows_chunked(build, ids, {})
    if rows == nil then return false, err end

    local by_task = {}
    for _, r in ipairs(rows) do
        local tid = tonumber(r[1])
        if by_task[tid] == nil then
            by_task[tid] = { step_name = r[2], responsible_name = r[3], responsible_id = tonumber(r[4]) }
        end
    end
    for _, a in ipairs(actions) do
        local cur = by_task[a.id]
        a.step_name = cur and cur.step_name or ""
        a.responsible_name = cur and cur.responsible_name or ""
        a.responsible_id = cur and cur.responsible_id or nil
    end
    return true
end

-- شمارندهٔ هر تب/وضعیت برای داشبورد — یک کوئری تجمیعی به‌جای چند کوئری جدا
local function fetch_scope_counts(user_id, now_raw)
    local params = {}
    local scope_sql = build_scope_clause(SCOPE_ALL, user_id, params)
    local resp_params = {}
    local resp_sql = build_scope_clause(SCOPE_RESPONSIBLE, user_id, resp_params)
    local auth_params = {}
    local auth_sql = build_scope_clause(SCOPE_AUTHOR, user_id, auth_params)
    local mem_params = {}
    local mem_sql = build_scope_clause(SCOPE_MEMBER, user_id, mem_params)

    -- ترتیب پارامترها باید دقیقاً با ترتیب ظاهرشدن عبارت‌ها در SELECT یکی باشد
    local all_params = {}
    for _, v in ipairs(resp_params) do table.insert(all_params, v) end
    for _, v in ipairs(auth_params) do table.insert(all_params, v) end
    for _, v in ipairs(mem_params) do table.insert(all_params, v) end
    for _, v in ipairs(params) do table.insert(all_params, v) end
    table.insert(all_params, now_raw)
    for _, v in ipairs(params) do table.insert(all_params, v) end
    for _, v in ipairs(params) do table.insert(all_params, v) end

    local query = [[
SELECT
    SUM(CASE WHEN ]] .. resp_sql .. [[ AND tt.STATUS = ]] .. TASK_STATUS_OPEN .. [[ THEN 1 ELSE 0 END),
    SUM(CASE WHEN ]] .. auth_sql .. [[ THEN 1 ELSE 0 END),
    SUM(CASE WHEN ]] .. mem_sql .. [[ THEN 1 ELSE 0 END),
    SUM(CASE WHEN ]] .. scope_sql .. [[ AND tt.STATUS = ]] .. TASK_STATUS_OPEN .. [[
             AND tt.T_END_DATE > 0 AND tt.T_END_DATE < ? THEN 1 ELSE 0 END),
    SUM(CASE WHEN ]] .. scope_sql .. [[ AND tt.STATUS = ]] .. TASK_STATUS_OPEN .. [[ THEN 1 ELSE 0 END),
    SUM(CASE WHEN ]] .. scope_sql .. [[ AND (tt.STATUS = ]] .. TASK_STATUS_CLOSE .. [[
             OR tt.T_REAL_END_DATE > 0) THEN 1 ELSE 0 END)
FROM todo_task tt
WHERE tt.ARCHIVE_FLAG = 0
]]

    local rows, err = fetch_rows(query, all_params)
    if rows == nil then return nil, err end
    local r = rows[1] or {}
    return {
        responsible_open = tonumber(r[1]) or 0,
        authored = tonumber(r[2]) or 0,
        member = tonumber(r[3]) or 0,
        overdue = tonumber(r[4]) or 0,
        open_total = tonumber(r[5]) or 0,
        done_total = tonumber(r[6]) or 0,
    }
end

-- گزینه‌های فیلتر (بخش/رده/موضوع/جریان کار) — فقط مواردی که واقعاً در دامنهٔ کاربر دیده می‌شوند
local function fetch_filter_options()
    local options = { sections = {}, categories = {}, topics = {}, workflows = {} }

    local rows = fetch_rows("SELECT ID, SECTION_NAME FROM todo_section ORDER BY S_ORDER, SECTION_NAME", {})
    for _, r in ipairs(rows or {}) do
        table.insert(options.sections, { id = tonumber(r[1]), name = safe_str(r[2], "—") })
    end

    rows = fetch_rows(
        "SELECT ID, CATEGORY_TITLE, COALESCE(SECTION_ID, 0) FROM todo_category ORDER BY C_ORDER, CATEGORY_TITLE", {})
    for _, r in ipairs(rows or {}) do
        table.insert(options.categories,
            { id = tonumber(r[1]), name = safe_str(r[2], "—"), section_id = tonumber(r[3]) or 0 })
    end

    rows = fetch_rows(
        "SELECT ID, NAME, COALESCE(CATEGORY_ID, 0) FROM todo_topic WHERE COALESCE(topic_disabled, 0) = 0 ORDER BY NAME",
        {})
    for _, r in ipairs(rows or {}) do
        table.insert(options.topics,
            { id = tonumber(r[1]), name = safe_str(r[2], "—"), category_id = tonumber(r[3]) or 0 })
    end

    rows = fetch_rows(
        "SELECT ID, WF_TITLE, COALESCE(CATEGORY_ID, 0) FROM todo_workflow WHERE COALESCE(wf_disabled, 0) = 0 ORDER BY WF_TITLE",
        {})
    for _, r in ipairs(rows or {}) do
        table.insert(options.workflows,
            { id = tonumber(r[1]), name = safe_str(r[2], "—"), category_id = tonumber(r[3]) or 0 })
    end

    return options
end

-- ============================================================
-- WRITE LAYER — ایجاد اقدام واقعی از داخل بات
-- ============================================================
-- مرجع: docs/context/TeamyarInternalApiReference.md، ماژول «اقدام» = module_id 8
--   /api/todo/taskadd  فیلدهای الزامی: wf_id, topic_id  (هر دو باید از یک «رده» باشند وگرنه error)
--   موفق: {"success":true,"data":{"task_id":N}}   ناموفق: {"success":false,"error":{"message":"..."}}

local TODO_MODULE_ID = 8

local function normalize_api_response(res)
    if res == nil then return nil end
    if type(res) == "string" then
        local ok, decoded = pcall(function() return json.decode(res) end)
        if ok and type(decoded) == "table" then return decoded end
        return nil
    end
    if type(res) == "table" then return res end
    return nil
end

local function api_error_message(payload, fallback)
    if type(payload) == "table" and type(payload.error) == "table" then
        local msg = payload.error.message
        if msg ~= nil and trim(msg) ~= "" then return tostring(msg) end
    end
    return fallback
end

-- اعتبارسنجی هم‌رده‌بودن جریان کار و موضوع — قبل از تماس با API، تا خطای مبهم سرور نگیریم
local function validate_wf_topic_pair(wf_id, topic_id)
    local rows, err = fetch_rows([[
SELECT
    (SELECT COALESCE(CATEGORY_ID, 0) FROM todo_workflow WHERE ID = ?),
    (SELECT COALESCE(CATEGORY_ID, 0) FROM todo_topic WHERE ID = ?)
]], { wf_id, topic_id })
    if rows == nil then return false, "خطا در بررسی جریان کار و موضوع: " .. tostring(err) end
    local r = rows[1] or {}
    local wf_cat = tonumber(r[1])
    local topic_cat = tonumber(r[2])
    if wf_cat == nil then return false, "جریان کار انتخاب‌شده یافت نشد." end
    if topic_cat == nil then return false, "موضوع انتخاب‌شده یافت نشد." end
    if wf_cat ~= topic_cat then
        return false, "جریان کار و موضوع باید از یک رده باشند (قانون سرور Teamyar)."
    end
    return true
end

local function create_action(input)
    local wf_id = to_positive_int(input["wf_id"])
    local topic_id = to_positive_int(input["topic_id"])
    local title = trim(input["task_title"])
    local comment = trim(input["comment"])

    if wf_id == nil then return { ok = false, error = "انتخاب «جریان کار» الزامی است." } end
    if topic_id == nil then return { ok = false, error = "انتخاب «موضوع» الزامی است." } end
    if title == "" then return { ok = false, error = "وارد کردن «عنوان اقدام» الزامی است." } end
    if #comment > 50000 then
        return { ok = false, error = "متن توضیحات از حد مجاز (۵۰٬۰۰۰ کاراکتر) بیشتر است." }
    end

    local pair_ok, pair_err = validate_wf_topic_pair(wf_id, topic_id)
    if not pair_ok then return { ok = false, error = pair_err } end

    local req = {
        wf_id = wf_id,
        topic_id = topic_id,
        task_title = title,
        comment = comment,
        task_type = 1,
    }

    local deadline_raw = parse_jalali_input(input["deadline"])
    if deadline_raw then
        -- مهلت را به انتهای همان روز می‌بریم تا خودِ روز مهلت هم شمرده شود
        req.deadline = string.format("%.0f", deadline_raw + CONFIG.DAY_TICKS - 1)
    end

    local crm_id = to_positive_int(input["crm_id"])
    if crm_id then req.crm_id = crm_id end

    local ok, raw = pcall(function()
        return teamyar.call_api(TODO_MODULE_ID, "/api/todo/taskadd", req)
    end)
    if not ok then
        return { ok = false, error = "فراخوانی سرویس ایجاد اقدام ناموفق بود: " .. tostring(raw) }
    end

    local payload = normalize_api_response(raw)
    if payload == nil then
        return { ok = false, error = "پاسخ نامعتبر از سرویس ایجاد اقدام دریافت شد." }
    end

    local success = payload.success
    local task_id = nil
    if type(payload.data) == "table" then task_id = to_positive_int(payload.data.task_id) end

    if success == true or success == 1 then
        if task_id == nil then
            return { ok = false, error = "اقدام ثبت شد ولی شناسه‌ای برنگشت — لطفاً در ماژول اقدام بررسی کنید." }
        end
        return { ok = true, task_id = task_id }
    end

    return { ok = false, error = api_error_message(payload, "ثبت اقدام توسط سرور رد شد.") }
end

-- ============================================================
-- ROW SERIALIZATION (برای جدول سمت کلاینت)
-- ============================================================

local function action_state_label(a, now_raw)
    if a.status == TASK_STATUS_CLOSE or a.real_end_raw > 0 then
        return "کامل‌شده", "done"
    end
    if a.end_raw > 0 and now_raw and a.end_raw < now_raw then
        return "معوق", "overdue"
    end
    return "باز", "open"
end

local function remaining_label(a, now_raw)
    if a.status == TASK_STATUS_CLOSE or a.real_end_raw > 0 then return "—", 0 end
    if a.end_raw == nil or a.end_raw <= 0 or now_raw == nil then return "بدون مهلت", 0 end
    local diff_days = (a.end_raw - now_raw) / CONFIG.DAY_TICKS
    if diff_days < 0 then
        return fmt_dec1(-diff_days) .. " روز تاخیر", diff_days
    end
    return fmt_dec1(diff_days) .. " روز مانده", diff_days
end

local function serialize_actions(actions, now_raw, user_id)
    local out = {}
    for _, a in ipairs(actions) do
        local state_label, state_key = action_state_label(a, now_raw)
        local remain_text, remain_days = remaining_label(a, now_raw)
        table.insert(out, {
            id = a.id,
            title = a.title,
            section = safe_str(a.section_name, "—"),
            category = safe_str(a.category_title, "—"),
            topic = safe_str(a.topic_name, "—"),
            workflow = safe_str(a.wf_title, "—"),
            step = safe_str(a.step_name, "—"),
            responsible = safe_str(a.responsible_name, "—"),
            author = safe_str(a.author_name, "—"),
            start_date = jalali_date_str(a.start_raw) or "—",
            due_date = jalali_date_str(a.end_raw) or "—",
            state = state_label,
            state_key = state_key,
            remaining = remain_text,
            remaining_days = remain_days,
            is_mine = (a.responsible_id ~= nil and a.responsible_id == user_id) and 1 or 0,
        })
    end
    return out
end

-- ============================================================
-- ACTIONS (AJAX)
-- ============================================================

local function respond(payload)
    teamyar.write_result(json.encode(payload))
end

local function handle_list(input, user_id, now_raw)
    local filters = parse_filters(input)
    local where_sql, params = build_where(filters, user_id, now_raw)

    local count_params = {}
    for _, v in ipairs(params) do table.insert(count_params, v) end
    local total, count_err = count_actions(where_sql, count_params)
    if total == nil then
        return { ok = false, error = "خطا در شمارش اقدام‌ها: " .. tostring(count_err) }
    end

    local actions, err, ids = fetch_actions(where_sql, params, filters)
    if actions == nil then
        return { ok = false, error = "خطا در دریافت اقدام‌ها: " .. tostring(err) }
    end

    local steps_ok, steps_err = attach_current_steps(actions, ids)
    if not steps_ok then
        return { ok = false, error = "خطا در دریافت مراحل جاری: " .. tostring(steps_err) }
    end

    return {
        ok = true,
        total = total,
        page = filters.page,
        page_size = filters.page_size,
        page_count = math.max(1, math.ceil(total / filters.page_size)),
        rows = serialize_actions(actions, now_raw, user_id),
    }
end

local function handle_summary(user_id, now_raw)
    local counts, err = fetch_scope_counts(user_id, now_raw)
    if counts == nil then
        return { ok = false, error = "خطا در محاسبهٔ خلاصه: " .. tostring(err) }
    end
    return { ok = true, counts = counts }
end

-- مقادیر واقعیِ ستون‌های مرحله‌ای که در ماژول بومی نوار «پیش‌نویس/بررسی/اجرا/کامل» را می‌سازند.
-- خروجی این تشخیص، مبنای نگاشت دقیق ۱:۱ در پاس بعدی است (عمداً حدس نزده‌ایم).
local function handle_state_probe()
    local task_states = fetch_rows([[
SELECT COALESCE(tt.state, -1) AS st, COUNT(*) AS c
FROM todo_task tt
WHERE tt.ARCHIVE_FLAG = 0
GROUP BY COALESCE(tt.state, -1)
ORDER BY c DESC
LIMIT 30
]], {})

    local task_progress = fetch_rows([[
SELECT COALESCE(tt.progress, -1) AS pr, COUNT(*) AS c
FROM todo_task tt
WHERE tt.ARCHIVE_FLAG = 0
GROUP BY COALESCE(tt.progress, -1)
ORDER BY c DESC
LIMIT 30
]], {})

    local step_states = fetch_rows([[
SELECT COALESCE(st.state, -1) AS st, COALESCE(st.STEP_NAME, '') AS nm, COUNT(*) AS c
FROM todo_step st
GROUP BY COALESCE(st.state, -1), COALESCE(st.STEP_NAME, '')
ORDER BY c DESC
LIMIT 40
]], {})

    return {
        ok = true,
        task_state = task_states or {},
        task_progress = task_progress or {},
        step_state = step_states or {},
        note = "برای نگاشت دقیق نوار مرحله‌ای ماژول بومی (پیش‌نویس/بررسی/اجرا/کامل) استفاده می‌شود.",
    }
end

local function handle_whoami(input)
    local user_id, source = resolve_current_user(input)
    local info = raw_user_info()
    local keys = {}
    if type(info) == "table" then
        for k, v in pairs(info) do
            table.insert(keys, { key = tostring(k), value = tostring(v), type = type(v) })
        end
    end
    return {
        ok = true,
        resolved_user_id = user_id,
        resolved_from = source,
        fullname = fetch_user_name(user_id),
        user_info_available = (info ~= nil),
        user_info_fields = keys,
    }
end

-- ============================================================
-- HTML / CSS
-- ============================================================

local function build_css()
    return [==[
<style>
@font-face {
  font-family: "PeydaReport";
  font-style: normal;
  font-weight: 400;
  src: url(data:font/truetype;charset=utf-8;base64,{{PEYDA_REGULAR_B64}}) format("truetype");
}
@font-face {
  font-family: "PeydaReport";
  font-style: normal;
  font-weight: 700;
  src: url(data:font/truetype;charset=utf-8;base64,{{PEYDA_BOLD_B64}}) format("truetype");
}
#amRoot, #amRoot * {
  font-family: "PeydaReport", "Peyda", "IRANSans", "Tahoma", "Arial", sans-serif !important;
  box-sizing: border-box;
}
#amRoot { direction: rtl; background: #ffffff; color: #000000; font-size: 14px; padding: 0 0 24px 0; }

/* ---- تمام صفحه: toggle کلاس CSS، نه Fullscreen API (باگ تاییدشدهٔ iframe/موبایل) ---- */
#amRoot.am-fullscreen {
  position: fixed; inset: 0; z-index: 9999;
  overflow-y: auto; overflow-x: hidden;
  background: #ffffff; padding: 0 0 24px 0;
}

/* ---- Header ---- */
#amRoot .am-header {
  background: #16509D; color: #ffffff;
  display: flex; align-items: center; justify-content: space-between;
  gap: 12px; padding: 12px 16px; flex-wrap: wrap;
}
#amRoot .am-brand { display: flex; align-items: center; gap: 12px; }
#amRoot .am-brand img { height: 34px; width: auto; display: block; }
#amRoot .am-title { font-size: 15px; font-weight: bold; color: #ffffff; }
#amRoot .am-user { font-size: 14px; color: #ffffff; opacity: 0.92; }

/* ---- Toolbar: در صفحهٔ RTL، flex-end یعنی سمت چپ (قانون CLAUDE.md) ---- */
#amRoot .am-toolbar { display: flex; justify-content: flex-end; gap: 8px; flex-wrap: wrap; }
#amRoot .am-tool-btn {
  background: #ffffff; color: #16509D; border: 1px solid #ffffff;
  border-radius: 4px; padding: 6px 12px; font-size: 14px; font-weight: bold; cursor: pointer;
}
#amRoot .am-tool-btn:hover { background: #f5f5f5; }

/* ---- Layout ---- */
#amRoot .am-body { display: flex; align-items: stretch; gap: 0; min-height: 420px; }
#amRoot .am-side {
  width: 208px; flex: 0 0 208px; background: #f5f5f5; border-left: 1px solid #dddddd;
  padding: 12px 0; transition: width 0.15s ease, flex-basis 0.15s ease;
}
#amRoot .am-side.collapsed { width: 52px; flex-basis: 52px; }
#amRoot .am-side.collapsed .am-nav-text, #amRoot .am-side.collapsed .am-nav-badge { display: none; }
#amRoot .am-nav-item {
  display: flex; align-items: center; gap: 8px;
  padding: 10px 14px; font-size: 14px; color: #000000; cursor: pointer;
  border-right: 3px solid transparent; white-space: nowrap;
}
#amRoot .am-nav-item:hover { background: #e9eef5; }
#amRoot .am-nav-item.active {
  background: #ffffff; border-right-color: #16509D; color: #16509D; font-weight: bold;
}
#amRoot .am-nav-item.am-nav-create { color: #16509D; font-weight: bold; }
#amRoot .am-nav-icon { width: 18px; text-align: center; font-size: 14px; }
#amRoot .am-nav-badge {
  margin-right: auto; background: #16509D; color: #ffffff;
  border-radius: 10px; padding: 1px 8px; font-size: 14px; min-width: 26px; text-align: center;
}
#amRoot .am-nav-sep { height: 1px; background: #dddddd; margin: 8px 12px; }
#amRoot .am-main { flex: 1 1 auto; padding: 14px 16px; min-width: 0; }

/* ---- Filters ---- */
#amRoot .am-filters {
  background: #f5f5f5; border: 1px solid #dddddd; border-radius: 4px;
  padding: 10px 12px; margin-bottom: 12px;
  display: flex; flex-wrap: wrap; gap: 10px; align-items: flex-end;
}
#amRoot .am-field { display: flex; flex-direction: column; gap: 4px; }
#amRoot .am-field label { font-size: 14px; font-weight: bold; color: #16509D; }
#amRoot .am-field input, #amRoot .am-field select {
  font-size: 14px; padding: 5px 8px; border: 1px solid #cccccc; border-radius: 4px;
  background: #ffffff; color: #000000; min-width: 132px; height: 32px;
}
#amRoot .am-btn {
  background: #16509D; color: #ffffff; border: 1px solid #16509D; border-radius: 4px;
  padding: 6px 16px; font-size: 14px; font-weight: bold; cursor: pointer; height: 32px;
}
#amRoot .am-btn:hover { background: #123f7a; }
#amRoot .am-btn-ghost { background: #ffffff; color: #16509D; }
#amRoot .am-btn-ghost:hover { background: #e9eef5; }

/* ---- Chevron state bar (الگوی نوار مرحله‌ای ماژول بومی) ---- */
#amRoot .am-states { display: flex; gap: 4px; margin-bottom: 12px; flex-wrap: wrap; }
#amRoot .am-state {
  background: #f5f5f5; color: #000000; border: 1px solid #dddddd;
  padding: 6px 18px; font-size: 14px; cursor: pointer; border-radius: 4px;
}
#amRoot .am-state.active { background: #16509D; color: #ffffff; border-color: #16509D; font-weight: bold; }
#amRoot .am-state .am-state-count { font-size: 14px; opacity: 0.85; }

/* ---- Tabs (همه/مسئولم/مؤلفم/عضوم — مطابق ماژول بومی) ---- */
#amRoot .am-tabs { display: flex; gap: 0; border-bottom: 2px solid #dddddd; margin-bottom: 12px; flex-wrap: wrap; }
#amRoot .am-tab {
  padding: 8px 18px; font-size: 14px; cursor: pointer; color: #000000;
  border-bottom: 3px solid transparent; margin-bottom: -2px;
}
#amRoot .am-tab.active { color: #16509D; font-weight: bold; border-bottom-color: #16509D; }

/* ---- KPI cards ---- */
#amRoot .am-kpis { display: flex; flex-wrap: wrap; gap: 10px; margin-bottom: 16px; }
#amRoot .am-kpi {
  flex: 1 1 170px; background: #ffffff; border: 1px solid #dddddd; border-radius: 4px;
  padding: 12px 14px; cursor: pointer; min-width: 150px;
}
#amRoot .am-kpi:hover { border-color: #16509D; }
#amRoot .am-kpi-label { font-size: 14px; color: #666666; margin-bottom: 6px; }
#amRoot .am-kpi-value { font-size: 22px; font-weight: bold; color: #16509D; }
#amRoot .am-kpi.am-kpi-alert { background: #16509D; border-color: #16509D; }
#amRoot .am-kpi.am-kpi-alert .am-kpi-label { color: #ffffff; opacity: 0.9; }
#amRoot .am-kpi.am-kpi-alert .am-kpi-value { color: #ffffff; }

/* ---- Section titles ---- */
#amRoot .am-section-title {
  font-size: 15px; font-weight: bold; color: #16509D;
  margin: 16px 0 8px 0; padding-bottom: 6px; border-bottom: 1px solid #dddddd;
}

/* ---- Table ---- */
#amRoot .am-table-wrap { overflow-x: auto; border: 1px solid #dddddd; border-radius: 4px; }
#amRoot table.data-table { width: auto; min-width: 100%; border-collapse: collapse; background: #ffffff; }
#amRoot table.data-table thead th {
  background: #16509D; color: #ffffff; font-size: 15px; font-weight: bold;
  padding: 9px 12px; text-align: right; white-space: nowrap; width: 1%;
  cursor: pointer; user-select: none; position: relative;
}
#amRoot table.data-table thead th:hover { background: #123f7a; }
#amRoot table.data-table thead th.sort-asc::after { content: " ▲"; font-size: 14px; }
#amRoot table.data-table thead th.sort-desc::after { content: " ▼"; font-size: 14px; }
#amRoot table.data-table tbody td {
  font-size: 14px; color: #000000; padding: 8px 12px; text-align: right;
  white-space: nowrap; width: 1%; border-top: 1px solid #eeeeee;
}
#amRoot table.data-table tbody tr:nth-child(even) { background: #f5f5f5; }
#amRoot table.data-table tbody tr:hover { background: #e9eef5; }
#amRoot table.data-table tbody tr.am-row-overdue td { font-weight: bold; }
#amRoot .am-id-link { color: #16509D; font-weight: bold; text-decoration: none; cursor: pointer; }
#amRoot .am-id-link:hover { text-decoration: underline; }
#amRoot .am-badge {
  display: inline-block; padding: 2px 10px; border-radius: 10px; font-size: 14px;
  border: 1px solid #dddddd; background: #f5f5f5; color: #000000;
}
#amRoot .am-badge-overdue { background: #16509D; color: #ffffff; border-color: #16509D; font-weight: bold; }
#amRoot .am-badge-done { background: #ffffff; color: #666666; }
#amRoot .am-badge-mine { background: #16509D; color: #ffffff; border-color: #16509D; }

/* ---- Pagination ---- */
#amRoot .am-pager { display: flex; align-items: center; gap: 8px; margin-top: 10px; flex-wrap: wrap; }
#amRoot .am-pager-info { font-size: 14px; color: #666666; }

/* ---- Create form ---- */
#amRoot .am-form { max-width: 720px; }
#amRoot .am-form .am-field { margin-bottom: 12px; width: 100%; }
#amRoot .am-form .am-field input, #amRoot .am-form .am-field select, #amRoot .am-form .am-field textarea {
  width: 100%; font-size: 14px; padding: 7px 10px; border: 1px solid #cccccc;
  border-radius: 4px; background: #ffffff; color: #000000;
}
#amRoot .am-form textarea { min-height: 110px; resize: vertical; height: auto; }
#amRoot .am-form-hint { font-size: 14px; color: #666666; margin-top: 4px; }
#amRoot .am-alert {
  padding: 10px 14px; border-radius: 4px; font-size: 14px; margin-bottom: 12px; display: none;
}
#amRoot .am-alert.show { display: block; }
#amRoot .am-alert-ok { background: #16509D; color: #ffffff; font-weight: bold; }
#amRoot .am-alert-err { background: #f5f5f5; color: #000000; border: 1px solid #16509D; font-weight: bold; }

/* ---- Empty / loading ---- */
#amRoot .am-empty { padding: 28px 12px; text-align: center; color: #666666; font-size: 14px; }

/* ---- Help modal ---- */
#amRoot .am-modal {
  display: none; position: fixed; inset: 0; background: rgba(0, 0, 0, 0.45); z-index: 10000;
}
#amRoot .am-modal.active { display: block; }
#amRoot .am-modal-box {
  background: #ffffff; max-width: 760px; margin: 40px auto; border-radius: 6px;
  max-height: 84vh; display: flex; flex-direction: column;
}
#amRoot .am-modal-head {
  background: #16509D; color: #ffffff; padding: 10px 16px; border-radius: 6px 6px 0 0;
  display: flex; justify-content: space-between; align-items: center;
}
#amRoot .am-modal-head h3 { margin: 0; font-size: 15px; font-weight: bold; color: #ffffff; }
#amRoot .am-modal-close { background: transparent; border: none; color: #ffffff; font-size: 16px; cursor: pointer; }
#amRoot .am-modal-body { padding: 14px 18px; overflow-y: auto; font-size: 14px; line-height: 2; }
#amRoot .am-modal-body h4 { font-size: 15px; font-weight: bold; color: #16509D; margin: 12px 0 4px 0; }
#amRoot .am-modal-body ul { margin: 0; padding-right: 20px; }

#amRoot .am-footer { font-size: 14px; color: #666666; padding: 12px 16px 0 16px; }
</style>
]==]
end

local function build_body_html()
    return [==[
<div id="amRoot">

  <div class="am-header">
    <div class="am-brand">
      <img src="data:image/png;base64,{{LOGO140_WHITE_B64}}" alt="140" />
      <span class="am-title">ماژول اقدام</span>
      <span class="am-user">{{USER_NAME}}</span>
    </div>
    <div class="am-toolbar">
      <button type="button" class="am-tool-btn" onclick="amToggleFullScreen()">تمام صفحه</button>
      <button type="button" class="am-tool-btn" onclick="amExportToExcel()">خروجی Excel</button>
      <button type="button" class="am-tool-btn" onclick="amOpenHelp()">راهنما</button>
    </div>
  </div>

  <div class="am-body">

    <div class="am-side" id="amSide">
      <div class="am-nav-item" onclick="amToggleSide()">
        <span class="am-nav-icon">☰</span><span class="am-nav-text">جمع کردن منو</span>
      </div>
      <div class="am-nav-sep"></div>
      <div class="am-nav-item am-nav-create" onclick="amSwitchView('create')" id="amNavCreate">
        <span class="am-nav-icon">＋</span><span class="am-nav-text">ایجاد اقدام</span>
      </div>
      <div class="am-nav-sep"></div>
      <div class="am-nav-item active" onclick="amSwitchView('dashboard')" id="amNavDashboard">
        <span class="am-nav-icon">◧</span><span class="am-nav-text">داشبورد من</span>
      </div>
      <div class="am-nav-item" onclick="amOpenList('responsible')" id="amNavResponsible">
        <span class="am-nav-icon">◆</span><span class="am-nav-text">اقدام‌های من</span>
        <span class="am-nav-badge" id="amBadgeResponsible">0</span>
      </div>
      <div class="am-nav-item" onclick="amOpenList('author')" id="amNavAuthor">
        <span class="am-nav-icon">✎</span><span class="am-nav-text">ثبت‌شده توسط من</span>
        <span class="am-nav-badge" id="amBadgeAuthor">0</span>
      </div>
      <div class="am-nav-item" onclick="amOpenList('member')" id="amNavMember">
        <span class="am-nav-icon">◎</span><span class="am-nav-text">عضو هستم</span>
        <span class="am-nav-badge" id="amBadgeMember">0</span>
      </div>
      <div class="am-nav-item" onclick="amOpenList('all')" id="amNavAll">
        <span class="am-nav-icon">▤</span><span class="am-nav-text">همهٔ اقدام‌ها</span>
      </div>
    </div>

    <div class="am-main">

      <!-- ============ VIEW: DASHBOARD ============ -->
      <div id="amViewDashboard">
        <div class="am-section-title">خلاصهٔ اقدام‌های من</div>
        <div class="am-kpis">
          <div class="am-kpi" onclick="amOpenList('responsible','open')">
            <div class="am-kpi-label">در انتظار اقدام من</div>
            <div class="am-kpi-value" id="amKpiResponsible">—</div>
          </div>
          <div class="am-kpi am-kpi-alert" onclick="amOpenList('all','overdue')">
            <div class="am-kpi-label">معوق (از مهلت گذشته)</div>
            <div class="am-kpi-value" id="amKpiOverdue">—</div>
          </div>
          <div class="am-kpi" onclick="amOpenList('all','open')">
            <div class="am-kpi-label">باز</div>
            <div class="am-kpi-value" id="amKpiOpen">—</div>
          </div>
          <div class="am-kpi" onclick="amOpenList('all','done')">
            <div class="am-kpi-label">کامل‌شده</div>
            <div class="am-kpi-value" id="amKpiDone">—</div>
          </div>
          <div class="am-kpi" onclick="amOpenList('author')">
            <div class="am-kpi-label">ثبت‌شده توسط من</div>
            <div class="am-kpi-value" id="amKpiAuthor">—</div>
          </div>
          <div class="am-kpi" onclick="amOpenList('member')">
            <div class="am-kpi-label">عضو هستم</div>
            <div class="am-kpi-value" id="amKpiMember">—</div>
          </div>
        </div>

        <div class="am-section-title">اقدام‌هایی که همین حالا منتظر من هستند</div>
        <div class="am-table-wrap">
          <table class="data-table" id="amDashTable">
            <thead>
              <tr>
                <th>شناسه</th><th>عنوان</th><th>مرحلهٔ جاری</th><th>رده</th>
                <th>موضوع</th><th>مهلت</th><th>باقی‌مانده</th><th>وضعیت</th>
              </tr>
            </thead>
            <tbody id="amDashBody">
              <tr><td colspan="8" class="am-empty">در حال بارگذاری…</td></tr>
            </tbody>
          </table>
        </div>
      </div>

      <!-- ============ VIEW: LIST ============ -->
      <div id="amViewList" style="display:none;">
        <div class="am-tabs">
          <div class="am-tab" data-scope="all" onclick="amSetScope('all')">همه</div>
          <div class="am-tab active" data-scope="responsible" onclick="amSetScope('responsible')">مسئولم</div>
          <div class="am-tab" data-scope="author" onclick="amSetScope('author')">مؤلف/مالکم</div>
          <div class="am-tab" data-scope="member" onclick="amSetScope('member')">عضوم</div>
        </div>

        <div class="am-states">
          <div class="am-state active" data-state="all" onclick="amSetState('all')">همه</div>
          <div class="am-state" data-state="open" onclick="amSetState('open')">باز</div>
          <div class="am-state" data-state="running" onclick="amSetState('running')">در جریان</div>
          <div class="am-state" data-state="overdue" onclick="amSetState('overdue')">معوق</div>
          <div class="am-state" data-state="done" onclick="amSetState('done')">کامل‌شده</div>
        </div>

        <div class="am-filters">
          <div class="am-field">
            <label for="amSearch">جستجو (عنوان یا شناسه)</label>
            <input type="text" id="amSearch" placeholder="مثلاً: درخواست پرداخت" />
          </div>
          <div class="am-field">
            <label for="amSection">بخش</label>
            <select id="amSection" onchange="amOnSectionChange()"></select>
          </div>
          <div class="am-field">
            <label for="amCategory">رده</label>
            <select id="amCategory" onchange="amOnCategoryChange()"></select>
          </div>
          <div class="am-field">
            <label for="amTopic">موضوع</label>
            <select id="amTopic"></select>
          </div>
          <div class="am-field">
            <label for="amWorkflow">جریان کار</label>
            <select id="amWorkflow"></select>
          </div>
          <div class="am-field">
            <label for="amFromDate">از تاریخ</label>
            <input type="text" id="amFromDate" placeholder="1405/01/01" />
          </div>
          <div class="am-field">
            <label for="amToDate">تا تاریخ</label>
            <input type="text" id="amToDate" placeholder="1405/12/29" />
          </div>
          <button type="button" class="am-btn" onclick="amApplyFilters()">اعمال فیلتر</button>
          <button type="button" class="am-btn am-btn-ghost" onclick="amResetFilters()">پاک کردن</button>
        </div>

        <div class="am-table-wrap">
          <table class="data-table" id="amListTable">
            <thead>
              <tr>
                <th>شناسه</th><th>عنوان</th><th>بخش</th><th>رده</th><th>موضوع</th>
                <th>جریان کار</th><th>مرحلهٔ جاری</th><th>مسئول</th><th>مؤلف</th>
                <th>شروع</th><th>مهلت</th><th>باقی‌مانده</th><th>وضعیت</th>
              </tr>
            </thead>
            <tbody id="amListBody">
              <tr><td colspan="13" class="am-empty">در حال بارگذاری…</td></tr>
            </tbody>
          </table>
        </div>

        <div class="am-pager">
          <button type="button" class="am-btn am-btn-ghost" onclick="amPrevPage()">صفحهٔ قبل</button>
          <span class="am-pager-info" id="amPagerInfo">—</span>
          <button type="button" class="am-btn am-btn-ghost" onclick="amNextPage()">صفحهٔ بعد</button>
        </div>
      </div>

      <!-- ============ VIEW: CREATE ============ -->
      <div id="amViewCreate" style="display:none;">
        <div class="am-section-title">ایجاد اقدام جدید</div>
        <div class="am-alert am-alert-ok" id="amCreateOk"></div>
        <div class="am-alert am-alert-err" id="amCreateErr"></div>
        <div class="am-form">
          <div class="am-field">
            <label for="amNewCategory">رده</label>
            <select id="amNewCategory" onchange="amOnNewCategoryChange()"></select>
            <div class="am-form-hint">جریان کار و موضوع باید از یک رده باشند — با انتخاب رده، دو فهرست پایین فیلتر می‌شوند.</div>
          </div>
          <div class="am-field">
            <label for="amNewWorkflow">جریان کار</label>
            <select id="amNewWorkflow"></select>
          </div>
          <div class="am-field">
            <label for="amNewTopic">موضوع</label>
            <select id="amNewTopic"></select>
          </div>
          <div class="am-field">
            <label for="amNewTitle">عنوان اقدام</label>
            <input type="text" id="amNewTitle" maxlength="200" placeholder="عنوان روشن و کوتاه" />
          </div>
          <div class="am-field">
            <label for="amNewDeadline">مهلت</label>
            <input type="text" id="amNewDeadline" placeholder="1405/06/20" />
            <div class="am-form-hint">اگر خالی بماند، سرور به‌صورت پیش‌فرض ۷ روز بعد را ثبت می‌کند.</div>
          </div>
          <div class="am-field">
            <label for="amNewComment">توضیحات مرحلهٔ اول</label>
            <textarea id="amNewComment" placeholder="شرح درخواست…"></textarea>
          </div>
          <button type="button" class="am-btn" id="amCreateBtn" onclick="amSubmitCreate()">ثبت اقدام</button>
          <button type="button" class="am-btn am-btn-ghost" onclick="amSwitchView('dashboard')">انصراف</button>
        </div>
      </div>

      <div class="am-footer" id="amFooter"></div>
    </div>
  </div>

  <div class="am-modal" id="amHelpModal">
    <div class="am-modal-box">
      <div class="am-modal-head">
        <h3>راهنمای ماژول اقدام</h3>
        <button type="button" class="am-modal-close" onclick="amCloseHelp()">✕</button>
      </div>
      <div class="am-modal-body">
        <h4>این صفحه چیست؟</h4>
        بازطراحی ظاهری ماژول «اقدام» (گردش‌کار BPMN سازمان). همهٔ داده‌ها بر اساس
        <b>کاربری که همین حالا بات را اجرا می‌کند</b> فیلتر می‌شوند.

        <h4>منوی کناری</h4>
        <ul>
          <li><b>ایجاد اقدام:</b> فرم ثبت اقدام جدید — واقعاً روی سرور ثبت می‌شود.</li>
          <li><b>داشبورد من:</b> خلاصهٔ عددی + فهرست اقدام‌هایی که منتظر شما هستند.</li>
          <li><b>اقدام‌های من / ثبت‌شده توسط من / عضو هستم / همه:</b> ورود مستقیم به فهرست با همان فیلتر.</li>
          <li>با «جمع کردن منو» ساید‌بار باریک می‌شود تا فضای جدول بیشتر شود.</li>
        </ul>

        <h4>تب‌ها (مطابق ماژول بومی)</h4>
        <ul>
          <li><b>مسئولم:</b> مسئولِ مرحلهٔ بازِ فعلی شما هستید (توپ دست شماست).</li>
          <li><b>مؤلف/مالکم:</b> اقدام را شما ثبت کرده‌اید یا مالکش هستید.</li>
          <li><b>عضوم:</b> در فهرست مطلعین هستید یا روی اقدام کامنت گذاشته‌اید.</li>
          <li><b>همه:</b> اجتماع هر سه مورد بالا.</li>
        </ul>

        <h4>نوار وضعیت</h4>
        <ul>
          <li><b>باز:</b> هنوز بسته نشده. <b>در جریان:</b> باز است و دست‌کم یک مرحله‌اش تمام شده.</li>
          <li><b>معوق:</b> باز است ولی از مهلتش گذشته — با رنگ سازمانی مشخص می‌شود.</li>
          <li><b>کامل‌شده:</b> بسته‌شده یا تاریخ پایان واقعی دارد.</li>
        </ul>

        <h4>کار با جدول</h4>
        <ul>
          <li>کلیک روی <b>عنوان هر سطر</b>، صفحهٔ کامل آن اقدام را در تب جدید باز می‌کند.</li>
          <li>کلیک روی <b>عنوان هر ستون</b>، جدول را بر اساس همان ستون صعودی/نزولی مرتب می‌کند.</li>
          <li><b>خروجی Excel</b> فقط سطرهای همان صفحهٔ نمایش‌داده‌شده را خروجی می‌گیرد.</li>
          <li>فیلترهای بالای جدول با دکمهٔ «اعمال فیلتر» روی سرور اجرا می‌شوند (نه فقط روی صفحهٔ جاری).</li>
        </ul>

        <h4>تاریخ‌ها</h4>
        همهٔ تاریخ‌ها شمسی‌اند. در فیلترها و فرم، قالب ورودی <b>۱۴۰۵/۰۶/۰۹</b> است.
      </div>
    </div>
  </div>
</div>
]==]
end

local function build_js()
    return [==[
<script>
(function () {
  "use strict";

  var AM = window.AM_MODULE = window.AM_MODULE || {};
  var BOOT = {{BOOTSTRAP_JSON}};

  /* ---------------------------------------------------------------
     ملاحظهٔ «ماژول ساز»: وقتی بات از داخل صفحهٔ ماژول باز می‌شود،
     location.pathname مسیر صفحهٔ میزبان است نه /bot/run/... — پس مسیر
     فراخوانی خودِ بات باید fallback داشته باشد. مقدار fallback از سمت
     سرور تزریق می‌شود (CONFIG.RUN_PATH_FALLBACK یا bot_config)، تا با
     هر run_path واقعی که بات روی آن مستقر شده هماهنگ بماند.
  --------------------------------------------------------------- */
  var RUN_URL = (location.pathname.indexOf('/bot/run/') === 0) ? location.pathname : BOOT.run_path_fallback;

  var state = {
    view: 'dashboard',
    scope: 'responsible',
    stateFilter: 'all',
    page: 1,
    lastRows: [],
    lastTableId: 'amDashTable'
  };

  /* ---------------- transport ---------------- */
  function callBot(payload, onSuccess, onError) {
    var body = { customform: JSON.stringify(payload) };
    if (window.jQuery && window.jQuery.Teamyar && window.jQuery.Teamyar.ajax) {
      window.jQuery.Teamyar.ajax({
        block_holder: '#amRoot',
        options: {
          block_holder: '#amRoot',
          url: RUN_URL, type: 'POST', dataType: 'json', async: true, data: body
        },
        events: {
          success: function (res) { handleResponse(res, onSuccess, onError); },
          error: function () { onError('ارتباط با سرور برقرار نشد.'); }
        }
      });
      return;
    }
    var form = new URLSearchParams();
    form.append('customform', body.customform);
    fetch(RUN_URL, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' },
      body: form.toString()
    }).then(function (r) { return r.json(); })
      .then(function (res) { handleResponse(res, onSuccess, onError); })
      .catch(function () { onError('ارتباط با سرور برقرار نشد.'); });
  }

  function handleResponse(res, onSuccess, onError) {
    if (!res) { onError('پاسخ خالی از سرور دریافت شد.'); return; }
    if (res.ok === false) { onError(res.error || 'خطای نامشخص.'); return; }
    onSuccess(res);
  }

  /* ---------------- helpers ---------------- */
  function el(id) { return document.getElementById(id); }
  function fa(n) { return Number(n || 0).toLocaleString('en-US'); }
  function txt(v) { return (v === null || v === undefined) ? '' : String(v); }

  function cell(value, className) {
    var td = document.createElement('td');
    if (className) { td.className = className; }
    td.textContent = txt(value);
    return td;
  }

  function badgeCell(row) {
    var td = document.createElement('td');
    var span = document.createElement('span');
    span.className = 'am-badge';
    if (row.state_key === 'overdue') { span.className += ' am-badge-overdue'; }
    if (row.state_key === 'done') { span.className += ' am-badge-done'; }
    span.textContent = txt(row.state);
    td.appendChild(span);
    return td;
  }

  function titleCell(row) {
    var td = document.createElement('td');
    var a = document.createElement('a');
    a.className = 'am-id-link';
    a.textContent = txt(row.title);
    a.target = '_blank';
    a.rel = 'noopener';
    a.href = '/?page=/todo/report/' + encodeURIComponent(row.id);
    td.appendChild(a);
    if (row.is_mine === 1) {
      var mine = document.createElement('span');
      mine.className = 'am-badge am-badge-mine';
      mine.style.marginRight = '6px';
      mine.textContent = 'من';
      td.appendChild(mine);
    }
    return td;
  }

  function idCell(row) {
    var td = document.createElement('td');
    var a = document.createElement('a');
    a.className = 'am-id-link';
    a.textContent = fa(row.id);
    a.target = '_blank';
    a.rel = 'noopener';
    a.href = '/?page=/todo/report/' + encodeURIComponent(row.id);
    td.appendChild(a);
    return td;
  }

  function emptyRow(tbody, colspan, message) {
    tbody.innerHTML = '';
    var tr = document.createElement('tr');
    var td = document.createElement('td');
    td.className = 'am-empty';
    td.colSpan = colspan;
    td.textContent = message;
    tr.appendChild(td);
    tbody.appendChild(tr);
  }

  /* ---------------- rendering ---------------- */
  function renderDashboardRows(rows) {
    var tbody = el('amDashBody');
    if (!rows.length) { emptyRow(tbody, 8, 'اقدام بازی که منتظر شما باشد وجود ندارد.'); return; }
    tbody.innerHTML = '';
    rows.forEach(function (row) {
      var tr = document.createElement('tr');
      if (row.state_key === 'overdue') { tr.className = 'am-row-overdue'; }
      tr.appendChild(idCell(row));
      tr.appendChild(titleCell(row));
      tr.appendChild(cell(row.step));
      tr.appendChild(cell(row.category));
      tr.appendChild(cell(row.topic));
      tr.appendChild(cell(row.due_date));
      tr.appendChild(cell(row.remaining));
      tr.appendChild(badgeCell(row));
      tbody.appendChild(tr);
    });
  }

  function renderListRows(rows) {
    var tbody = el('amListBody');
    if (!rows.length) { emptyRow(tbody, 13, 'با این فیلترها اقدامی یافت نشد.'); return; }
    tbody.innerHTML = '';
    rows.forEach(function (row) {
      var tr = document.createElement('tr');
      if (row.state_key === 'overdue') { tr.className = 'am-row-overdue'; }
      tr.appendChild(idCell(row));
      tr.appendChild(titleCell(row));
      tr.appendChild(cell(row.section));
      tr.appendChild(cell(row.category));
      tr.appendChild(cell(row.topic));
      tr.appendChild(cell(row.workflow));
      tr.appendChild(cell(row.step));
      tr.appendChild(cell(row.responsible));
      tr.appendChild(cell(row.author));
      tr.appendChild(cell(row.start_date));
      tr.appendChild(cell(row.due_date));
      tr.appendChild(cell(row.remaining));
      tr.appendChild(badgeCell(row));
      tbody.appendChild(tr);
    });
  }

  function renderPager(res) {
    el('amPagerInfo').textContent =
      'صفحهٔ ' + fa(res.page) + ' از ' + fa(res.page_count) + '  —  مجموع ' + fa(res.total) + ' اقدام';
  }

  /* ---------------- data loading ---------------- */
  function currentFilters() {
    return {
      search: el('amSearch').value,
      section_id: el('amSection').value,
      category_id: el('amCategory').value,
      topic_id: el('amTopic').value,
      workflow_id: el('amWorkflow').value,
      from_date: el('amFromDate').value,
      to_date: el('amToDate').value
    };
  }

  function loadSummary() {
    callBot({ action: 'summary' }, function (res) {
      var c = res.counts || {};
      el('amKpiResponsible').textContent = fa(c.responsible_open);
      el('amKpiOverdue').textContent = fa(c.overdue);
      el('amKpiOpen').textContent = fa(c.open_total);
      el('amKpiDone').textContent = fa(c.done_total);
      el('amKpiAuthor').textContent = fa(c.authored);
      el('amKpiMember').textContent = fa(c.member);
      el('amBadgeResponsible').textContent = fa(c.responsible_open);
      el('amBadgeAuthor').textContent = fa(c.authored);
      el('amBadgeMember').textContent = fa(c.member);
    }, function (msg) {
      el('amKpiResponsible').textContent = '—';
      el('amFooter').textContent = 'خطا در خلاصه: ' + msg;
    });
  }

  function loadDashboardList() {
    var payload = { action: 'list', scope: 'responsible', state: 'open', page: 1, page_size: 15 };
    callBot(payload, function (res) {
      state.lastRows = res.rows || [];
      state.lastTableId = 'amDashTable';
      renderDashboardRows(state.lastRows);
      initSortable(el('amDashTable'));
    }, function (msg) {
      emptyRow(el('amDashBody'), 8, 'خطا: ' + msg);
    });
  }

  function loadList() {
    var payload = currentFilters();
    payload.action = 'list';
    payload.scope = state.scope;
    payload.state = state.stateFilter;
    payload.page = state.page;
    emptyRow(el('amListBody'), 13, 'در حال بارگذاری…');
    callBot(payload, function (res) {
      state.lastRows = res.rows || [];
      state.lastTableId = 'amListTable';
      renderListRows(state.lastRows);
      renderPager(res);
      state.pageCount = res.page_count;
      initSortable(el('amListTable'));
    }, function (msg) {
      emptyRow(el('amListBody'), 13, 'خطا: ' + msg);
    });
  }

  /* ---------------- navigation ---------------- */
  function setNavActive(id) {
    ['amNavDashboard', 'amNavResponsible', 'amNavAuthor', 'amNavMember', 'amNavAll', 'amNavCreate']
      .forEach(function (nid) { el(nid).classList.remove('active'); });
    if (id) { el(id).classList.add('active'); }
  }

  AM.switchView = function (view) {
    state.view = view;
    el('amViewDashboard').style.display = (view === 'dashboard') ? '' : 'none';
    el('amViewList').style.display = (view === 'list') ? '' : 'none';
    el('amViewCreate').style.display = (view === 'create') ? '' : 'none';
    if (view === 'dashboard') { setNavActive('amNavDashboard'); loadSummary(); loadDashboardList(); }
    if (view === 'create') { setNavActive('amNavCreate'); }
  };

  AM.openList = function (scope, stateFilter) {
    state.scope = scope || 'all';
    state.stateFilter = stateFilter || 'all';
    state.page = 1;
    syncTabs();
    syncStates();
    AM.switchView('list');
    setNavActive(
      scope === 'responsible' ? 'amNavResponsible' :
      scope === 'author' ? 'amNavAuthor' :
      scope === 'member' ? 'amNavMember' : 'amNavAll');
    loadList();
  };

  function syncTabs() {
    document.querySelectorAll('#amRoot .am-tab').forEach(function (t) {
      t.classList.toggle('active', t.getAttribute('data-scope') === state.scope);
    });
  }

  function syncStates() {
    document.querySelectorAll('#amRoot .am-state').forEach(function (s) {
      s.classList.toggle('active', s.getAttribute('data-state') === state.stateFilter);
    });
  }

  AM.setScope = function (scope) { state.scope = scope; state.page = 1; syncTabs(); loadList(); };
  AM.setState = function (st) { state.stateFilter = st; state.page = 1; syncStates(); loadList(); };
  AM.applyFilters = function () { state.page = 1; loadList(); };
  AM.resetFilters = function () {
    ['amSearch', 'amFromDate', 'amToDate'].forEach(function (id) { el(id).value = ''; });
    el('amSection').value = '';
    el('amCategory').value = '';
    fillCategoryOptions('');
    fillTopicWorkflowOptions('');
    state.page = 1;
    loadList();
  };
  AM.prevPage = function () { if (state.page > 1) { state.page -= 1; loadList(); } };
  AM.nextPage = function () {
    if (!state.pageCount || state.page < state.pageCount) { state.page += 1; loadList(); }
  };

  AM.toggleSide = function () { el('amSide').classList.toggle('collapsed'); };

  /* ---------------- filter option lists ---------------- */
  function option(value, label) {
    var o = document.createElement('option');
    o.value = value;
    o.textContent = label;
    return o;
  }

  function fillSelect(select, items, placeholder) {
    select.innerHTML = '';
    select.appendChild(option('', placeholder));
    items.forEach(function (it) { select.appendChild(option(it.id, it.name)); });
  }

  function fillCategoryOptions(sectionId) {
    var items = BOOT.options.categories.filter(function (c) {
      return !sectionId || String(c.section_id) === String(sectionId);
    });
    fillSelect(el('amCategory'), items, 'همهٔ رده‌ها');
  }

  function fillTopicWorkflowOptions(categoryId) {
    var topics = BOOT.options.topics.filter(function (t) {
      return !categoryId || String(t.category_id) === String(categoryId);
    });
    var flows = BOOT.options.workflows.filter(function (w) {
      return !categoryId || String(w.category_id) === String(categoryId);
    });
    fillSelect(el('amTopic'), topics, 'همهٔ موضوعات');
    fillSelect(el('amWorkflow'), flows, 'همهٔ جریان‌های کاری');
  }

  AM.onSectionChange = function () {
    fillCategoryOptions(el('amSection').value);
    fillTopicWorkflowOptions('');
  };
  AM.onCategoryChange = function () { fillTopicWorkflowOptions(el('amCategory').value); };

  /* ---------------- create form ---------------- */
  AM.onNewCategoryChange = function () {
    var categoryId = el('amNewCategory').value;
    var topics = BOOT.options.topics.filter(function (t) { return String(t.category_id) === String(categoryId); });
    var flows = BOOT.options.workflows.filter(function (w) { return String(w.category_id) === String(categoryId); });
    fillSelect(el('amNewTopic'), topics, 'انتخاب کنید…');
    fillSelect(el('amNewWorkflow'), flows, 'انتخاب کنید…');
  };

  function showCreateMessage(kind, message) {
    var ok = el('amCreateOk');
    var err = el('amCreateErr');
    ok.classList.remove('show');
    err.classList.remove('show');
    if (kind === 'ok') { ok.innerHTML = message; ok.classList.add('show'); }
    else { err.textContent = message; err.classList.add('show'); }
  }

  AM.submitCreate = function () {
    var btn = el('amCreateBtn');
    btn.disabled = true;
    showCreateMessage('err', '');
    el('amCreateErr').classList.remove('show');
    callBot({
      action: 'create',
      wf_id: el('amNewWorkflow').value,
      topic_id: el('amNewTopic').value,
      task_title: el('amNewTitle').value,
      deadline: el('amNewDeadline').value,
      comment: el('amNewComment').value
    }, function (res) {
      btn.disabled = false;
      var link = '/?page=/todo/report/' + encodeURIComponent(res.task_id);
      showCreateMessage('ok',
        'اقدام با شناسهٔ ' + fa(res.task_id) + ' ثبت شد. ' +
        '<a href="' + link + '" target="_blank" rel="noopener" style="color:#ffffff;text-decoration:underline;">مشاهدهٔ اقدام</a>');
      el('amNewTitle').value = '';
      el('amNewComment').value = '';
      el('amNewDeadline').value = '';
      loadSummary();
    }, function (msg) {
      btn.disabled = false;
      showCreateMessage('err', msg);
    });
  };

  /* ---------------- toolbar ---------------- */
  /* تمام صفحه: فقط toggle کلاس CSS — نه Fullscreen API (روی iframe/موبایل غیرقابل‌اتکاست) */
  AM.toggleFullScreen = function () { el('amRoot').classList.toggle('am-fullscreen'); };
  AM.openHelp = function () { el('amHelpModal').classList.add('active'); };
  AM.closeHelp = function () { el('amHelpModal').classList.remove('active'); };

  function csvCell(t) {
    t = (t === null || t === undefined ? '' : String(t)).replace(/\s+/g, ' ').trim();
    if (t.indexOf(',') !== -1 || t.indexOf('"') !== -1) { t = '"' + t.replace(/"/g, '""') + '"'; }
    return t;
  }

  AM.exportToExcel = function () {
    var table = el(state.lastTableId);
    if (!table) { return; }
    var lines = [];
    var heads = table.querySelectorAll('thead th');
    var hl = [];
    for (var h = 0; h < heads.length; h++) { hl.push(csvCell(heads[h].innerText)); }
    lines.push(hl.join(','));
    var rows = table.querySelectorAll('tbody tr');
    for (var i = 0; i < rows.length; i++) {
      var cells = rows[i].querySelectorAll('td');
      var line = [];
      for (var c = 0; c < cells.length; c++) { line.push(csvCell(cells[c].innerText)); }
      lines.push(line.join(','));
    }
    var blob = new Blob(['﻿' + lines.join('\r\n')], { type: 'text/csv;charset=utf-8;' });
    var url = URL.createObjectURL(blob);
    var link = document.createElement('a');
    link.href = url;
    link.download = 'اقدام-ها.csv';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  };

  /* ---------------- sortable headers ---------------- */
  function getCellSortValue(cellNode) {
    var text = cellNode ? cellNode.innerText.trim() : '';
    var n = text.replace(/[,%]/g, '').trim();
    if (n !== '' && /^-?\d+(\.\d+)?$/.test(n)) { return parseFloat(n); }
    return text;
  }

  function sortTableByColumn(table, colIndex, dir) {
    var tbody = table.querySelector('tbody');
    var rows = Array.prototype.slice.call(tbody.querySelectorAll('tr'));
    rows.sort(function (ra, rb) {
      var a = getCellSortValue(ra.children[colIndex]);
      var b = getCellSortValue(rb.children[colIndex]);
      var cmp;
      if (typeof a === 'number' && typeof b === 'number') { cmp = a - b; }
      else { cmp = String(a).localeCompare(String(b), 'fa'); }
      return dir === 'asc' ? cmp : -cmp;
    });
    rows.forEach(function (row) { tbody.appendChild(row); });
  }

  function initSortable(table) {
    if (!table || table.getAttribute('data-sortable') === '1') { return; }
    table.setAttribute('data-sortable', '1');
    var headers = table.querySelectorAll('thead th');
    headers.forEach(function (th, colIndex) {
      th.addEventListener('click', function () {
        var dir = th.classList.contains('sort-asc') ? 'desc' : 'asc';
        headers.forEach(function (h) { h.classList.remove('sort-asc', 'sort-desc'); });
        th.classList.add(dir === 'asc' ? 'sort-asc' : 'sort-desc');
        sortTableByColumn(table, colIndex, dir);
      });
    });
  }

  /* ---------------- boot ---------------- */
  function init() {
    fillSelect(el('amSection'), BOOT.options.sections, 'همهٔ بخش‌ها');
    fillCategoryOptions('');
    fillTopicWorkflowOptions('');
    fillSelect(el('amNewCategory'), BOOT.options.categories, 'انتخاب کنید…');
    fillSelect(el('amNewTopic'), [], 'ابتدا رده را انتخاب کنید');
    fillSelect(el('amNewWorkflow'), [], 'ابتدا رده را انتخاب کنید');
    el('amFooter').textContent = BOOT.footer || '';
    if (BOOT.user_warning) { showCreateMessage('err', BOOT.user_warning); }
    syncTabs();
    syncStates();
    AM.switchView('dashboard');
  }

  el('amHelpModal').addEventListener('click', function (e) {
    if (e.target === el('amHelpModal')) { AM.closeHelp(); }
  });
  document.addEventListener('keydown', function (e) { if (e.key === 'Escape') { AM.closeHelp(); } });

  /* پل به onclickهای inline (که در ماژول‌ساز هم قابل اتکاست) */
  window.amSwitchView = AM.switchView;
  window.amOpenList = AM.openList;
  window.amSetScope = AM.setScope;
  window.amSetState = AM.setState;
  window.amApplyFilters = AM.applyFilters;
  window.amResetFilters = AM.resetFilters;
  window.amPrevPage = AM.prevPage;
  window.amNextPage = AM.nextPage;
  window.amToggleSide = AM.toggleSide;
  window.amOnSectionChange = AM.onSectionChange;
  window.amOnCategoryChange = AM.onCategoryChange;
  window.amOnNewCategoryChange = AM.onNewCategoryChange;
  window.amSubmitCreate = AM.submitCreate;
  window.amToggleFullScreen = AM.toggleFullScreen;
  window.amOpenHelp = AM.openHelp;
  window.amCloseHelp = AM.closeHelp;
  window.amExportToExcel = AM.exportToExcel;

  init();
})();
</script>
]==]
end

-- ============================================================
-- PAGE RENDER
-- ============================================================

-- json.encode سپس امن‌سازی برای درج داخل <script> (بدون شکستن تگ)
local function json_for_script(value)
    local encoded = json.encode(value)
    local out = tostring(encoded)
        :gsub("<", "\\u003C")
        :gsub(">", "\\u003E")
        :gsub("&", "\\u0026")
    return out
end

-- جایگزینی امن Placeholder: از تابع استفاده می‌شود تا کاراکتر % در مقدار، تفسیر نشود
local function fill(template, key, value)
    local out = template:gsub("{{" .. key .. "}}", function() return value end)
    return out
end

local function render_page(ctx)
    local bootstrap = {
        run_path_fallback = ctx.run_path_fallback,
        user_id = ctx.user_id,
        options = ctx.options,
        footer = ctx.footer,
        user_warning = ctx.user_warning,
    }

    local html = build_css() .. build_body_html() .. build_js()
    html = fill(html, "PEYDA_REGULAR_B64", PEYDA_REGULAR_B64)
    html = fill(html, "PEYDA_BOLD_B64", PEYDA_BOLD_B64)
    html = fill(html, "LOGO140_WHITE_B64", LOGO140_WHITE_B64)
    html = fill(html, "USER_NAME", escape_html(ctx.user_label))
    html = fill(html, "BOOTSTRAP_JSON", json_for_script(bootstrap))
    return html
end

local function render_error_page(message)
    return build_css() .. [==[
<div id="amRoot">
  <div class="am-header">
    <div class="am-brand"><span class="am-title">ماژول اقدام</span></div>
  </div>
  <div class="am-main">
    <div class="am-alert am-alert-err show">]==] .. escape_html(message) .. [==[</div>
  </div>
</div>
]==]
end

-- ============================================================
-- MAIN
-- ============================================================

local function main()
    local input = teamyar.get_input() or {}

    -- ورودی‌های AJAX این بات با کلید customform (JSON) می‌آیند — الگوی تاییدشدهٔ بات‌های [Module]
    if input["customform"] ~= nil and trim(input["customform"]) ~= "" then
        local ok, decoded = pcall(function() return json.decode(input["customform"]) end)
        if ok and type(decoded) == "table" then
            for k, v in pairs(decoded) do input[k] = v end
        end
    end

    -- مسیر اجرای بات برای فراخوانی‌های AJAX داخلی؛ قابل بازنویسی از تب «پیکربندی» بات
    -- (بدون نیاز به دیپلوی مجدد، اگر run_path تغییر کرد یا بات کپی شد)
    local run_path_fallback = CONFIG.RUN_PATH_FALLBACK
    local cfg_ok, cfg = pcall(function() return teamyar.get_config() end)
    if cfg_ok and type(cfg) == "table" and type(cfg.data) == "table" then
        local configured = trim(cfg.data.run_path)
        if configured ~= "" then run_path_fallback = configured end
    end

    local action = trim(input["action"])

    if action == "whoami" then
        respond(handle_whoami(input))
        return
    end

    local user_id, user_source = resolve_current_user(input)
    local now_raw = now_filetime()

    if action == "state_probe" then
        respond(handle_state_probe())
        return
    end

    if action == "list" or action == "summary" or action == "create" then
        if user_id == nil then
            respond({
                ok = false,
                error = "شناسهٔ کاربر جاری قابل تشخیص نبود. یک بار بات را با action=whoami اجرا کنید " ..
                    "تا کلید درست در CONFIG.USER_ID_KEYS تنظیم شود، یا پارامتر user_id را ارسال کنید.",
            })
            return
        end
        if action == "list" then
            respond(handle_list(input, user_id, now_raw))
        elseif action == "summary" then
            respond(handle_summary(user_id, now_raw))
        else
            local result = create_action(input)
            respond(result)
        end
        return
    end

    -- ---------- رندر صفحهٔ کامل (HTML) ----------
    local user_label = ""
    local user_warning = nil
    if user_id == nil then
        user_warning = "شناسهٔ کاربر جاری قابل تشخیص نبود؛ فهرست‌ها خالی می‌مانند. " ..
            "یک بار بات را با action=whoami اجرا کنید تا کلید درست شناسایی شود."
        user_label = "کاربر ناشناس"
    else
        local name = fetch_user_name(user_id)
        user_label = safe_str(name, "کاربر " .. tostring(user_id))
    end

    local options = fetch_filter_options()
    local generated = jalali_datetime_str(now_raw) or ""

    local footer = "ماژول اقدام — تولید گزارش: " .. generated ..
        "  |  دامنهٔ داده: کاربر جاری (" .. tostring(user_source) .. ")"

    local html = render_page({
        run_path_fallback = run_path_fallback,
        user_id = user_id,
        user_label = user_label,
        user_warning = user_warning,
        options = options,
        footer = footer,
    })

    teamyar.write_result(html)
end

local ok, err = pcall(main)
if not ok then
    teamyar.write_result(render_error_page("خطای غیرمنتظره در اجرای بات: " .. tostring(err)))
end
