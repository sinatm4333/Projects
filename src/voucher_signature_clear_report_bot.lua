...
.danger-band{ background:var(--accent); color:#fff; border-radius:12px; padding:16px 20px; margin-bottom:16px; }
.danger-band h2{ margin:0 0 8px; font-size:15px; font-weight:bold; }
.danger-band ul{ margin:0; padding-inline-start:20px; }
.danger-band li{ font-size:14px; line-height:2; }
.notice{ background:#fff; border:1.5px solid var(--accent); border-radius:12px; padding:14px 18px; margin-bottom:16px; font-size:14px; line-height:2; }
.notice strong{ font-weight:bold; }
.warn-list{ background:var(--zebra); border:1px solid var(--border); border-radius:10px; padding:10px 20px; margin-bottom:14px; }
.warn-list li{ font-size:14px; line-height:2; }
...
.filter-field input, .filter-field select{ font-size:14px; padding:8px 10px; border:1px solid var(--border); border-radius:8px; background:#fff; color:#000; }
...
table.data-table thead th.sort-asc::after{ content:" \25B2"; }
table.data-table thead th.sort-desc::after{ content:" \25BC"; }
table.data-table tbody tr:nth-child(even){ background:var(--zebra); }
.table-scroll{ overflow-x:auto; max-height:60vh; }
.chip{ display:inline-block; padding:2px 10px; border-radius:11px; font-size:14px; font-weight:bold; white-space:nowrap; }
.chip.on{ background:var(--accent); color:#fff; }
.chip.off{ background:var(--zebra); color:var(--muted); border:1px solid var(--border); }
.chip.line{ background:#fff; color:var(--accent); border:1.5px solid var(--accent); }
a.link{ color:var(--accent); text-decoration:none; font-weight:bold; }
a.link:hover{ text-decoration:underline; }
.empty-row{ text-align:center; color:var(--muted); padding:16px; font-size:14px; }
.confirm-box{ display:flex; gap:12px; flex-wrap:wrap; align-items:flex-end; }
.confirm-box .filter-field{ min-width:220px; }
.run-log{ margin-top:14px; }
...
.modal-content{ background:#fff; border-radius:14px; max-width:760px; width:92%; max-height:82vh; overflow-y:auto; }
...
.modal-body{ padding:18px 20px; font-size:14px; line-height:2.1; }
.modal-body h4{ font-size:15px; font-weight:bold; margin:14px 0 6px; }
.modal-body ul{ padding-inline-start:20px; margin:0; }
</style>
]]
...
local function render_execute_panel(scope, filters)
    ...
    return table.concat({
        ...
        '<div class="filter-actions">',
        '<button type="button" class="btn-primary" id="runButton">شروع پاک‌سازی</button>',
        '</div></div>',
...
local function render_help_modal()
    return table.concat({
        ...
        '<button class="modal-close" type="button" onclick="closeHelp()">×</button></div>',
...
function toggleFullScreen(){
  document.getElementById('reportRoot').classList.toggle('pseudo-fullscreen');
  window.scrollTo(0, 0);
}
...
function exportToExcel(){
  ...
  var headLine = [];
  for (var h = 0; h < heads.length; h++) headLine.push(csvCell(heads[h].innerText));
...
function renderRunResults(payload){
  var box = document.getElementById('runLog');
  ...
  if (summary.aborted) head += '<br><strong>اجرا متوقف شد:</strong> ' + escapeHtml(summary.abort_reason || '');
  head += '</div>';
...
function runClear(){
  ...
  if (confirmInput.value.trim() !== CONFIRM_PHRASE) {
    box.innerHTML = '<div class="notice">برای شروع، عبارت «' + escapeHtml(CONFIRM_PHRASE) +
...
local function render_page(filters, scope, actor)
    ...
    return table.concat({
        ...
        '<button type="button" class="btn-toolbar secondary" onclick="openHelp()">راهنما</button>',
        '</div>',
