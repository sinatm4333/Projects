...
| `hr_companion_report_bot.lua` (جدید، هنوز مستقر نشده) | HTML/JSON | `days` (طول بازه، پیش‌فرض ۳۱)، `from_date`/`to_date` (FILETIME عددی)، `personnel_id`/`profile_id` (اختیاری — باز کردن پروندهٔ فرد دیگر)، `format=json`، `type=whoami` (تشخیص کلید شناسهٔ کاربر جاری)، `type=celebration_thread`/`type=celebrate` (گفتگوی تبریک) | **پنل پرسنلی «همراه ۱۴۰»** — نسخهٔ دادهٔ واقعی از HTML نمونهٔ کاربر. پنج تب: نمای کلی (وضعیت امروز، شاخص‌های بازه، ریتم کاری)، تردد و کارکرد (`hr_work_time` + رویدادهای خام `hr_ext_time` با نام دستگاه از `hr_machine`، و شیفت واقعی هر روز از `hr_day_details`)، درخواست‌های من (`hr_vacation` + `hr_vacation_type` + `hr_leave_verify` برای زنجیرهٔ تایید، به‌علاوهٔ `hr_overtime_request` و `hr_telework_request`)، اطلاعات پرسنلی (`hr_personnels`/`hr_personnel_order`/`hr_education`)، و «همراهِ روز و تولدها» (پیام روزانهٔ چرخشی بر پایهٔ `report_dimdate.JYDAY`، تولدهای امروز/این ماه از `profile_user_info.BIRTHDAY`، و **گفتگوی تبریک create-or-join**: عنوان قطعی در `todo_task` جست‌وجو می‌شود و پیام با `/api/todo/task/comment/add` به همان گفتگو اضافه یا با `/api/todo/taskadd` ساخته می‌شود — نیازمند `celebration_wf_id`/`celebration_topic_id` در `bot_config`؛ بدون آن‌ها بقیهٔ بات سالم کار می‌کند). **محدودیت‌های شناخته‌شده:** ثبت درخواست جدید، فیش حقوقی، و برچسب «تردد ناموفق» عمداً پیاده نشده؛ `WORKING_HOURS`/`LEAVE_PER_MONTH` چون واحدشان تایید نشده از UI حذف و فقط در `format=json` باقی مانده‌اند |
