...
| `call_center_admin_dashboard_probe_json_bot.lua` (پروب اکتشافی، موقت) | JSON | `date_from`, `date_to` (FILETIME، پیش‌فرض ۳۰ روز اخیر)، `sample_limit` | قبل از نوشتن `call_center_admin_dashboard_report_bot.lua`: توزیع واقعی `DialStatus`، تعیین تجربی قرارداد Inbound/Outbound، وجود/عدم‌وجود اسکیمای Presence آنلاین/مشغول کارمند، وجود جدول آرشیو، نمونهٔ `voip_TY_PERMISSION`/`voip_phone_extension`. نتیجه با SQL خام مستقیم توسط کاربر گرفته شد (نه با این بات — این سشن دسترسی شبکه به `erp.bimehland.com` ندارد)؛ نتیجه در `call_center_admin_dashboard_report_bot.lua` مستند و اعمال شده |
| `call_center_admin_dashboard_report_bot.lua` (دیپلوی‌شده، فیدبک زنده اعمال شد) | HTML | `type=1` AJAX از/تا (سه Dropdown شمسی روز/ماه/سال، پیش‌فرض ۷ روز اخیر؛ بارگذاری اولیه بدون ورودی) | داشبورد واحد مدیریتی کال‌سنتر — جایگزین ۹ بات پراکندهٔ بخش «بات‌های کال‌سنتر / VOIP» زیر. موبایل-اول (CSS پایه تک‌ستونه، `@media(min-width)` گسترش به تبلت/دسکتاپ)، گاید ۱۴۰. سه بخش: نمای‌کلی (۱۲ کارت KPI شامل ورودی/خروجی/داخلی/بدون‌اتصال/AHT/ساعت‌اوج + دونات نتیجه/جهت + تاپ ۱۰ کارمند)، روند تماس‌ها (روزانه + ساعتی)، عملکرد کارمندان (نمودار توزیع پاسخ‌گویی + جدول کامل قابل‌مرتب‌سازی). شمارش تماس همه‌جا `COUNT(DISTINCT linkedid)` (نه `COUNT(*)`، چون هر تماس ۲ Leg دارد)؛ Inbound/Outbound/Internal با تطبیق profile ID (نه آستانهٔ عددی) — تماس داخلی کارمند↔کارمند از ورودی/خروجی جدا شمرده می‌شود؛ «وضعیت کارمندان» = عملکردی، نه presence لحظه‌ای. **باگ واقعی رفع‌شده (فیدبک اول دیپلوی زنده):** فیلتر با `<form method="POST" action="">` خام مرورگر کاربر را از بات بیرون می‌انداخت — endpoint واقعی بات customform می‌خواهد نه urlencoded خام؛ با `$.Teamyar.ajax` + `customform` (الگوی اثبات‌شدهٔ `moadian_invoices_dashboard_report_bot.lua`) جایگزین شد، پاسخ فقط JSON با تکه‌های HTML (نه کل سند) برای رفرش سریع بدون بارگذاری دوبارهٔ فونت/لوگو. `CONFIG.EXCLUDED_AGENT_IDS`: چون `profile_main` ستون فعال/غیرفعال ندارد، فهرست دستی شناسهٔ کارمندان سابق که هنوز نگاشت داخلی/VOIP‌شان به تماس زنده وصل مانده (Workaround نمایشی، اصلاح ریشه‌ای باید سمت PBX/VOIP انجام شود) — تاییدشده با کاربر برای شناسهٔ ۳۶۵۲۰. تاییدشده با `luac -p` + یک هارنس Lua با runtime شبیه‌سازی‌شده (دو سناریو: بارگذاری اولیه + `type=1`، هر دو با `node --check` روی JS استخراج‌شده هم اعتبارسنجی شدند) |
...
**بعدی:** `call_center_admin_dashboard_report_bot.lua` نوشته و اعتبارسنجی شد — ردیفش در جدول «Reference Bots» بالای همین فایل است. منتظر دیپلوی توسط کاربر (SID خودش) روی Teamyar.
