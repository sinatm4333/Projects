# دریافت لیست اطلاعات جریان های کار یک رده

دریافت لیست اطلاعات جریان های کار یک رده با شناسه رده

## آدرس

```
/api/todo/wf/list/get
```
...
| `data[].form_id` | integer (int64) | شناسه فرمsource table name : `0000000`.todo_form \| column name: ID |
...
| `data[].form_id_request` | integer (int64) | شناسه فرمی که میخواهیم در زمان ایجاد اقدام از طریق پورتال به کاربر نمایش داده شود و پر شودsource table name : `0000000`.todo_form \| column name: ID |
...
| `data[].add_task_mandatory_fields` | integer (int64) | جهت افزودن اقدام در حاضر میتوانیم 3 فیلد را اجباری تعیین کنیم , 2 به توان n در دیتا بیس دخیره میشودTODO_TASK_EXTRA_FIELD_PRODUCT = 2,TODO_TASK_EXTRA_FIELD_CRM = 1,TODO_TASK_EXTRA_FIELD_PROJECT = 4, |
