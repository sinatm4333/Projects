...
| `voucher_signature_clear_report_bot.lua` (بات ۶۱۷، `run/443/VoucherSignatureClear`) | HTML | `org`، `itype`، `date_from`/`date_to` (شمسی)، `invoice_id`، `scope` (`signed`\|`full`\|`any`)، `cap` (سقف سند در هر اجرا، پیش‌فرض **۱**)، `payload_mode` (`minimal`\|`full`)؛ AJAX داخلی: `action=execute` + `confirm` | **بات نویسنده — تنها بات این ریپو که روی دادهٔ مالی می‌نویسد.** خواستهٔ کاربر: خودکارسازی دسته‌ایِ همان کار دستی «سند حسابداری فاکتور را ویرایش کن تا امضاهایش باطل شود». مسیر: فاکتور → `pa_voucher_record` (با `R_TYPE IN (14,22)`، چون `REFFERE_ID` چندریختی است) → `pa_voucher` → `pa_voucher_signs`؛ سپس ویرایش شرحِ یک ردیف سند از طریق **ماژول حسابداری** (`teamyar.call_api(10, "/api/voucher/records_update", …)`) با Toggle یک فاصلهٔ انتهایی. **فرضیهٔ تأییدنشده:** ماژول حسابداری هیچ endpoint امضا/حذف‌امضا ندارد (۲۴ endpoint، ر.ک. `TeamyarInternalApiReference.md`) — اینکه ویرایش از مسیر API همان منطق باطل‌کنندهٔ امضای UI را فعال کند تأیید نشده؛ برای همین بات بعد از هر سند `pa_voucher_signs` را دوباره می‌شمارد و بین `cleared` / `no_write` (API موفق گفت ولی چیزی ننوشت) / `no_effect` (نوشت ولی امضا نرفت) تفکیک می‌کند و با اولین حالت غیرموفق کل دسته را متوقف می‌کند. گام «اول امضا کن» خودکارشدنی نیست (API امضا وجود ندارد) — ستون‌های `LOCK_VOUCHER`/`STATUS`/`CHANGED_FLAG` در پیش‌نمایش برای همین‌اند. محافظ‌ها: پیش‌نمایش پیش‌فرض و فقط‌خواندنی، عبارت تأیید «حذف امضا»، اجرا بدون بازهٔ تاریخ یا `invoice_id` ممنوع، سقف پیش‌فرض ۱ سند (سقف مطلق ۲۰۰)، رد کردن سندی که شرحش به ۵۰۰ کاراکتر رسیده (به‌جای بریدن متن)، ثبت pre-image هر رکورد در لاگ، و **مقایسهٔ رکورد قبل/بعد از نوشتن؛ تغییر هر ستونی جز `CONTENT`/`DATE_MODIFY` = توقف فوری اجرا**. دامنه همیشه سمت سرور از روی فیلتر بازساخته می‌شود (فهرست سند از مرورگر پذیرفته نمی‌شود). `public_access` باید خاموش بماند — خود اسکریپت هم آن را رد می‌کند. لوگو ۱۴۰ (White) + فونت Peyda + پالت #16509D طبق الزام CLAUDE.md. **نتیجهٔ آزمون زنده (1405/06/04) — فرضیهٔ اصلی رد شد:** روی سند ۵۰۸۳ (ORG 8، ۱۵ امضا) نوشتن قطعاً انجام شد (`CONTENT` عوض شد، بازرسی یکپارچگی پاک) ولی امضاها **۱۵ → ۱۵** ماندند. یعنی `voucher/records_update` منطق باطل‌کنندهٔ امضا را فعال نمی‌کند؛ آن منطق در کنترلر ماژول حسابداری است و مسیر `/api/` دورش می‌زند. بات این را به‌درستی `no_effect` گزارش کرد و دسته را متوقف کرد (طبق طراحی — ادعای موفقیت جعلی نداد). **مسیر API برای این کار بن‌بست است**؛ ر.ک. یادداشت زیر `/api/voucher/records_update` در `TeamyarInternalApiReference.md`. گام بعدی پیشنهادی: فراخوانی endpointهای واقعی پنل حسابداری از سمت مرورگر (JS خود بات داخل نشست کاربر اجرا می‌شود) — نیازمند ثبت آدرس آنها از تب Network. |
