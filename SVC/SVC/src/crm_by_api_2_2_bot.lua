...
-- =========================================
-- Create Account (module 10) for REAL + LEGAL
...
-- =========================================
-- CRM Upsert
-- national_code رشته 10 رقمی با صفرهای ابتدا
-- آدرس: حقیقی => home.address | حقوقی => work.address
-- نام حقوقی: اگر خالی بود موبایل ننشیند، "شرکت بدون نام"
-- بعد از create/update کلاینت، ساخت حساب هم انجام شود
-- =========================================
local function updateInsertCrm(national_code, mobile, fname, lname, city, state, address_text, zip_code, is_man, email, user_type, saite_id)
  ...
  -- نام‌ها: حقوقی موبایل جای نام ننشیند
  ...
  if user_id ~= nil and tonumber(user_id) > 0 then
      ...
      -- ساخت حساب بعد از update (برای حقیقی و حقوقی)
  ...
  if res_create_crm and res_create_crm.success == true then
    ...
    if user_id ~= nil then
      ...
      -- ساخت حساب بعد از create (برای حقیقی و حقوقی)
      create_account_for_client(user_id, fname .. " " .. lname)
...
-- =========================================
-- Get Customer Preview (with locations) — external API (not erp.bimehland.com)
-- =========================================
...
-- =========================================
-- Main Fetch List (c_url_api_id) and Process — external API
-- =========================================
...
local function process_customer(customer_id)
  ...
  if has_legal then
    ...
    -- businessName جای نام شرکت حقوقی
...
-- =========================================
-- RUN: page-by-page loop over external customer list, upsert into CRM
-- =========================================
local page = 1
local pagesize = 10
