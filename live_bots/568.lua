...
function getData(queryType , pageFrom , perPage , pageTo )
  ...
  if stock ~= nil and stock[1] ~= nil then 
...
--- manager
--------------------------------------------
function savefactor(fid,referenceNumber,date_replace,org_id)
 ...
 teamyar.write_log("org_id---"..org_id)
  local config_id = queryResultOne([[select  CONFIG_ID  from bot_command_config where COMMAND_ID=]]..c_bot_send_id..[[ and json_unquote(config->"$.org_id")=]]..org_id,{})
   teamyar.write_log("config_id---"..json.encode(config_id))
  local res_bot = ""
  ...
  res_bot = teamyar.run_command("2/tax_gov_test/"..config_id , {invoice_id = tostring(fid), referenceNumber = referenceNumber,date_replace= date_replace});
  end
...
elseif type ~= nil and type == 10 then
  ...
  --  teamyar.write_log("date_replace input---"..json.encode(input))
  local msg ="شماره فاکتور:"..fid.."  ".. savefactor(fid,referenceNumber,date_replace,input.org_id)
...
elseif type ~= nil and type == 11 then
  ...
  for i,v in ipairs (res_data) do 
    if  v.status~="تایید شده" then
      savefactor(v.id, v.referenceNumber, date_replace,input.org_id)-- .."<hr>"
