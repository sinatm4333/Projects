...
| `sales_duplicate_order_invoices_report_bot.lua` (بات **478** «فاکتورهای تکراری با شماره سفارش سایت»، `run/443/GetDupplicateFactorsByName2`، cat 59 «Sale»، subsystem `SALES_VIEW/23_2` — **دیپلوی و روی دادهٔ زنده تایید شد ۱۴۰۵/۰۶/۰۹**: ۵۲۴ گروه تکراری/۱٬۰۵۳ فاکتور با لینک‌های سالم. دیپلوی با `deploy_teamyar_bot.ps1` + پارامترهای echo (`-Categories '59_1' -SubsystemValue '23_2' -SubsystemName 'SALES_VIEW' -PublicAccess 1 -ShowInWidget 1 -OpenSource 1 -Description '' -BotConfigJson <زنده>`) — `update_bot_preserve.ps1` روی این سرور 502 می‌دهد، استفاده نشود) | HTML/JSON | `format=json` (اختیاری) | هر فاکتور فروش حذف‌نشده‌ای که TITLE (شماره سفارش سایت) آن بیش از یک بار ثبت شده — کل تاریخچه، بدون فیلتر سازمان (عمداً مثل بات مبدأ). ستون‌ها: شماره سفارش، تعداد تکرار گروه، شماره فاکتور (**لینک** به `/?page=/sales/invoice/view_invoice/{ID}`)، تاریخ شمسی (JOIN بازه‌ای `report_dimdate` — همان الگوی بات مبدأ؛ `REPORT_FN_JDATE` روی این سرور وجود ندارد)، مبلغ ریالی (÷MONEY_SCALE با هزارگان — بات مبدأ عدد خام ×10^7 نشان می‌داد)، کد/نام مشتری (**لینک** به `/?page=/crm/client/edit/{CLIENT_ID}`). عنوان‌های خالی از گروه تکراری حذف شدند؛ سقف نمایش ۲۰۰۰ ردیف با جمع‌های کل جداگانه و ردیف اطلاع‌رسانی pin‌شده (کلاس `notice-row`، از مرتب‌سازی مستثنا). ظاهر کامل گاید: Peyda embedded، #16509D، سورت هدر، تمام‌صفحهٔ CSS-toggle (نه Fullscreen API)، Excel/راهنما |
...
| `sales_invoice_note_mismatch_report_bot.lua` (بازنویسی بات زندهٔ **483** «مغایرت گیری توضیحات سایت با صاحب فاکتور»، `run/443/ClienVsNote` — **دیپلوی و روی دادهٔ زنده تایید شد 1405/06/09** (۲۴۹هزار فاکتور کل، ۲۰۰هزار بررسی، ۲۷٬۱۸۶ مغایرت). **cat_id=59 «Sale»** — نه ۷۹: اولین دیپلوی اشتباهاً دسته را به ۷۹ منتقل و public_access/show_in_widget/open_source را صفر کرد؛ همان جلسه با -CatId 59 و فلگ‌های ۱ بازگردانی و با bot_command_categories تایید شد (ر.ک. هشدار Update در CLAUDE.md)) | HTML | بدون ورودی | مغایرت‌گیری NOTE فاکتور فروش با نام صاحب فاکتور: NOTE و نام مشتری نرمال‌سازی می‌شوند (حذف فاصله/کاراکترهای نامرئی + «ی» عربی→فارسی بدون «ئ»، با `string.char` نه Literal نامرئی)؛ NOTE خالی یا فاقد نام مشتری = مغایرت. ستون‌ها: فاکتور (نمایش `INVOICE_CODE`، لینک با `si.ID` به `view_invoice`)، وضعیت (CASE روی CANCELED/CLOSE_PRE/INVOICE_ID/PRE_INVOICE)، مشتری (لینک CRM با `pa_client.REFFERE_ID` به `/?page=/crm/client/edit/{id}` + پسوند section=2 با Concatenation)، NOTE (کوتاه‌شده + Tooltip). باگ‌های نسخهٔ مبدأ که رفع شد: esc() نابودشده توسط Quirk ذخیره‌سازی (XSS + فاصلهٔ اضافه در «>»)، قالب‌سازی gsub روی توکن‌های {{X}} (تزریق توکن از دادهٔ NOTE)، ۱۰ کوئری probe برای حدس ستون شمارهٔ فاکتور، ۶ کوئری چندپلتفرمهٔ تاریخ، لینک CRM تاییدنشده. سقف بررسی ۲۰۰هزار فاکتور + سقف رندر ۵هزار ردیف با هشدار صریح (نه حذف خاموش) |
| `api_caller_json_bot.lua` (bot 623، cat_id=79، `443/api_caller_json`) | JSON | `module_id` (الزامی)، `url` (الزامی، مثلاً `/api/dialog/add`)، `params` (شیء یا رشتهٔ JSON)، `secret_key` (وقتی هدر `secret-key` در دسترس نیست، مثل فراخوانی از بات دیگر) | فراخوان عمومی APIهای داخلی (`teamyar.call_api`) بر پایهٔ الگوی `call_api_1_bot.lua`، با سه تفاوت: `module_id`/`url` ورودی‌اند (نه hardcode)، کلید از `bot_config.secret_key` خوانده می‌شود (نه داخل سورس)، و **allowlist اجباری** دارد. رفتار پیش‌فرض fail-closed است: بدون `secret_key` و `allowed_modules` در `bot_config` هیچ فراخوانی انجام نمی‌شود؛ `allowed_paths` هم اختیاری و برای محدودتر کردن است. خروجی شامل `request` و پاسخ خام `response` است — همین برای **استخراج schema واقعی یک endpoint** قبل از استفاده در بات کافی است. `hr_companion_report_bot.lua` وقتی `api_caller_path` در تنظیماتش باشد، فراخوانی‌های ماژول گفتگو را از همین بات رد می‌کند |
| `hr_companion_report_bot.lua` (bot 624، cat_id=79، `443/hr_companion_report`) | HTML/JSON | **پیش‌فرض بازه = ماه شمسی جاری** (روز اول ماه تا امروز، مرز ماه از `report_dimdate`)؛ `month_offset` (۱=ماه قبل کامل، ...)، `days` (بازهٔ دلخواه — حالت ماهانه را خاموش می‌کند)، `from_date`/`to_date` (FILETIME عددی)، `format=json`، `type=whoami` (تشخیص کلید شناسهٔ کاربر جاری)، `type=celebration_thread`/`type=celebrate` (گفتگوی تبریک). **شناسهٔ پرسنل از ورودی پذیرفته نمی‌شود** — همیشه کاربرِ واردشده | **پنل پرسنلی «همراه ۱۴۰»** — نسخهٔ دادهٔ واقعی از HTML نمونهٔ کاربر. پنج تب: نمای کلی، تردد و کارکرد (`hr_work_time` + رویدادهای خام `hr_ext_time` با نام دستگاه از `hr_machine` + شیفت واقعی هر روز از `hr_day_details`)، درخواست‌های من (`hr_vacation` + `hr_vacation_type` + `hr_leave_verify` + `hr_overtime_request` + `hr_telework_request`)، اطلاعات پرسنلی، و «همراهِ روز و تولدها». **پیام روز:** جدول ۳۶۶ پیام یکتا که با `report_dimdate.JYDAY` اندیس می‌شود — هر روزِ سال پیام خودش را دارد و تا پایان سال تکرار نمی‌شود. **گفتگوی تبریک (create-or-join روی ماژول گفتگو، module_id=9):** عنوان قطعی در `chat_dialogs.TOPIC` جست‌وجو می‌شود؛ نبود → `/api/dialog/add` (گروه از `celebration_group_id` در `bot_config` یا `/api/group/get`)، سپس `/api/assign/add` برای پیوستن کاربر. پیام‌ها داخل خود ماژول گفتگو نوشته و از `chat_message` خوانده می‌شوند. schema هر چهار API گفتگو تایید و ثبت شده (`dialog/add`، `group/get`، `group/add`، `assign/add`) — گروه هم با نام قطعی در `chat_group` پیدا یا با `group/add` ساخته می‌شود. پیام تبریک با `/api/message/add` (schema تاییدشده با probe زنده — فیلدهای مسطح `dialog_id/content/type=1/is_public=0/reply_id/user_id/author_id`) مستقیماً از پنل ارسال می‌شود؛ صاحب تولد هم با `assign/add` به گفتگو اضافه می‌شود (`person_profile_id`). ورودی `message` متن دلخواه است، خالی → پیام پیش‌فرض. `celebration_enabled` از 1405/06/07 **پیش‌فرض روشن** است (با `0` در bot_config خاموش می‌شود). **APIهای رسمی منابع انسانی (ماژول ۱۳، هر سه با fallback به دیتابیس و نمایش منبعِ استفاده‌شده):** `leaveTransferGet` برای مانده مرخصی، `orderInDateGet` برای حکم فعال (که کارت «تنظیمات حکم من» را هم می‌سازد)، و `profileSupervisorGet` برای سرپرست مستقیم (فقط در نمای امروز — این API «زمان حاضر» است). **فیش حقوقی (صفحهٔ جدید، 1405/06/09):** فهرست فیش‌های خود کاربر (`hr_payslip` از راه `ORDER_ID`→حکم) + مودال اقلام (`hr_payslip_payment_detail` با `PAYSLIP_VIEW=1` و غیرصفر) — فقط نمایش مقادیر ثبت‌شده، بدون هیچ محاسبه. **وام/مساعده (1405/06/09):** کارت فقط‌خواندنی در تب درخواست‌ها (`hr_loan` + `hr_loan_verify` + نام نوع از `hr_loan_setting`)؛ ثبت وام از پنل هنوز فراهم نیست (schema ناتمام — مرجع API). **لغو درخواست (1405/06/09):** دکمهٔ «لغو درخواست» روی ردیف‌های در انتظارِ خود کاربر — کلاینت‌ساید با هندلر رسمی `GET /hr/vacation/delete` (ماموریت: `/hr/mission/delete`) از راه iframe مخفی (fetch در بعضی محیط‌ها بلاک است). **ثبت درخواست (`type=request_add`، از 1405/06/07؛ ماموریت از 1405/06/09 با `req_city_type`):** فرم «ثبت درخواست جدید» در تب درخواست‌ها — مرخصی ساعتی/روزانه با `/api/hr/vacation_update` (schema کشف و سه‌بار اثبات‌شده؛ مرجع API را ببینید)، تاریخ شمسی متنی → daykey از `report_dimdate`، ساعت واقعی → ثانیهٔ ذخیره‌ای (−۳:۳۰). **محدودیت‌های شناخته‌شده:** ثبت ماموریت (مسیر جدای `/hr/mission`)، فیش حقوقی و برچسب «تردد ناموفق» عمداً پیاده نشده؛ `WORKING_HOURS`/`LEAVE_PER_MONTH` چون واحدشان تایید نشده از UI حذف و فقط در `format=json` باقی مانده‌اند |
| `m_higher_profit_bot.lua` (بات ۲۹۴ «Bot Higher Profit Dealer»، `run/2/m_higher_profit`، cat_id=28 — نامش قدیمی است ولی محتوایش ویجت «تردد دورکاری امروز» است) | Widget (RES via `2/res_bot`) + JSON | `type=3` → JSON ترددهای دورکاری امروزِ کاربر جاری (`hr_ext_time.not_telework=0` روی `EXT_DATE` امروز) + `cur_date` + `last_act` از `admin_view_session`؛ بدون `type` → قالب ویجت با پیوست‌های خود بات (`main.js`/`main.css`/`Persian.js`/`English.js` — روی بات زنده، در ریپو نیستند) | **دیپلوی و تایید شاخهٔ JSON روی دادهٔ زنده 1405/06/08.** اصلاح‌ها: ۱) مقایسهٔ `ctype == 3` با ورودی رشته‌ای هرگز درست نمی‌شد → `tonumber`؛ ۲) `TIME_FROM`/`TIME_TO` خام (پایهٔ UTC، ‏−3:30) با `cur_date` محلی مقایسه می‌شد → آفست `TZ_OFFSET_TICKS` با نگهبان صفر/NULL؛ ۳) SQL پارامتری؛ ۴) `JSON_ARRAYAGG` خالی → `JSON_ARRAY()`؛ ۵) `last_activity` با `MAX` قطعی شد (پایه/واحدش هنوز تاییدنشده)؛ ۶) حذف `write_log` دیباگ؛ ۷) `pcall` + خطای فارسی. شکل خروجی (`data`/`last_act` = ردیف اول fetch) و شناسهٔ قالب `most_higher_dealer_profit_chart` عمداً حفظ شد تا `main.js` نشکند. **هشدار دیپلوی بات‌های ویجتی:** فرم update همهٔ پرچم‌ها را بازنویسی می‌کند — `show_in_widget=1`/`public_access=1`/`show_in_portal_menu=1`/`open_source=1`/آیکون/رنگ زندهٔ بات را عیناً پاس بدهید (پارامترهای جدید `deploy_teamyar_bot.ps1`: `-ShowInWidget -ShowInPortalMenu -PublicAccess -OpenSource -NotShowingInIframe -Icon -Color`). **شاخهٔ ویجت هنوز خراب است به‌دلیل حادثهٔ `2/res_bot`، ردیف بعد را ببینید.** |
| **حادثهٔ `2/res_bot` (بات ۲۵۷، cat_id=80) — قالب مشترک همهٔ ویجت‌های داشبورد، از 1405/06/04 خالی شده** (سورس `src/res_bot_1_bot.lua` = کپی سالم) | RES template (`result_type=2`) | ورودی از `run_command`: `id`, `tpl_name` (`html`/`table`/`chart`), `body`/`generatetd`/`generatedata`, `script`, `css`, `title`, `ajax`, `control`, `path`+`lang` | **~۱۷۰ بات ویجتی** (`COMMAND LIKE '%"2/res_bot"%'` — از جمله ۱۹۹ و ۲۹۴) به آن `run_command` می‌زنند و HTML خروجی‌اش `<script src='/bot/run/2/res_bot/main.js'>` و `xlsx.full.min.js` را hardcode لود می‌کند؛ با خالی‌شدن command (۰ بایت، MODIFY_DATE=1405/06/04، MODIFIER=10053) **و حذف هر ۱۱ پیوستش**، همهٔ این ویجت‌ها شکسته‌اند. ۶ کپی سالم و بایت‌به‌بایت یکسان (MD5 `bb9e3e0c…`): بات‌های 172/174/303/333/350 (`2/res_bot_1..5`) و 393 (`443/res_bot`) — هرکدام با هر ۱۱ پیوست. **بازگردانی command از سمت Claude توسط لایهٔ مجوز مسدود شد** — دستور آماده در دست کاربر است؛ پیوست‌ها هم در `src/res_bot_257_attachments/` دانلود شده‌اند و باید از پنل بات ۲۵۷ آپلود شوند (endpoint آپلود پیوست ناشناخته). فیلدهای زندهٔ ۲۵۷ که هنگام update باید حفظ شوند: `name=res`, `run_path=res_bot`, `result_type=2`, `show_in_widget=1`, `show_in_portal_menu=1`, `public_access=1`, `open_source=1`, `not_showing_in_iframe=0`, `icon=tyf-module1`, `color=#5e35b1`, `cat=80_1` |
