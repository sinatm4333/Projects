...
function getCallsInfo(data)
    ...
    box_clause = [[  CallerProfileID = ]]..users;
    ...
    if(data.date_from) then
        if (string.len(data.date_from) >= 1 and string.len(data.date_to) >= 1) then
            ...
            box_clause = box_clause.. [[ (  DATE BETWEEN ]]..date_from..[[  AND  ]]..date_to..[[ ) ]];
        end
    end
    ...
    local param = {
        query=query,
        params={input.date_from,input.date_to} 
    } 
    db.query(param) 
    local result={} ; 
    local record={}; 
    while db.query_fetch(record) do 
        table.insert(result,{total=record[1],sum=record[2],avg=record[3],answer=record[4]}) 
    end 
    db.query_free();
...
local template = teamyar.run_command("2/res_bot",{
    ...
    generatedata=[[(data)=>{
    return    ty__main.createDataForChartOutbound(data);
  }]],
