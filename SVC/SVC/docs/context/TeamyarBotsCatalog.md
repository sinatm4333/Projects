...
منبع: فایل‌های `.tybot` که کاربر مستقیماً از پنل Teamyar export و ارسال کرد (همان الگوی بخش HR بالا — نه سینک زنده، در `TeamyarBotsLiveRegistry.json` ثبت نشده‌اند). سورس کامل دیسک ندارد؛ منبع صادق همان فایل‌های `command_<id>.tybot` کاربر است. همه روی `voip_calls` (+ `UNION` با `` `0000000_archive`.voip_calls `` در اغلب‌شان — وجودش هنوز با پروب تایید نشده) و `CallerProfileID`/`ConnectedLineProfileID` → `profile_main.FULLNAME` کار می‌کنند. `DialStatus`: 5=پاسخ، 3=بی‌پاسخ، 4=مشغول، 6=قطع (فرض همهٔ ۹ بات، هنوز با دادهٔ زنده تایید نشده). **هشدار ناسازگاری:** بات ۲۶۴ و ۲۶۲ قرارداد Inbound/Outbound متفاوتی دارند (نگاه کنید به کامنت‌های `call_center_admin_dashboard_probe_json_bot.lua`) — قبل از استفاده از این بات‌ها به‌عنوان مرجع inbound/outbound، نتیجهٔ آن پروب را بررسی کنید.
