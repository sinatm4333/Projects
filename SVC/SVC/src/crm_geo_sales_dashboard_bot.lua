...
--      docs/context/SalesReportBotsReference.md).
--   ۴) کارت‌های KPI و کیفیت داده بالای صفحه Clickable شدند (Drill-down به بخش مرتبط).
--   ۵) طبق درخواست صریح کاربر (مشابه محاسبهٔ Monetary در بات ۶۰۰ که با فیلتر تاریخ است): تمام
--      Drill-down های Lazy (مشتریان یک شهر، فاکتورهای یک مشتری، فاکتورهای بدون نگاشت جغرافیایی) اکنون
--      با همان بازهٔ تاریخ فعال Dashboard (پیش‌فرض سال مالی جاری) محاسبه می‌شوند، نه کل تاریخچه —
--      ر.ک. resolve_active_date_from_key و getActiveDateFrom(). پیش از این، «مبلغ کل خرید» نمایش‌داده‌شده
--      در فهرست مشتریان هر شهر کل تاریخچه بود، نه فقط سال مالی جاری — ناسازگار با KPIهای بالای صفحه.
