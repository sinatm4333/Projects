...
| `cash_flow_treasury_dashboard_report_bot.lua` (جدید v01، هنوز مستقر نشده) | HTML | `org_id` (پیش‌فرض ۸ — Dropdown از `org_info`، دادهٔ چک/درخواست فعلاً عملاً فقط زیر سازمان ۸ است) | داشبورد مدیریتی Cash Flow/خزانه‌داری — KPI چک صادره/دریافتی در جریان، چک سررسیدشده/گذشته، درخواست خزانه‌داری تایید/پرداخت‌شده و در انتظار تایید، نیاز نقدینگی ۷ روز آینده، موجودی نقد و بانک، Cash Gap؛ نمودار Stacked Bar روزانه (۷ روز)، Donut ترکیب تعهدات، روند ۳۰ روز اخیر، میله‌ای چک به تفکیک واحد؛ ۴ جدول Drill-down (سررسیدهای ۷ روز آینده، چک صادرشده، چک دریافتی، درخواست خزانه‌داری). **Schema تأییدشده با چند دور Query Discovery زنده روی دادهٔ واقعی (نه حدس) + تأیید صریح کاربر برای کدهای STATUS/TYPE بدون Lookup:** `pa_pdc` (TYPE=1 صادره/TYPE=2 دریافتی؛ STATUS=1=در جریان؛ **تاریخ صدور=`EXPORT_DATE`، سررسید=`DATE_ISSUE`** — نام‌گذاری گمراه‌کننده، با Join زنده به `report_dimdate` تأیید شد؛ واحد=`unit_id`→`pa_pdc_units`)، `pa_request`+`pa_request_record`+`pa_request_record_det` (STATUS: 0=پیش‌نویس/1=در انتظار تایید/2=ردشده/3=تایید‌وپرداخت‌شده یک‌جا — قابل‌تفکیک نیست، KPI «تاییدشده ولی پرداخت‌نشده» به‌جایش روی STATUS=1 صادقانه لیبل شد؛ ذی‌نفع=`COALESCE(pa_client.NAME, DESCRIPTION)`؛ مرکز هزینه چون فقط ۸.۶٪ پوشش داشت از جدول حذف شد؛ «تاریخ مورد نیاز» = همان `REQUEST_DATE`)، موجودی بانک/صندوق از `pa_bank_account`/`pa_cashdesk`→`ACCOUNT_ID`→`pa_account.BALANCE` (موجودی بانک یک مانده تجمیعی حساب معین مشترک است، نه per-account). Join تاریخ همه‌جا `BETWEEN DATEKEY AND DATEKEY+863999999999` است نه `=` (ستون‌های تاریخ pa_pdc/pa_request جزء ساعت دارند — تست زنده اثبات کرد `=` همیشه NULL می‌دهد). سازمان از `org_info` (همان جدول ACL سازمان بات RFM ۵۰۱) خوانده می‌شود اما بدون بوت‌استرپ کامل `res_v2`/`readyCodes` (هیچ تابع کمکی RES این‌جا مصرف نمی‌شود؛ رندر کاملاً HTML/CSS/JS اختصاصی خودِ بات است، نه `install_res.resReport` که اجازهٔ KPI/نمودار/لوگو الزامی را نمی‌دهد). لوگو ۱۴۰ (White) + فونت Peyda + پالت #16509D طبق CLAUDE.md. **محدودیت شناخته‌شدهٔ v01:** معنای کدهای STATUS چک به‌جز ۱ در DB Lookup ندارد (خام «کد وضعیت: N» نمایش داده می‌شود)؛ «دستهٔ درخواست» (`pa_request_record_det.TYPE`) فقط کد ۲=«حسابداری» تأیید شد. هنوز روی Teamyar دیپلوی/تست زنده نشده. |
