...
| 620 | تسویه خودکار — نسخهٔ بک‌فیل انبوه (کپی بات ۵۸۲، ۱۴۰۵/۰۶/۰۵). برای پاکسازی یک‌بارهٔ صف بزرگ (مثال کاربر: ~۴۰۰٬۰۰۰ فاکتور، سال مالی ۱۴۰۴) که چک‌باکس/UI (بات ۶۱۲) برایش عملی نیست. فیلتر/کوئری عیناً از ۵۸۲ (بازه از `pa_fiscal_year` برای سال `_TARGET_FISCAL_YEAR="1404"`، نه `day_befor`)؛ تفاوت اصلی: حلقهٔ تسویه **زمان‌محور** است — تا `_MAX_RUNTIME_SECONDS` (پیش‌فرض ۱۶۲۰۰ ثانیه؛ باید کمی زیر `max_execute_time` واقعی بات روی پنل باشد — کاربر خودش ۱۸۰۰۰ ثانیه ست کرده) پردازش می‌کند و تمیز متوقف می‌شود، به‌جای پردازش نامحدود همه‌چیز در یک اجرا. چون فاکتور تسویه‌شده خودش از شرط کوئری خارج می‌شود، هر اجرای بعدی خودکار از همان‌جا ادامه می‌دهد — بدون checkpoint دستی؛ باید تا خالی‌شدن صف چندبار دستی اجرا (یا زمان‌بندی) شود. رفع باگ همراه (از ۵۸۲): `update_invoice_history` فقط روی تسویهٔ موفق صدا زده می‌شود + `pcall` روی هر دو فراخوانی API. **بدون UI/RES — `readyCodes()`/`get_attachment` صدا زده نمی‌شود؛ ۴ فایلی که کاربر به این بات پیوست کرده (`data.txt/js/css`, `mySqlQuery.txt`) بی‌اثرند، کد هرگز نمی‌خواندشان.** (۱۴۰۵/۰۶/۰۶) `i.deleted = 0` به هر دو کوئری (SELECT اصلی + `q_count`) اضافه شد — عیناً از کوئری بات ۶۱۲ (`mySqlQuery.txt`)، چون بدون آن فاکتورهایی می‌آمدند که سند مربوطه‌شان امضا شده بود و API با «سند مربوطه امضا شده است...» ردشان می‌کرد. کاربر گزارش داد بعد از این فیکس هنوز خطا تکرار می‌شود — یعنی `deleted=0` به‌تنهایی علت واقعی نبود (یا فقط بخشی از آن بود)؛ ریشه‌ی دقیق هنوز تأیید نشده، چون دسترسی مستقیم به DB زنده/لاگ اجرا برای تشخیص لازم است. به‌جای حدس زدن روی فیلتر کوئری، `settleFactor` حالا دلیل خطا را هم برمی‌گرداند: در خلاصه‌ی پایان اجرا، شکست‌ها به «سند امضا شده» (دائمی — این فاکتورها هیچ‌وقت در `sales_invoice_settlement` ثبت نمی‌شوند و هر اجرا دوباره در صف ظاهر/رد می‌شوند) در برابر «خطای دیگر» تفکیک می‌شوند، با نمونه شناسه/پیام خطا — تا اجرای بعد بدون نیاز به لاگ خام مشخص شود دقیقاً کدام فاکتورها و با چه پیامی رد می‌شوند. کاربر گزارش داد این هم کافی نیست چون فاکتورهای امضاشده همچنان هر اجرا دوباره برمی‌گشتند و تسویه نمی‌شدند — ریشه‌ی واقعی: چون هیچ‌وقت در `sales_invoice_settlement` ثبت نمی‌شوند، هیچ‌چیزی جلوی برگشتنشان به کوئری صف را نمی‌گرفت. رفع شد با جدول اختصاصی جدید `sales_settlement_permanent_skip` (ORG_ID, INVOICE_ID, REASON, DATE_CREATE؛ `CREATE TABLE IF NOT EXISTS` خودکار در ابتدای هر اجرا، `ensurePermanentSkipTable`) — هر فاکتوری که `settleFactor` با خطای «امضا» رد کند در این جدول ثبت می‌شود (`recordPermanentSkip`) و هر دو کوئری صف (SELECT اصلی + `q_count`) با `NOT IN` این جدول را کنار می‌گذارند، پس دیگر برای همیشه در صف نمی‌ماند. **Fail-safe:** اگر جدول در دسترس نباشد، شرط `NOT IN` اصلاً به کوئری اضافه نمی‌شود (نه اینکه کل بات خطا بدهد) — بات به رفتار قبلی (بدون این فیلتر) برمی‌گردد و در لاگ/خلاصه‌ی نتیجه صریح اعلام می‌کند. **تأیید شده روی داده‌ی زنده (۱۴۰۵/۰۶/۰۶، تاریخچه‌ی اجرا):** `db.query` این پلتفرم با DDL (`CREATE TABLE`) «sql error» عمومی می‌دهد — دقیقاً همان الگوی خطای مبهم که برای `LIKE ?` هم دیده شده بود؛ به نظر می‌رسد کل مسیر DDL از `db.query` مسدود است (نه فقط محدودیت دسترسی این بات). fail-safe درست کار کرد (بات کرش نکرد، فقط فیلتر غیرفعال ماند). به همین دلیل تابع از `ensurePermanentSkipTable` (که `CREATE TABLE` می‌زد) به `checkPermanentSkipTableExists` (فقط یک `SELECT 1 ... LIMIT 1` سبک probe، بدون هیچ تلاش DDL) تغییر کرد — جدول باید **یک‌بار دستی، خارج از بات** (توسط ادمین DB، نه از طریق db.query/schema_probe_v2) ساخته شود؛ DDL دقیق در کامنت بالای `checkPermanentSkipTableExists` در سورس نگه داشته شده. هنوز تأیید نشده که `INSERT` پارامتری (`recordPermanentSkip`) روی این پلتفرم کار می‌کند یا نه — بعد از ساخت دستی جدول و برخورد بات به یک فاکتور امضاشده، لاگ اجرای بعدی باید این را روشن کند. دیپلوی شده، در حال تست. | `sales_settlement_backfill_queue_bot.lua` | `/bot/run/443/sales_settlement_backfill_queue` (فرضی — تأیید نشده) |
