...
--   این بات برای id=612 است (کپی بات 569 «Factor Seteelment By Selection» /
--   بات 588 «تسویه اتوماتیک گروهی» — نه کپی بات 582 که یک بات کاملاً جدا و بدون UI است).
--   منطق سرور (این فایل + data.txt + mySqlQuery.txt) عیناً از export زنده‌ی بات 569 گرفته
--   شده و دست‌نخورده است. تنها تغییر نسبت به 569/588 در پیوست data.js/data.css است:
--   «تسویه گروهی» به‌جای ارسال کل انتخاب در یک درخواست، صف انتخاب (که حالا بین صفحات
--   هم حفظ می‌شود) را به دسته‌های ۲۰تایی می‌شکند و پشت‌سرهم به همین endpoint
--   (type=201) می‌فرستد — چون endpoint از قبل idsCsv با هر طولی را می‌پذیرد، سمت سرور
--   نیازی به تغییر نداشت. جزئیات در data.js.
--------------------------------------------
--- CONFIG DATA
--------------------------------------------
local _PAR_PAGE = 200
...
function getTableSectionOne()
  ...
  local toolbar = '<div class="alert alert-info" style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">'
    ...
    .. '<span>برای تسویه هر فاکتور از دکمه «تسویه» استفاده کنید یا چند مورد را انتخاب و «تسویه گروهی» بزنید. انتخاب بین صفحات مختلف حفظ می‌شود و بیش از ۲۰ مورد به‌صورت صف/دسته‌ای پردازش می‌شود.</span>'
    .. '</div>';
