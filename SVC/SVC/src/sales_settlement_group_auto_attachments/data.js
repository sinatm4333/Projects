...
// تسویه تکی/گروهی یا سایر منطق بالا حذف/تغییر نکرده، فقط بعد از رندر شدن جدول در
// DOM روی همان جدول رندرشده کار می‌کند). جدول واقعی توسط ویجت هسته‌ای پلتفرم
// $.Teamyar.table() رندر می‌شود که نه سورسش در دسترس ماست و نه مرتب‌سازی/فیلتر
// دارد — این یک لایهٔ کلاینت‌ساید مستقل روی خروجی رندرشده است. چون جدول با هر
// صفحه‌بندی از نو در DOM رندر می‌شود، کلیک مرتب‌سازی با event delegation روی
// ریشهٔ بات پیاده شده — نیازی به bind دوباره بعد از هر رندر نیست.
// -------------------------------------------------------------------------
(function () {
    var BOT_NAME = "sales_settlement_group_auto";
    var ROOT_SELECTOR = 'section[data-name="' + BOT_NAME + '"]';
    var FILTER_INPUT_ID = "ssga-filter-input";
    ...
    function injectFilterInput() {
        ...
        input.className = "ssga-filter-input";
