...
<h3 style="background-color:#00bf7d;text-align: center;border:1px dots black;height:40px;weight:100%%;padding-top:5px">فروش 7 روز گذشته</h3>
<table>
  <thead>
...
input = [[ {"domain":"78.135.73.244","port":8001,"method":"GET","url":"/api/karbalad/lastsevendayssalesbyamount","ssl":true,"secure":false}]]
...
local status = res["result"]["status"]
 ...
 --teamyar.write_result(context,body)
   ...
   for i=1 , #JD, 1 do
        table.insert(myArray, {JD[i]["shamsiSalesYear"],JD[i]["salesStatus"],JD[i]["sum"]}) 
     end
