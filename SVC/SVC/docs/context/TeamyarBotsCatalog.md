...
| 598 (cat_id=80) | صفحه اصلی مودیان[Module] — بازطراحی کامل بات ۵۷۸ (v5، ۱۴۰۵/۰۵/۲۷). **دیگر از get_config/get_attachment/پیوست دستی استفاده نمی‌کند** — بعد از این‌که بارها تأیید شد «پیکربندی» بات هرگز مقدار واقعی ذخیره‌شده ندارد (چه ۵۹۵ چه ۵۹۸) و همین باعث کرش HTTP 500 (روی `count_day` خالی) و عدم بارگذاری تب «ارسالی» می‌شد، کوئری کاملاً خودکفا در Lua جاسازی شد؛ ورودی org_id (پیش‌فرض ۸) / from_date / to_date (شمسی، پیش‌فرض ۳۶۵ روز اخیر برای جدول‌ها) / limit (پیش‌فرض ۵۰۰). کوئری بر مبنای پیوست `query_list_invoice.txt` **بات مرجع فعال ۵۷۴** («ارسال گروهی به سامانه مودیان[Module]») بازسازی شد — نه اتچمنت‌های قدیمی بات ۵۷۸ — با دو اصلاح واقعی که از مقایسه با ۵۷۴ به دست آمد: فیلتر `i.type IN (1,3)` (در اتچمنت‌های ۵۷۸ نبود) و نگاشت وضعیت کامل‌تر شامل کدهای ۱۱۹/۱۲۰/۲۱۹/۲۲۰/۳۱۹/۳۲۰/۴۰۰ (کران بالای هر بازه هم اکنون شامل خود عدد است: `<=120/220/320` نه `<120/220/320`). ظاهر: پالت #16509D + فونت Peyda **واقعاً self-contained** (base64، دو @font-face 400/700 — طبق v5 دیگر placeholder جای‌گذارنده نیست)، جعبه جستجوی آنی هر تب، کارت‌های KPI (تعداد ارسالی/ابطالی/اصلاحی **محدود به سال مالی جاری** طبق درخواست کاربر — همان الگوی `resolve_fiscal_year_start`/`report_dimdate` در crm_geo_sales_dashboard_bot.lua، با آیکون ⓘ توضیح‌دهنده کنار عنوان کارت‌ها؛ جدول‌های زیر کارت‌ها هم‌چنان بازهٔ ۳۶۵ روزهٔ خودشان را دارند)، هدر مرتب‌سازی‌پذیر، Excel فقط روی ردیف‌های فیلترشده. escape_html به فرم متعارف concatenation اصلاح شد (طبق قانون امنیتی CLAUDE.md). AJAX هر تب (type=1/2/3/4): فقط fetch خام، **بدون مهلت (timeout) مصنوعی** — مهلت ۲۰ ثانیه‌ای نسخهٔ قبلی حذف شد چون خودش باعث «لود نشدن» تب «ارسالی» (~75هزار ردیف) می‌شد، نه خطای واقعی سرور. تأییدشده روی داده زنده (پیش از v5): ارسالی=74,238، ابطالی=2,449، اصلاحی=۰ | `moadian_factor_tabs_report_bot.lua` | `/bot/run/2/moadian_index_m_1` |
