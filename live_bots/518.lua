...
local query = [[      select count(distinct t.id) c,(select fullname from profile_main where id=mr.manager)manager,mr.unit,
                            (select sex from profile_user_info where id=mr.manager)is_male
                            from todo_task t inner join todo_task_steps ts on ts.task_id=t.id inner join todo_step s on s.id=ts.STEP_ID
                            inner join todo_workflow wf on wf.id=t.WORK_FLOW_ID inner join pers_manager mr on mr.pers=ts.RESPONSIBLE_ID  where t.STATUS=1  and ts.FLAG_LAST_STEP=1 and ts.status=0
                           ]]
  --where 1
...
--  if dated ~= nil and json.encode(dated) ~= "" then 
 ...
 -- end 
     query = query.. [[ group by  mr.manager,mr.unit order by mr.unit ]]
-------------
  local rep_data ={}
