...
local function make_action_type_key(workflow_id, topic_id)
    local normalized_workflow_id = tonumber(workflow_id) or workflow_id
    local normalized_topic_id = tonumber(topic_id) or 0
    return tostring(normalized_workflow_id) .. ":" .. tostring(normalized_topic_id)
end
...
local function parse_action_type_keys(source)
    ...
    local function append_key(value, workflow_only)
        local text = tostring(value or "")
        local workflow_id, topic_id = text:match("^(%d+):(%d+)$")
        if workflow_only then
            workflow_id = text:match("^(%d+)$")
            topic_id = "0"
        end
        if workflow_id == nil then
            return
        end
        local key = make_action_type_key(workflow_id, topic_id)
    ...
    local function collect(raw, workflow_only)
        if type(raw) == "table" then
            for _, value in pairs(raw) do
                append_key(value, workflow_only)
            end
        elseif type(raw) == "string" then
            for value in raw:gmatch("[^,;]+") do
                append_key(value:match("^%s*(.-)%s*$"), workflow_only)
            end
        elseif raw ~= nil then
            append_key(raw, workflow_only)
        end
    end

    collect(source["action_type_ids"] or source["actionTypeIds"], false)
    collect(source["workflow_ids"] or source["workflowIds"], true)
    collect(source["workflow_id"] or source["workflowId"], true)
    table.sort(keys)
    return keys
end
...
SELECT
    ...
    ]] .. fmt_jalali_datetime("MAX(bh.DATE_CREATE)") .. [[ AS last_run,
    ]] .. fmt_jalali_datetime("bc.MODIFY_DATE") .. [[ AS modify_date
FROM bot_command bc
