...
**Client-side sort/filter — achievable, without touching core (added 1405/05/26):** `$.Teamyar.table()` still emits a plain `<table><thead><tr><th>` / `<tbody><tr><td>` structure into the DOM — a per-bot `data.js` can layer independent click-to-sort (event delegation on the bot's root `section[data-name=...]`, so it survives the table being re-rendered on each page/filter change — no rebind needed) and a text filter input (hides/shows rendered `<tbody>` rows) on top of that output, exactly like the `initSortableTables()` pattern used in the self-contained (non-RES) HTML bots. This is a DOM-layer addition, not a change to the widget's own source — safe, done for `crm_rfm_1` (`scratchpad/data_css_bot600.css`/`data_js_bot600.js`), `set_by_select_1` (`src/set_by_select_1_attachments/`) and `sales_settlement_group_auto` (`src/sales_settlement_group_auto_attachments/`). **Caveat:** RES paginates server-side (`getData(_TYPE_PAGE_REPORT, pageFrom, perPage)`), so this only sorts/filters the rows currently rendered on the page you're viewing, not the bot's full server-side result set — a true global sort/filter would require changing `getData()`/the SQL (out of scope here, that's bot logic).
