...
| `sales_settlement_group_auto_report_bot.lua` (bot 588, cat_id=79) | HTML | RES-framework (`install_res`/`res_v2`), no `bot_customform` | **کپی بات ۵۶۹** («تسویه گروهی» / `Factor Seteelment By Selection`) با نام «تسویه اتوماتیک گروهی». فهرست فاکتورهای قابل‌تسویه + تسویه تکی/گروهی واقعی روی فاکتورها (`/api/sales/create_settlement`) — بات مالی نویسنده، نه صرفاً گزارشی. وابسته به ۴ پیوست (`src/sales_settlement_group_auto_attachments/`: `mySqlQuery.txt`, `data.txt`, `data.js`, `data.css`) که باید دستی از پنل بات آپلود شوند (endpoint آپلود attachment ناشناخته) + یک نمونه‌ی «پیکربندی» (کد حساب تسویه=۱۰۱۰۰۱۰۰۴، نوع تسویه=نقد، کپی از «تسویه پیش‌فرض» بات ۵۶۹) که باید از تب «پیکربندی» بات ساخته شود. **ظاهر (اصلاح‌شده ۱۴۰۵/۰۵/۲۶):** `data.css` قبلی غیرمنطبق با گایدلاین بود — فونت «Zar» لینک‌شده از مسیر یک بات دیگر (`bot/run/2/motor_doc_print/zar.*`، هرگز روی این بات آپلود نشده)، بدون پالت #16509D، و چند کلاس کارتی بی‌اسکوپ (`holder_box1`/`each_holder1`/`cs_btn`/`ths`/`tds`/...) که هیچ‌کدام در Lua این بات استفاده نمی‌شوند (باقیماندهٔ کپی/پیست از بات ۵۶۹ برای یک UI نامرتبط). با نسخهٔ منطبق جایگزین شد: فونت Peyda (base64 embedded) + پالت #16509D + اندازهٔ فونت‌ها + اسکوپ‌شده زیر `section[data-name="sales_settlement_group_auto"]` (طبق `run_slug` واقعی بات در `TeamyarBotsLiveRegistry.json`) + دکمهٔ راهنما (همان الگوی `set_by_select_1`). **مرتب‌سازی/فیلتر:** مثل بات ۶۰۰ — لایهٔ کلاینت‌ساید در `data.js` (کلیک سرستون برای مرتب‌سازی صعودی/نزولی + جعبهٔ جستجوی متنی)، فقط روی صفحهٔ رندرشدهٔ جاری، بدون دست‌بردن در `getFilters`/`settleSingle`/`settleSelected`/`toggleAll`/`sendSettleRequest`. هنوز هم مثل قبل، پیوست‌ها دستی آپلود نشده‌اند. |
...
| 569 (cat_id=79) | Factor Seteelment By Selection (تسویه گروهی، منبع اصلی؛ ظاهر `src/set_by_select_1_attachments/` — ۱۴۰۵/۰۵/۲۶: مرتب‌سازی کلیکی سرستون + جعبهٔ جستجو/فیلتر متنی به data.css/data.js اضافه شد) | `set_by_select_bot.lua` | `/bot/run/2/set_by_select` |
