...
| `action_module_report_bot.lua` (**ماژول اقدام** — بات **۶۳۳**، `443/action_module`، cat_id=79، result_type=1) | HTML/JSON | `action=list|summary|create|whoami|state_probe` (از طریق `customform` JSON)، `scope=all|responsible|author|member`، `state=all|open|running|overdue|done`، `search`، `section_id`/`category_id`/`topic_id`/`workflow_id`، `from_date`/`to_date` (شمسی)، `page`/`page_size`، `user_id` (اختیاری — fallback شناسایی کاربر) | بازطراحی ماژول «اقدام» (BPMN) به‌صورت یک بات HTML یکپارچه با ساید‌بار داخلی: داشبورد شخصیِ کاربرِ اجراکننده (KPI + اقدام‌های منتظر او)، فهرست با تب‌های ماژول بومی (همه/مسئولم/مؤلف-مالکم/عضوم)، نوار وضعیت (باز/در جریان/معوق/کامل‌شده)، فیلترهای بخش/رده/موضوع/جریان‌کار/بازهٔ تاریخ، و **فرم ایجاد اقدام واقعی** از طریق `teamyar.call_api(8, "/api/todo/taskadd")` با اعتبارسنجی هم‌رده‌بودن جریان‌کار و موضوع (قانون سرور) قبل از فراخوانی. دامنهٔ «عضویت» طبق تصمیم کاربر: مسئولِ مرحلهٔ باز فعلی + `todo_notify` + `todo_task_comments`. **ملاحظات ماژول‌ساز** (از بات ۵۱۴ «ماژول ساز اقدام[module]» و بات ۵۷۸ استخراج شد): `RUN_URL` با fallback چون داخل ماژول `location.pathname` مسیر بات نیست؛ پارامترها با کلید `customform` (JSON) POST می‌شوند؛ همهٔ CSS/JS خودکفا بدون attachment؛ تمام‌صفحه با toggle کلاس CSS نه Fullscreen API. `LIKE ?` عمداً استفاده نشده — جستجو با `LOCATE(?, col) > 0` (هم پارامتری هم مصون از باگ زندهٔ LIKE). **فرضیهٔ ۴۰۴ ماژول‌ساز (تاییدنشده):** همهٔ بات‌های `[Module]` که واقعاً در ماژول‌ساز باز می‌شوند در namespace `2/...` هستند (۵۱۴، ۵۷۲، ۵۷۴، ۵۷۸، ۵۹۸) در حالی که داشبوردهای این ریپو زیر `443/...` مستقر شده‌اند و ۴۰۴ می‌دهند؛ عامل دوم محتمل: `public_access=0` که اسکریپت دیپلوی هاردکد می‌فرستد. با یک کوئری روی `bot_command` (ستون‌های `run_path`/`folder_id`/`public_access`/`not_showing_in_iframe`) قابل تعیین‌تکلیف است. **رفع باگ ۱۴۰۵/۰۶/۰۹ (گزارش کاربر: «با چند کلیک روی منوی کناری HTML می‌پرد»):** با تست واقعی در Chromium (همان HTML بات، کلیک تک‌تک آیتم‌های منو، هر دو مسیر `fetch` و `$.Teamyar.ajax`) ثابت شد سمت کلاینت صفر خطا دارد — مشکل از پاسخ سرور بود: وقتی `action` به Lua نمی‌رسید، `main()` به شاخهٔ رندر صفحهٔ کامل می‌افتاد و به‌ازای هر کلیک ~۲۴۰KB HTML به‌عنوان پاسخ AJAX برمی‌گشت. علت محتمل: بات با `bot_customform` خالی (`"info":[]`) دیپلوی شده و پلتفرم فیلدهای declare‌نشده را بی‌صدا حذف می‌کند (همان پیتفال مستند `setListInputs`). سه لایه اصلاح شد: ۱) `action` همزمان از سه مسیر فرستاده/خوانده می‌شود (`customform`، فیلد مستقیم، و query string `am_action`)؛ ۲) گارد `looks_like_ajax` — درخواست AJAX هرگز HTML کامل نمی‌گیرد، به‌جایش JSON خطا همراه فهرست کلیدهای واقعاً دریافت‌شده (`received_keys`) برمی‌گردد تا رفتار پلتفرم مشخص شود؛ ۳) سمت کلاینت پاسخ غیرJSON هرگز داخل صفحه تزریق نمی‌شود. حین همین اصلاح یک باگ واقعی Lua هم گرفته شد: `ipairs` روی جدول کاندیداها با `nil` در خانهٔ اول بلافاصله متوقف می‌شد و مسیر `am_action` هرگز خوانده نمی‌شد. **تاییدنشده روی دادهٔ زنده:** کلید شناسهٔ کاربر در `teamyar.get_user_info()` (با `action=whoami` مشخص می‌شود) و نگاشت نوار مرحله‌ای بومی پیش‌نویس/بررسی/اجرا/کامل (با `action=state_probe`) — هیچ‌کدام حدس زده نشده |
