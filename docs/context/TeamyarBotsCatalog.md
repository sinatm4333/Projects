...
| `document_content_access_probe_json_bot.lua` (جدید، هنوز مستقر نشده — بات **تشخیصی موقت**) | JSON | `document_id` | پیش‌نیاز داشبورد چندفایلی CSV از روی «شناسه سند» ماژول اسناد (مثال زنده: id=۱۹۰۲۴/۱۹۰۲۵ در «اسناد / واحد مالی / فایل مشترک موبایل ۱۴۰ / مودیان»). `documents_main` فقط متادیتا دارد (نه محتوای باینری)؛ محتوای واقعی فایل روی دیسک/استوریج خارج از دیتابیس است و مسیرش با `documents_storage.LOCATION` (فقط ۲ ردیف در کل دیتابیس، ستون‌های `PARENT_ID`/`STORAGE_TYPE`/`LOCATION`) + `documents_main.ENCODING_KEY` ساخته می‌شود. این بات: متادیتای کامل سند، تمام ردیف‌های `documents_storage`، زنجیرهٔ `PARENT_ID`/`ROOT_FOLDER_ID` سند تا رسیدن به یک ریشهٔ storage، وجود سراسری `io`/`os` در Sandbox، و (در صورت وجود `io`) نتیجهٔ `io.open` روی چند الگوی مسیر محتمل (`LOCATION`+`ENCODING_KEY`) را برمی‌گرداند — فقط طول فایل + ۲۰۰ بایت اول، نه کل محتوا. **باید روی سرور واقعی توسط کاربر اجرا شود** (این محیط توسعه به `erp.bimehland.com` دسترسی شبکه ندارد) — خروجی JSON آن مسیر واقعی خواندن محتوای CSV در بات نهایی را تعیین می‌کند. |
