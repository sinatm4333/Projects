...
- **Mobile-responsive output — mandatory for all `result_type=HTML` bots (added 1405/05/26).** Reports must render usably on narrow (mobile) viewports, not only desktop:
  - Add `<meta name="viewport" content="width=device-width, initial-scale=1">` in the HTML head.
  - Wrap the report table in a horizontally scrollable container (e.g. `.table-wrap { overflow-x: auto; -webkit-overflow-scrolling: touch; }`). This is the mandatory mobile pattern for tables — it preserves the `white-space: nowrap` / `table { width: auto }` / `td,th { width: 1% }` rules above unchanged. **Never** add cell wrapping or column shrinking to "fix" mobile — scroll the container instead, do not touch the table's own no-wrap/auto-width rules.
  - Toolbar, summary strip, and header must use `flex-wrap: wrap` so buttons/labels stack instead of overflowing on narrow screens.
  - Containers outside the table use relative/fluid sizing (`%`, `max-width: 100%`) — never a fixed `px` page width.
  - Font sizes stay exactly as defined above (14px body / 15px titles) — never shrink fonts to fit mobile.
  - Sanity-check at common phone widths (360–414px) before considering an HTML bot done.
