...
| `/api/hr/profileSupervisorGet` | دریافت سرپرست تعیین‌شده در حکم فعال کاربر در شعبهٔ مربوطه | ۱۰۰٪ | ۱۴۰۴/۱۲/۰۴ | **✅ schema ثبت شد — پایین را ببینید** |
...
## ماژول «گفتگو» (Chat) — module_id `9`

schema چهار endpoint زیر از پورتال گرفته شد (۱۴۰۵/۰۶/۰۷، کاربر). این‌ها همان چیزی هستند که
«گفتگوی تبریک تولد» در `hr_companion_report_bot.lua` با آن‌ها ساخته می‌شود.

### `/api/dialog/add` — ساخت گفتگو

**Request:** `{"topic":"","group_id":0,"author_id":0,"show_in_portal":0}`
**Response:** `{"data":{"dialog_id":0},"error":{"status":0,"message":""},"success":0}`

⚠️ فیلد `type` و `status` در این API **وجود ندارند** (قبل از گرفتن schema حدس زده شده بودند و
اشتباه بودند). نوع گفتگو از روی گروه تعیین می‌شود، نه از payload.

### `/api/group/get` — خواندن یک گروه گفتگو

**Request:** `{"id":0}`
**Response:** `{"data":{"id":0,"name":"","status":0,"topics":[{"id":0,"title":"","password":"","channel_id":0,"channel_topic":""}],"keywords":[""],"public_name":""},"error":{…},"success":0}`

⚠️ **این API فهرست گروه‌ها را نمی‌دهد** — یک گروهِ مشخص را با شناسه می‌خواند. پس با آن نمی‌شود
گروه را «کشف» کرد. برای پیدا کردن گروه باید یا شناسه‌اش را از قبل داشت، یا در جدول `chat_group`
با نام جست‌وجو کرد، یا با `/api/group/add` ساختش.

### `/api/group/add` — ساخت گروه گفتگو
...
**Response:** `{"error":{"status":0,"message":""},"success":0}` — بدون `data`

⚠️ `assigned` آرایه است. فیلدهای `user_id` / `user_ids` در این API **وجود ندارند**.
این با `/api/client/assign/add` ماژول CRM (۱۴، «مطلع» مشتری) کاملاً فرق دارد.

**جریان کار پیاده‌شده در `hr_companion_report_bot.lua`:** نام گروه و عنوان گفتگو هر دو قطعی‌اند؛
گروه اول در `chat_group` با نام جست‌وجو می‌شود و فقط اگر نبود با `group/add` ساخته می‌شود، بعد
گفتگو در `chat_dialogs.TOPIC` جست‌وجو و فقط اگر نبود با `dialog/add` ساخته می‌شود، و در آخر کاربر
با `assign/add` به همان گفتگو می‌پیوندد. یعنی به‌ازای هر تولد فقط یک گفتگو ساخته می‌شود و بقیه به
همان می‌پیوندند.

### آدرس ماژول گفتگو در رابط کاربری (۱۴۰۵/۰۶/۰۷)

`/?page=/chat/index`

لینک کاملی که کاربر از پورتال داد پارامترهای `widget_show_header_checkbox`، `widget_header_color`،
`widget_show_background`، `widget_show_shadow`، `widget_title_color` و `widget_box_id` هم داشت؛
این‌ها **تنظیمات ظاهری ویجت‌اند و به گفتگو ربطی ندارند**، پس در بات نیامده‌اند. آدرس به‌صورت
**نسبی** استفاده می‌شود تا دامنه جایی hardcode نشود و روی هر میزبانی که بات از آن سرو می‌شود کار
کند (قانون دامنهٔ `erp.bimehland.com` هم به همین شکل رعایت می‌شود).

⚠️ **deep link به یک گفتگوی مشخص وجود ندارد.** طبق تایید کاربر، گفتگوها در این ماژول به‌صورت
**پاپ‌آپ** باز می‌شوند، نه با URL جدا. پس بیشترین کاری که می‌شود کرد باز کردن خودِ ماژول است.
`hr_companion_report_bot.lua` همین کار را می‌کند و در کنارش **عنوان قطعی گفتگو** را به کاربر نشان
می‌دهد («تبریک تولد ‹نام› — ‹تاریخ›») تا در فهرست پیدایش کند. آدرس از `bot_config.chat_module_url`
قابل تغییر است.
...
### `/api/hr/profileSupervisorGet` — دریافت سرپرست کاربر در یک شعبه

schema از پورتال گرفته شد (۱۴۰۵/۰۶/۰۶، اسکرین‌شات کاربر — تب «درخواست» و «پاسخ»).
توضیح پورتال: «دریافت اطلاعات سرپرست تعیین‌شده در حکم فعال مربوط به کاربر در شعبهٔ مربوطه
**برای زمان حاضر**».

**Request:** `{"org_id":0,"profile_id":0}`

| Field | Type | Format | Notes |
|---|---|---|---|
| `org_id` | integer | int64 | شعبهٔ مورد درخواست |
| `profile_id` | integer | int64 | **پروفایل** مورد درخواست — دقت: `profile_id` است، نه `personnel_id` |

**Response:** `{"data":{"id":0,"name":""},"error":{"status":0,"message":""},"success":0}`

| Field | Type | Format | Notes |
|---|---|---|---|
| `data.id` | integer | int64 | شناسهٔ سرپرست |
| `data.name` | string | | **نام سرپرست** |

**چرا با `orderInDateGet.supervisor` تکراری نیست** (سوالی که قبل از دیدن schema مطرح بود):
`orderInDateGet` فقط شناسهٔ عددی سرپرست را می‌دهد و برای نام باید به `profile_main` join زد؛ این API
خودِ نام را برمی‌گرداند و منطقش شعبه‌محور است. پس هر جا جواب بدهد، منبع ارجح «سرپرست مستقیم» است.

⚠️ **محدودیت زمانی — در `hr_companion_report_bot.lua` رعایت شده:** پورتال صریح گفته «برای زمان
حاضر». یعنی این API سرپرستِ همین لحظه را می‌دهد، نه سرپرست در یک تاریخ گذشته. بنابراین بات فقط وقتی
از آن استفاده می‌کند که `to_date` برابر امروز باشد؛ در نمای تاریخی، سرپرستِ همان حکم (از
`orderInDateGet` یا جدول) دست‌نخورده می‌ماند تا یک مقدار امروزی جای عدد تاریخی ننشیند. منبعِ
استفاده‌شده در `supervisor_source` و در کارت «منبع اطلاعات این صفحه» نوشته می‌شود.

---

## واحد ستون‌های مدت‌زمان در اسکیمای HR — تاییدشده روی دادهٔ زنده

۱۴۰۵/۰۶/۰۷ با اجرای مستقیم روی `hr_work_time` تایید شد که مدت‌ها بر حسب **tick (۱۰۰ نانوثانیه)**
ذخیره می‌شوند، یعنی `ساعت = مقدار / 10000000 / 3600`:
...
پس محاسبهٔ کارکرد، اضافه‌کاری، تاخیر، مرخصی و ماموریت در `hr_companion_report_bot.lua` درست است.

**هنوز جداگانه تایید نشده:** ستون‌های مدت‌دارِ `hr_personnel_order` (`WORKING_HOURS`،
`LEAVE_PER_MONTH`، `MAX_DELAY_MONTH`، `REST_DURING_WORK`، `MAX_HOURLY_LEAVE`) و
`hr_leave_remained_records.LEAVE_REMAINED`. این‌ها هم‌خانوادهٔ همان اسکیمایند و با همین ضریب
نمایش داده می‌شوند، ولی در بات هر کدام یک سقف منطقی دارند: اگر مقدار تبدیل‌شده بیرون از بازه بیفتد،
به‌جای عدد «—» نشان داده می‌شود تا یک واحدِ متفاوت هرگز به‌شکل عددِ باورپذیرِ غلط ظاهر نشود.

## کدهای وضعیت `hr_vacation.STATUS` — تاییدشده با توزیع دادهٔ زنده

| کد | تعداد ردیف | برچسب بات |
|---|---|---|
| `1` | ۴۴۷۸ | تایید شده |
| `0` | ۴۱۳ | در انتظار تایید |
| `2` | ۱۲۰ | رد شده |

کد `3` («لغو شده») در دادهٔ زنده وجود ندارد؛ برچسبش در بات می‌ماند ولی استفاده نمی‌شود. توزیع
(اکثریت تایید‌شده، بعد در انتظار، کمترین رد‌شده) با همین نگاشت می‌خواند. ستون «کد وضعیت» عمداً در
جدول درخواست‌ها نمایش داده می‌شود تا در صورت اختلاف با پنل رسمی، سریع قابل تطبیق باشد.
