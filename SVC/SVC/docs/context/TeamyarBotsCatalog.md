...
| `crm_rfm_bot.lua` (بات ۵۰۱ "RFM CRM" اصلی، مبدأ `crm_rfm_1_bot.lua`/بات ۶۰۰، `run/2/crm_rfm`، RES-framework) | HTML | فرم `data.js`/`data.txt` بات ۵۰۱ (`datef`,`datet`,`org`(ACL6)،`crm`(ACL7)،`ctype`(ACL8)،`cat`(ACL9)،`sort_key`،`sort_dir`) | تحلیل RFM مشتریان. **۱۴۰۵/۰۵/۲۷ دور اول: فقط ۳ باگ عددی گزارش‌شده توسط کاربر** مستقیماً در همین فایل اصلاح شد (عیناً همان اصلاح `getData()` که قبلاً روی بات ۶۰۰ تأیید شده بود): ۱) `f_nummber`/`m_nummber` هر دو از `cdata.rnumber` می‌خواندند (کپی/پیست) → ستون‌های F/M و فرمول سگمنت عملاً فقط با وزن R حساب می‌شدند — اصلاح شد هرکدام از `cdata.fnumber`/`cdata.mnumber` خودش بخواند؛ ۲) چک `tonumber(datef/datet) == nil` هرگز true نمی‌شد (فریم‌ورک مقدار نامعتبر را بی‌صدا به `0` تبدیل می‌کند نه `nil`) → `between 0 and {{datet}}` کل تاریخچهٔ فاکتورها را برمی‌گرداند — اصلاح شد به `<= 0` + پیش‌فرض «امروز»؛ ۳) مبنای Recency/Days طبق تأیید کاربر از «امروز واقعی» به «انتهای بازهٔ فیلترشده» (`datet`) تغییر کرد. **۱۴۰۵/۰۵/۲۷ دور دوم (طبق درخواست بعدی کاربر):** ۴) فیلتر سازمان درست شد — `data.txt` کلید `org_id` را به `org` تغییر داد (تنها این‌طور با `name="org"` که `data.js` واقعاً می‌فرستد یکی می‌شود؛ کد Lua از اول درست بود)؛ ورودی `crm` هم اصلاً در `data.txt` declare نشده بود، اضافه شد؛ ۵) alias نامعتبر `c.id`→`p.id` در فیلتر `cat`، و `LEFT JOIN profile_user_info ui ON ui.id = p.REFFERE_ID` برای فیلتر `ctype`/`crm` به پیوست SQL اضافه شد (هر دو کد مرده بودند)؛ id همهٔ فیلترهای ACL هم با `tonumber()` اعتبارسنجی می‌شوند؛ ۶) **مرتب‌سازی کل دیتاست** (نه فقط صفحهٔ نمایشی): چون `$.Teamyar.table()` ویجت هسته‌ای پلتفرم است (کلیک‌روی-ستون واقعی خارج از کنترل این بات، طبق یادداشت رندرینگ زیر)، به‌جایش دو ورودی `sort_key`/`sort_dir` به فرم فیلتر اضافه شد که مستقیماً `ORDER BY` را **داخل خودِ کوئری SQL** (قبل از `LIMIT`/`OFFSET`) عوض می‌کنند — پس صفحهٔ گزارش/اکسل/پرینت هر سه روی کل نتایج فیلترشده مرتب می‌شوند، نه فقط ردیف‌های صفحهٔ فعلی؛ `sort_key` از یک whitelist ثابت (`_SORT_COLUMNS` در Lua) عبور می‌کند، هرگز raw داخل SQL نمی‌رود؛ ستون نمایشی `Customer` HTML-wrapped است، برای مرتب‌سازی صحیح یک ستون خام `CustomerSort` به CTE «data» در پیوست SQL اضافه شد. **۱۴۰۵/۰۵/۲۷ دور سوم (طبق درخواست بعدی کاربر):** ۷) فیلتر «بازهٔ درآمد» اضافه شد — دو ورودی جدید `monetary_min`/`monetary_max`؛ چون `Monetary` یک `sum()` تجمیعی به‌ازای هر مشتری است نه ستون خام هر فاکتور، فیلترش با `HAVING 1=1 {{where_monetary}}` روی CTE `crm_factor` پیاده شد (نه `WHERE` — در `HAVING` برخلاف `WHERE`، ارجاع به alias ستون `Monetary` در MySQL مجاز است)؛ چون این فیلتر **قبل از** محاسبهٔ `ntile` (رتبهٔ R/F/M) در CTE بعدی اعمال می‌شود، رتبه‌ها هم مثل بقیهٔ فیلترها فقط نسبت به همان زیرمجموعهٔ فیلترشده حساب می‌شوند، نه پس از رتبه‌بندی روی کل مشتریان. **۱۴۰۵/۰۵/۲۷ دور چهارم (اصلاح تفاهم — منظور کاربر «مرکز درآمد» فاکتور بود، نه بازهٔ درآمد؛ دور سوم بی‌اثر نشد، کنارش اضافه شد):** ۸) فیلتر «مرکز درآمد» اضافه شد — برای جدا کردن مشتریان «همکار» از «مصرف‌کننده» بر اساس مرکزی که هنگام ثبت فاکتور رویش انتخاب شده. فیلد واقعی: `sales_invoice.SALES_CENTER` → `pa_center` (تأییدشده با شواهد کد: همین جدول در بات‌های دیگر این پروژه، مثلاً `sales_settlement_group_auto`/بات ۴۳۳، «مرکز فروش» نامیده می‌شود). چون ستون خام سطح فاکتور است (نه aggregate مثل Monetary)، `WHERE s.SALES_CENTER=...` در سطح `crm_factor` (قبل از `GROUP BY`) کافی بود، دقیقاً مثل `org`/`cat`/`ctype`. ویجت فیلتر (`center`) عیناً با همان الگوی `$.Teamyar.acl` + `GetDataACL` که این ۴ فیلتر دیگر استفاده می‌کنند اضافه شد — **نه** سازوکار عمومی `searcher_id`/`searcher_type`/`tableName` که بات‌های دیگر این پروژه (بات ۴۳۳) دارند، چون `data.js` این بات یک `getFilters()` اختصاصی دارد که کلاً مسیر عمومی `reportbotSearchers`/`data.txt` را نادیده می‌گیرد (طبق یادداشت رندرینگ بالا) — با `type=12` جدید در dispatch و تابع `centerAcl(data)` که `pa_center` را کوئری می‌کند. **۱۴۰۵/۰۵/۲۷ دور پنجم (گزارش باگ زنده از کاربر پس از تست دور چهارم روی Teamyar واقعی):** کاربر گزارش کرد «انتخاب سازمان فرقی نمی‌کند»، «محتوای فیلتر مرکز درآمد نمی‌آید»، و نهایتاً «بات خروجی نمی‌دهد». ریشهٔ باگ اصلی را با یک **هارنس شبیه‌سازی محلی Lua** پیدا کردیم (بدون نیاز به دیتابیس واقعی: globalهای `teamyar`/`db`/`time`/`queryTools`/`install_res` استاب شدند، `getData()` واقعی همین فایل روی پیوست SQL واقعی اجرا شد و متن نهایی کوئری چاپ شد) — نکتهٔ جانبی مهم برای تکرار این تکنیک: خودِ بات یک تابع global به‌نام `print` هم دارد (برای دکمهٔ چاپ گزارش) که `print` استاندارد Lua را در همان محیط سراسری بی‌صدا بازنویسی می‌کند؛ هارنس باید قبل از `dofile` مرجع `print` واقعی را نگه دارد. ۱۰) **باگ اصلی:** در دورهای ۱ و ۳، وقتی `datef`/`datet` خالی بودند هر دو به همان یک متغیر `currentdate` (نیمه‌شب امروز) پیش‌فرض می‌گرفتند → `s.RUN_DATE between X and X` یعنی بازه‌ای با **پهنای صفر** که هیچ فاکتوری برنمی‌گرداند (نه فقط دادهٔ امروز، هیچ‌چیز) — چون نتیجهٔ پایه از قبل همیشه خالی بود، هیچ فیلتری (ازجمله سازمان) طبیعتاً اثری نداشت. اصلاح شد: `datet` اکنون به `currentdate_time_str` (لحظهٔ واقعی الان، با متغیر از قبل موجود ولی بلااستفادهٔ `currentdate_time`، فقط با `string.format("%18.0f",...)` فرمت‌شده تا از نماد علمی در گسابِ SQL جلوگیری شود) پیش‌فرض می‌گیرد — بازهٔ پیش‌فرض واقعاً «از نیمه‌شب امروز تا الان» است. ۱۱) **فیلتر مرکز درآمد خالی بود:** `pa_center` مثل `pa_client` یک جدول per-org است (شاهد کد جدید: `pa_center.ORG_ID` در `warranty_branch_expense_detail_report_bot.lua` با `ce.ORG_ID = vr.ORG_ID` فیلتر می‌شود) — `centerAcl` دور چهارم بدون فیلتر `org_id` کوئری می‌زد. اصلاح شد مثل `crmAcl` با `data.org_id` فیلتر شود (که `GetDataACL` مشترک در `data.js` همیشه از مقدار فعلی ویجت «سازمان» می‌فرستد)، با نادیده‌گرفتن فیلتر (نه شکستن کوئری) وقتی سازمان هنوز انتخاب نشده. **این دور فقط `src/crm_rfm_bot.lua` تغییر کرد** (data.txt/data.js/query_list_invoice.txt دور چهارم دست‌نخورده معتبر ماندند) — به‌صورت لینک Artifact (کد Lua) تحویل داده شد؛ کاربر خودش در پنل جایگزین می‌کند. **۱۴۰۵/۰۵/۲۷ دور ششم (لیست «مرکز درآمد» هنوز خالی بود، حتی پس از فیکس org scoping):** ۱۲) بدون دسترسی زنده به دیتابیس/پنل دیباگ بات از این محیط، ریشهٔ دقیق قابل تأیید نبود (`pa_center` واقعاً ردیف ندارد؟ نام ستون فرق دارد؟ خطای دیگر SQL؟). به‌جای حدس بعدی، `centerAcl` لاگ تشخیصی گرفت: متن دقیق کوئری همیشه با `teamyar.write_log` ثبت می‌شود، و اجرای `queryResultAcl` داخل `pcall` قرار گرفت — اگر کوئری با خطا شکست، آن خطا هم لاگ می‌شود (به‌جای شکستن بی‌صدا) و دیالوگ به‌جای کرش فقط خالی برمی‌گردد؛ این‌ها از تب Log اجرای بات در پنل Teamyar قابل مشاهده‌اند. کاربر باید یک‌بار دیالوگ «مرکز درآمد» را باز کند و لاگ را برای ادامهٔ عیب‌یابی برگرداند. **۱۴۰۵/۰۶/۰۸ دور هفتم (گزارش کاربر: «بات کند است»/«همان لحظهٔ باز کردن صفحه لود گیر می‌کند یا اصلاً تمام نمی‌شود»):** ۱۳) همان مشکل شناخته‌شدهٔ RES که قبلاً روی بات ۶۰۰ هم ثبت شده بود — RES هنگام باز شدن صفحهٔ بات، قبل از هر فیلتری، یک‌بار `report()`/`getTableSectionOne()` را خودکار صدا می‌زند (با `page_search_first=true`؛ کلیک واقعی «اجرا کردن» آن را `false` می‌فرستد) و چون کوئری این بات سنگین است (join دو جدول بزرگ + سه window function)، اجرای آن روی همین فراخوانی خودکار دقیقاً «گیر کردن لود همان لحظهٔ باز کردن صفحه» را توضیح می‌دهد. `getTableSectionOne()` اصلاح شد که با `isAutoFirstLoad` این فراخوانی خودکار را تشخیص دهد و کوئری سنگین را فقط بعد از کلیک واقعی «اجرا کردن» اجرا کند؛ `excel()`/`print()` این پرچم را دریافت نمی‌کنند، دست‌نخورده ماندند. **هشدار مهم (طبق تأیید صریح کاربر با آگاهی از ریسک انجام شد):** این دقیقاً همان فیکسی است که یک‌بار قبلاً روی بات ۶۰۰ امتحان و به‌دلیل خراب‌شدن فرم فیلتر برگردانده شده بود (دلیل دقیق آن خرابی هیچ‌وقت مشخص نشد — احتمال این‌که باگ دیگری هم‌زمان در `data.txt`/`data.js` دخیل بوده که این بار (بعد از دورهای ۲ تا ۶) قبلاً اصلاح شده). اگر این‌بار هم فرم فیلتر خراب شد، این بلاک (`isAutoFirstLoad` در `getTableSectionOne`) باید فوراً برگردد به حالت قبل (همیشه `getData` واقعی، بدون شرط). فقط `src/crm_rfm_bot.lua` تغییر کرد؛ پیوست‌ها دست‌نخورده ماندند. **۱۴۰۵/۰۶/۰۸ دور هشتم (کاربر موقع Save واقعی در پنل Teamyar خطای compile گرفت — اولین بار که این فایل واقعاً Save می‌شد، دورهای قبل فقط فایل/لینک بودند):** ۱۴) خطا: `unexpected symbol near '<\239>'` در خط اول. علت: یک UTF-8 BOM (بایت‌های `EF BB BF`) که همیشه از قبل (احتمالاً از خودِ بات ۵۰۱ زندهٔ اصلی، مستقل از هر دخالتی در این پروژه) ابتدای فایل بود — کامپایلر Lua خودِ Teamyar این BOM را نادیده نمی‌گیرد، آن را نماد نامعتبر می‌بیند. حذف شد؛ بقیهٔ فایل (محتوا، انکودینگ UTF-8 بدون BOM) دست‌نخورده ماند. **درس گرفته‌شده برای این ریپو:** قبل از تحویل هر بات دیگری که همین‌طور BOM دارد، حتماً با `od -An -tx1 -N3 file.lua` چک شود که سه بایت اول `ef bb bf` نباشد. **۱۴۰۵/۰۶/۰۸ دور نهم («لود سریع شد» تأیید شد، ولی دو گزارش جدید — UI و «سرچ چیزی برنمی‌گرداند»):** ۱۵) ویجت‌های فیلتر جدید (`sort_key`/`sort_dir`/`monetary_min`/`monetary_max`) بدون `|| ""` در `data.js`، وقتی چیزی cache نشده بود، مقدار تحت‌اللفظی `"null"`/`"۰"` داخل کادر نشان می‌دادند (نه کادر خالی) — گمراه‌کننده. اصلاح شد در `data.js`؛ عنوان چندخطی `sort_key` هم کوتاه شد («مرتب‌سازی بر اساس»)، لیست مقادیر مجاز به یک `<div>` راهنمای جدا (`SORT_KEY_HINT` در `data.txt`) منتقل شد. ۱۶) کاربر گزارش کرد بعد از انتخاب فیلتر و زدن «اجرا کردن»، جدول نتایج خالی می‌ماند. **این خطر جدی است**: چون فیکس دور هفتم (`isAutoFirstLoad`/`page_search_first`) دقیقاً همین مسیر (`getTableSectionOne`) را عوض کرد، ممکن است `page_search_first` حتی روی کلیک واقعی «اجرا کردن» هم `true` بماند (ادعای «کلیک واقعی `false` می‌فرستد» فقط از خواندن سورس `res_v2` در یک نشست قبلی بود، هرگز زنده روی این مسیر خاص تأیید نشده) — یعنی ممکن است این فیکس کوئری واقعی را همیشه رد کند، نه فقط لود خودکار اول؛ احتمال دیگر: فیلترهای انتخاب‌شده (سازمان + بازهٔ تاریخ کوتاه) واقعاً هیچ فاکتوری نداشته‌اند. بدون دیتابیس زنده قابل تشخیص نیست — یک لاگ تشخیصی اضافه شد (`teamyar.write_log` با مقدار `page_search_first` در هر فراخوانی `getTableSectionOne`) تا از تب Log بات مشخص شود کدام است؛ اگر `false` بماند و کوئری واقعی صفر ردیف بدهد، مشکل داده است نه کد؛ اگر `true` بماند، فیکس دور هفتم باید فوراً برگردد. |
