...
-- ============================================================================
-- ⛔ نتیجهٔ آزمون زنده (1405/06/04) — فرضیهٔ زیر **رد شد**. روی سند ۵۰۸۳ (ORG 8، ۱۵ ردیف امضا):
--    نوشتن قطعاً انجام شد (`CONTENT` عوض شد، بازرسی یکپارچگی پاک، وضعیت `no_effect` نه `no_write`)
--    ولی امضاها **۱۵ → ۱۵** ماندند. یعنی `voucher/records_update` منطق باطل‌کنندهٔ امضا را فعال
--    نمی‌کند؛ آن منطق در کنترلر ماژول حسابداری است و مسیر `/api/` دورش می‌زند. **مسیر API برای این
--    کار بن‌بست است.** بات همین را گزارش کرد و دسته را متوقف کرد — طبق طراحی، بدون ادعای موفقیت جعلی.
--    این بات فعلاً به‌عنوان «پیش‌نمایش دامنه + ابزار آزمون» معتبر است، نه ابزار پاک‌سازی.
--
