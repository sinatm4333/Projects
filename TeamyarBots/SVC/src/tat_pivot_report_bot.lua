...
function getHeatMapColor(value, min, max) {
    ...
    // محاسبه رنگ از سبز روشن تا قرمز تیره
    var red, green, blue;
    
    if (normalized < 0.5) {
        // از سبز تا زرد (0 تا 0.5)
        red = Math.floor(255 * normalized * 2);
        green = 255;
        blue = Math.floor(255 * (1 - normalized * 2));
    } else {
        // از زرد تا قرمز (0.5 تا 1)
        red = 255;
...
cells.forEach(function(cell) {
    cell.addEventListener('mouseenter', function() {
        ...
        // تیره کردن تمام سلول‌های سطر از ابتدا تا این ستون
        ...
        // تیره کردن تمام سلول‌های ستون از ابتدا تا این سطر
        ...
        // همیشه تیره کردن ستون بازه زمانی (ستون 0) از ابتدا تا این سطر
        ...
        // همیشه تیره کردن ستون درصد (ستون آخر) از ابتدا تا این سطر
    ...
    cell.addEventListener('mouseleave', function() {
        // حذف کلاس hover-darken از همه سلول‌ها
...
when DATEDIFF(max(k.CommentDate),min(k.CommentDate)) > 0 and DATEDIFF(max(k.CommentDate),min(k.CommentDate)) <6 then 5
when DATEDIFF(max(k.CommentDate),min(k.CommentDate)) > 5 and DATEDIFF(max(k.CommentDate),min(k.CommentDate)) <11 then 10
when DATEDIFF(max(k.CommentDate),min(k.CommentDate)) > 10 and DATEDIFF(max(k.CommentDate),min(k.CommentDate)) <16 then 15
when DATEDIFF(max(k.CommentDate),min(k.CommentDate)) > 15 and DATEDIFF(max(k.CommentDate),min(k.CommentDate)) <21 then 20
when DATEDIFF(max(k.CommentDate),min(k.CommentDate)) > 20 and DATEDIFF(max(k.CommentDate),min(k.CommentDate)) <26 then 25
when DATEDIFF(max(k.CommentDate),min(k.CommentDate)) > 25 and DATEDIFF(max(k.CommentDate),min(k.CommentDate)) <31 then 30
else 31 end as TAT
from (
