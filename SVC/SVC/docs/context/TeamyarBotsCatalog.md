...
| `call_center_admin_dashboard_report_bot.lua` (جدید، هنوز مستقر نشده) | HTML | POST از/تا (سه Dropdown شمسی روز/ماه/سال، پیش‌فرض ۷ روز اخیر) | داشبورد واحد مدیریتی کال‌سنتر — جایگزین ۹ بات پراکندهٔ بخش «بات‌های کال‌سنتر / VOIP» زیر. موبایل-اول (CSS پایه تک‌ستونه، `@media(min-width)` گسترش به تبلت/دسکتاپ)، گاید ۱۴۰. سه بخش: نمای‌کلی (KPI کل/پاسخ/ناموفق/ورودی/خروجی/مدت + دونات نتیجه/جهت + تاپ ۱۰ کارمند)، روند تماس‌ها (روزانه + ساعتی)، عملکرد کارمندان (جدول کامل قابل‌مرتب‌سازی، ورودی/خروجی تفکیک‌شده). شمارش تماس همه‌جا `COUNT(DISTINCT linkedid)` (نه `COUNT(*)`، چون هر تماس ۲ Leg دارد)؛ Inbound/Outbound با تطبیق profile ID نه آستانهٔ عددی؛ «وضعیت کارمندان» = عملکردی، نه presence لحظه‌ای. تاییدشده با `luac -p` + یک هارنس Lua با runtime شبیه‌سازی‌شده (هر ۶ کوئری، تطابق تعداد `?`/params؛ خروجی HTML بدون نشت nil) |
