...
function getCallsInfo(data)
    ...
    box_clause = [[  ConnectedLineProfileID = ]]..users;
    ...
    local param = {
        query=query,
         params={data.date_from,data.date_to} 
    } 
    db.query(param) 
    local result={} ; 
    local record={}; 
...
local template = teamyar.run_command("2/res_bot",{
    ...
    url:'/bot/run/2/voip_inbound',data:()=>{ 
    ...
    generatedata=[[(data)=>{
    return    ty__main.createdataForChartInbound(data);
  }]],
