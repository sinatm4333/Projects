...
| (بدون id — هنوز دیپلوی نشده) | تسویه خودکار — نسخهٔ بک‌فیل انبوه (کپی بات ۵۸۲، ۱۴۰۵/۰۶/۰۵). برای پاکسازی یک‌بارهٔ صف بزرگ (مثال کاربر: ~۴۰۰٬۰۰۰ فاکتور) که چک‌باکس/UI (بات ۶۱۲) برایش عملی نیست. فیلتر/کوئری عیناً از ۵۸۲؛ تفاوت: حلقهٔ تسویه **زمان‌محور** است — تا `_MAX_RUNTIME_SECONDS` (پیش‌فرض ۱۶۲۰۰ ثانیه؛ باید کمی زیر `max_execute_time` واقعی بات روی پنل باشد) پردازش می‌کند و تمیز متوقف می‌شود، به‌جای پردازش نامحدود همه‌چیز در یک اجرا. چون فاکتور تسویه‌شده خودش از شرط کوئری خارج می‌شود، هر اجرای بعدی خودکار از همان‌جا ادامه می‌دهد — بدون checkpoint دستی؛ باید تا خالی‌شدن صف چندبار دستی اجرا (یا زمان‌بندی) شود. رفع باگ همراه (از ۵۸۲): `update_invoice_history` فقط روی تسویهٔ موفق صدا زده می‌شود + `pcall` روی هر دو فراخوانی API. **دیپلوی نشده — کاربر با `max_execute_time` بزرگ (کاربر خودش ۱۸۰۰۰ ثانیه ست کرده) روی یک id جدید دیپلوی می‌کند.** | `sales_settlement_backfill_queue_bot.lua` | نامشخص |
