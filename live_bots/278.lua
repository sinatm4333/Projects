--Bot Responsible Recruitment In Current Month by zmo
--ver 001
--start 2024-4-18
...
function getDeposits(logins)
  ...
  if cli_rand_answer ~= auth_answer_answer then
    ...
    teamyar.write_log(json.encode(li))
    for i = 1, #li do
      if li[i] ~= nil then 
...
------------------
function loadData()
  local data = teamyar.get_data("RRC_data")
...
function WidgetTemplate2()  
  ...
  if user_info.lang_id == 4 then
    str_title = " میزان جذب هر کارشناس در ماه جاری"
    str_lang = teamyar.get_attachment("Persian.js");
  else
    str_title = "Responsible Recruitment In Current Month"
...
elseif input.type == 3 then 
  local chash_data = {rrc_responsible_id = input.responsible_id, rrc_rn = input.rn, rrc_dr = input.dr, rrc_drn = input.drn }
  teamyar.set_data("rrc_data", chash_data);
  data = teamyar.get_data("rrc_data")
  responsible_id = data.value.rrc_responsible_id;
  if responsible_id ~= nill  and responsible_id ~= "" then 
    WidgetTemplate()  
  end
elseif input.type == 4 then 
  ...
  local str_query = [[  select value_char from CRM_FIELD_EXT_VALUE  v inner join crm_field_ext ex on ex.id=v.field_id where ex.name='Account'                               
                                  ...
                                  and  p.user_id=]]..input.responsible_id..[[) ]]
