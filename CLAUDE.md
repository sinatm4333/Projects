...
**قبل از Update هر باتی که این ریپو نساخته (تایید زنده 1405/06/09، بات 483):** اول `GET /bot/command/view?id={id}&cat_id={هرچیزی}` بزن و `cat_id` واقعی + فلگ‌های `public_access`/`show_in_widget`/`open_source`/`show_in_portal_menu` زنده را بخوان، بعد عین همان‌ها را به `deploy_teamyar_bot.ps1` پاس بده (`-CatId`، `-PublicAccess`، `-ShowInWidget`، `-OpenSource`). پیش‌فرض اسکریپت همهٔ فلگ‌ها را ۰ و دسته را `-CatId` می‌فرستد — یعنی یک Update سرسری بات را به دستهٔ دیگری «منتقل» می‌کند و از ویجت/دسترسی عمومی می‌اندازد (سر بات ۴۸۳ آمد: از دستهٔ «Sale»=59 به ۷۹ رفت و کاربر بات را «گم کرد»؛ نام دسته‌ها در جدول `bot_section` است). نکته: فیلد `cat_id` در پاسخ view فقط echo پارامتر URL است — دستهٔ واقعی را از فیلد `categories` (نام دسته) یا از `bot_command_categories` بگیر.

**تکمیل (تایید زنده 1405/06/09، بات 478 «فاکتورهای تکراری با شماره سفارش سایت»، دستهٔ Sale/subsystem-دار):**
- **شناسه و فرمت دقیق دستهٔ واقعی** را از فرم ویرایش بگیر: `GET /bot/command?cat_id={هرچیزی}&id={id}` (پاسخ `botCommandFunc`) — آرایهٔ `categories` با `{id, cat:[{id,name}], is_default}` برمی‌گردد. فرمت فیلد POST طبق `bot.js beforSubmit`: **`<categoryId>_<isDefault>`** با کاما و دقیقاً یک عضو `_1` (مثلاً `59_1` برای Sale) — **نه** `cat_sec`؛ مقدار غلط → `400 NOT_EXIST_CATEGORY_DEFAULT`. با `-Categories '59_1'` پاس بده.
- **زیرسیستم را هم echo کن**: همان پاسخ فرم ویرایش، `subsystem: {id:"23_2", name:"SALES_VIEW"}` می‌دهد → `-SubsystemValue '23_2' -SubsystemName 'SALES_VIEW'`. نفرستادنش زیرسیستم بات را پاک می‌کند.
- **`bot_config` زندهٔ بات را echo کن** (`-BotConfigJson`) وگرنه Update آن را خالی می‌کند — مقدارش را از view بخوان و verbatim پاس بده. فایل موقت باید UTF-8 **بدون BOM** باشد (BOM باعث شد config یک بار خالی ذخیره شود؛ اسکریپت از 1405/06/09 اصلاح شده).
- **`-Description ''` را صریح پاس بده** وقتی توضیح زندهٔ بات خالی است، وگرنه اسکریپت «Deployed from ...» می‌گذارد.
- **`scripts/update_bot_preserve.ps1` استفاده نشود** — شکل POST آن (فیلدهای متنی از فایل + ترتیب متفاوت، `command` آخر) روی این سرور همیشه **502 Bad Gateway** می‌گیرد، حتی برای بات سادهٔ تازه‌ساخته (تست ایزوله ۱۴۰۵/۰۶/۰۹ با بات موقت 630، غیرفعال‌شده). مسیر درستِ Update حفظ‌کننده: همین `deploy_teamyar_bot.ps1` با پارامترهای echo بالا.
...
- **Confirmed live (1405/06/09, `sales_invoice_note_mismatch_report_bot.lua` bot 483) — saving a bot's `command` also silently normalizes Arabic Yeh characters to Persian: literal `ي` (U+064A) and `ى` (U+0649) in the source become `ی` (U+06CC) on the server** (verified by byte-diffing the saved command against the deployed file — the only 2 changed chars were these, in comments). Presentation-form variants (ﻱﻲﻳﻴ / ﯼﯽﯾﯿ) were NOT touched. Consequence: any bot whose *logic* depends on matching literal Arabic-Yeh characters (normalizers, dedup/compare bots) must build them with `string.char(...)` byte sequences, never literals — a literal would be silently rewritten and the branch could never match.
...
- **Confirmed live (1405/05/26, `crm_geo_sales_dashboard_bot.lua` bot 604) — the browser Fullscreen API (`Element.requestFullscreen()`) is unreliable for this platform's «تمام صفحه» toolbar button: content can go fullscreen but become unscrollable** (seen on Android Chrome; Teamyar bots may also render inside an iframe without `allow="fullscreen"`, which the Fullscreen API depends on). **Do not rely on `requestFullscreen()`/`:fullscreen` CSS for the mandatory fullscreen toolbar button** — instead toggle a plain CSS class on the report root (`position:fixed; inset:0; z-index:9999; overflow-y:auto`) via JS `classList.toggle(...)`. No Fullscreen API calls, no `:fullscreen`/`-webkit-full-screen` pseudo-classes — this sidesteps the platform/mobile inconsistency entirely and is the pattern now used in both `crm_geo_sales_dashboard_bot.lua` and `sales_revenue_center_dashboard_report_bot.lua`.
  **Addendum (1405/06/07, bot 624) — the class-toggle alone is NOT enough when the bot renders inside the portal, because `result_type=1` output is injected into a same-origin `<iframe>`** (`core_callbot_command_view`; confirmed from platform source `000_top.js`: `$.Teamyar.bot.appendIframForresult` → `tools.iframhtml.control({iframe: ...})`, with `height_auto` keeping the iframe's inline height equal to the content height via a MutationObserver). Inside that iframe, `position:fixed; inset:0` fills the *iframe's* viewport — which IS the full content height — so the «تمام صفحه» button appears to do nothing/break. **Fix (no Fullscreen API, verified live in a faithful harness): escape the iframe through the parent document** — the iframe is same-origin, so `window.frameElement` is accessible: inject a `<style>` into the parent head with `iframe.hr140-fs{position:fixed !important; ... height:100vh !important;}` and toggle that class on `frameElement` (plus `parent body overflow:hidden` while active). The `!important` stylesheet rule is essential: the platform's MutationObserver keeps rewriting the iframe's *inline* height (non-important) on every DOM mutation and would otherwise undo the fullscreen height immediately. Wrap in try/catch — standalone runs (`/bot/run/...` opened directly) have no `frameElement` and the plain class-toggle already works there. See `toggleFullScreen()` in `hr_companion_report_bot.lua`. Bots 604/609 have the same limitation when viewed inside the portal panel.
- **Confirmed live (1405/06/07, `hr_companion_report_bot.lua` bot 624) — the SQL function `REPORT_FN_JDATE(col, '/')` DOES NOT EXIST on this database.** Every call to it fails the *entire* query with the generic `"sql error"`, so a single such column silently kills a whole report section. Proven directly on bot 589 (`mode=rawp`): `SELECT 1` succeeds, `SELECT REPORT_FN_JDATE(133000000000000000, '/')` fails, and `SELECT ROUTINE_NAME FROM information_schema.ROUTINES WHERE ROUTINE_NAME LIKE 'REPORT%'` returns **zero rows** (all 48 routines on the server belong to `sys`; schema `0000000` defines none). Server is MySQL **8.4.3**. This contradicts the RES-framework note above, which mentions `REPORT_FN_JDATE` as if it were available — that mention is about the platform's own PHP/Lua layer, not a callable SQL function.
  **Replacement (drop-in):** `report_dimdate.JNDATE` already stores exactly the same `'YYYY/MM/DD'` Jalali string, keyed by `DATEKEY` = the FILETIME of that day's midnight (confirmed: `DATEKEY=134324352000000000` → `JNDATE='1405/06/07'`).
  ```lua
  local function sql_jalali_date(col)
      return "COALESCE((SELECT rdj.JNDATE FROM report_dimdate rdj WHERE rdj.DATEKEY = " ..
          "FLOOR(COALESCE(" .. col .. ", 0) / " .. FT_DAY .. ") * " .. FT_DAY .. "), '')"
  end
  ```
  `FLOOR(x / FT_DAY) * FT_DAY` is deliberate instead of `x - MOD(x, FT_DAY)`: it references `col` **once**, so the helper stays safe when `col` is a bound `?` placeholder (the `MOD` form would duplicate the placeholder and shift every following parameter). Rounding to midnight also makes it correct on time-bearing columns like `DATE_CREATE`. `DATEKEY = 0` matches no row → `''`, the same result the old `COALESCE(..., '')` produced. Where a query **already** joins `report_dimdate rd` on that same column, select `COALESCE(rd.JNDATE, '')` directly instead of nesting a redundant subquery.
  **Audit every other bot in `src/` that calls `REPORT_FN_JDATE`** — the failure mode is a silently empty report section, not a visible error.
- **Confirmed live (1405/06/07, bot 624) — HR time-of-day values are stored 3:30 behind Iran local time (UTC), while day-key columns are local.** Verified by matching two `hr_vacation` rows of personnel 356 against the official Teamyar HR module UI: stored `45000s=12:30` ↔ real `16:00`, stored `19800s=05:30` ↔ real `09:00`. Day-key columns (`WORK_DATE`, `DAY_DATE`, `DATE_VACATOIN`, `EXT_DATE`, `report_dimdate.DATEKEY`) are exact multiples of `FT_DAY` and already match the local Jalali date — only the intra-day component is shifted. **Any time-of-day display must add `TZ_OFFSET_TICKS = 126000000000` (+3:30)**: `MOD(MOD(col, FT_DAY) + 126000000000, FT_DAY)` (see `sql_time_of_day` in `hr_companion_report_bot.lua`); Jalali-date conversion of true timestamps should add the offset before flooring (see `sql_jalali_date`). Durations (`TOTAL_WORK`, `TOTAL_TIME`, `TIME_TO - TIME_FROM`) need **no** offset.
- **Confirmed live (1405/06/07, bot 624) — `hr_vacation` column semantics are NOT what the names suggest:** `TYPE` is the request *shape* (1=hourly leave, 2=daily leave, 3=hourly mission, 4=daily mission — the whole table holds only these four; TYPE 3/4 always have KIND=0), and **`KIND` is the leave-type FK → `hr_vacation_type.ID`** (2=PAID/استحقاقی etc.). `hr_vacation_type` is tiny (7 rows: MEDICAL, PAID, PERSUASIVE, NOT_PAID, MARRIAGE, HOLIDAY, زایمان) and its `TYPE` column is the pay *category* (1=با حقوق، 2=بدون حقوق), **not unique** — joining `ON vt.TYPE = v.TYPE` fans each request out up to 5× with wrong names (this exact bug shipped in bot 624's first deploy: one real استحقاقی request rendered as MEDICAL+PAID+PERSUASIVE+MARRIAGE rows). Correct join: `LEFT JOIN hr_vacation_type vt ON vt.ID = v.KIND`. `NAME` holds raw English codes needing Persian translation (map in `vacation_type_label`). `TOTAL_TIME_LICENSE` = requested duration (the UI's «مرخصی» column), `TOTAL_TIME` = computed used time («مرخصی استفاده شده»). Daily requests show «یک روز», no times.
- **Confirmed live (1405/06/07, bot 624) — `hr_day_details` has up to 3 bands per day, distinguished by `TYPE`: 2 = the actual duty shift (~7.5h avg), 5 = pre-shift overtime window (~2.5h), 3 = post-shift overtime window (~9h, runs to 23:59).** Summing all bands for موظفی gives nonsense like «17:59» — filter `d.TYPE = 2` when computing shift window/duty minutes. Also `hr_work_time.ABSENCE` is total **کسر کار** (shortfall vs duty, verified: 43min on a 7:17-worked 8h day), not just late-arrival تأخیر — label it «کسر کار».
- **More HR semantics confirmed live (1405/06/07, bot 624):**
  - **`hr_ext_time.TYPE`**: 1=normal device punch (تردد عادی، 43582 rows), 2=daily mission, 3=hourly mission, 4=daily leave, 5=hourly leave — each non-1 count matches its `hr_vacation` shape count exactly (30/77/2732/2177). A mapping that labels TYPE=1 as anything but تردد عادی mislabels virtually every attendance event.
  - **`hr_personnel_order` duration fields (`WORKING_HOURS`, `LEAVE_PER_MONTH`, `MAX_DELAY_MONTH`, `MAX_HOURLY_LEAVE`, `MIN_HOURLY_LEAVE`) are in SECONDS**, both in the DB and in `orderInDateGet`'s response (28800=8h duty — matches the TYPE=2 calendar band; 900=15min; 10800=3h). Dividing by 10^7 as if ticks silently zeroes them all (rendered «—»).
  - **`orderInDateGet`/HR APIs return `ACCESS_DENIED` for non-admin users** — every self-service bot must treat an API error as "fall back to DB" and must NOT surface the API error in the UI when the fallback succeeds. The DB order row has all the same fields incl. the boolean flags (`TELEWORK_REQUEST`, `OVERTIME_DISABLED`, ... , `unemployment_insurance_exemption`).
  - **`hr_work_time` single-punch days keep `LAST_OUT = FIRST_IN`** (until the second punch) with `TOTAL_WORK=0`, and the current in-progress day can already carry `ABSENT=1` + a full-shift `ABSENCE`. When totalling کسر کار, only count days with `ABSENT=0` and a completed punch pair — otherwise absent days (whose whole duty lands in ABSENCE, already counted as روزهای غیبت) and today double the figure severalfold (live: 15:03 shown vs the real ~1:03).
