...
function customerAcl(data)
  local query_param = [[ SELECT JSON_ARRAYAGG(JSON_OBJECT('id',i, 'name',n, 'type',1))
  from (select distinct id i ,concat(code,'_',name)n from pa_client where type=1 and VOUCHER_ALLOW=1  ]]
  if data.search ~= nil and #data.search > 0 then
    query_param = query_param ..  [[ and name like N'%]]..data.search..[[%' or  code like  N'%]]..data.search..[[%' ]]
  end
...
elseif input.type == 1 then 
 ...
 decimal_discount as (select decimal_count d  ,i.id fid from  PA_SYMBOLS ps  inner join sales_invoice i on i.SYMBOL_ID=ps.id where ps.org_id=]]..org_id..[[ ),
  ...
  decimaln as (select coalesce((select case when decimal_count>0  then decimal_count else fee_decimal end),0 )d 
  ...
  --where status
  if  datef ~= nil and datet ~= nil and datef ~= "" and datet ~= ""    then 
     with_str=with_str..[[ and  v.RUN_DATE <=]]..datef..[[ and  v.RUN_DATE >=(select start_date from pa_fiscal_year where START_DATE <=]]..datef..[[ and END_DATE >=]]..datet..[[ and  org_id=]]..org_id..[[) ]]
  end
  ...
  if  input.floating ~= nil and input.floating ~= nil   then 
   ...
   with_str=with_str.. [[) union (SELECT DISTINCT COALESCE((select name from pa_floating where id=_oc.FLOATING_ID and org_id=_oc.org_id),'--') floating,
                            COALESCE( (select name from pa_project where id=_oc.project_id and org_id=_oc.org_id),'--') prj,_oc.id fid,_oc.invoice_id fi, _oc.RUN_DATE d, 0 vi, 0 vid,
  ...
  with_str=with_str..[[ GROUP BY _oc.ID,_oc.TITLE,_oc.INVOICE_CODE,_oc.INVOICE_ID,_oc.FLOATING_ID, _oc.ORG_ID,_oc.CLIENT_ID,_oc.ACCOUNT_ID,_oc.PRE_INVOICE,_oc.STATUS,_oc.TYPE,_oc.RUN_DATE,_oh.CODE,
                                  ...
                                  union
                                  ( select COALESCE((select name from pa_floating where id=floating_account_id and org_id=v.org_id),'--') floating, COALESCE( (select name from pa_project where id=project_id  and org_id=v.org_id),'--')   prj,0 fid,0 fi,v.run_date d, v.voucher_id vi,v.id vid,
                                  ...
                                  v.title t,(select name from pa_client where id=vr.client_id and org_id=v.org_id)crm,(select reffere_id from pa_client where id=vr.client_id and org_id=v.org_id) crm_id from pa_voucher v inner join 
  ...
  with_str=with_str..[[  and vr.deleted=0 and v.deleted=0  and
                                  ((select status from pa_pdc where id=vr.PDC_ID and org_id=v.org_id)<>3  or (select status from pa_pdc where id=vr.PDC_ID and org_id=v.org_id) is null) 
