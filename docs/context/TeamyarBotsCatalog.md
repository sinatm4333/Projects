...
| `document_content_access_probe_json_bot.lua` (جدید، هنوز مستقر نشده — بات **تشخیصی موقت**، در حال حاضر نسخهٔ v02) | JSON | بدون ورودی الزامی (`document_id` ثابت `TEST_DOCUMENT_ID=19025` در کد؛ اگر ورودی `document_id`/`id`/`doc_id` داده شود همان استفاده می‌شود) | پیش‌نیاز داشبورد چندفایلی CSV از روی «شناسه سند» ماژول اسناد (مثال زنده: id=۱۹۰۲۴/۱۹۰۲۵ در «اسناد / واحد مالی / فایل مشترک موبایل ۱۴۰ / مودیان»). `documents_main` فقط متادیتا دارد (نه محتوای باینری). **v01 تایید کرد** `io`/`os` در Sandbox این پلتفرم موجود نیستند — پس مسیر مستقیم دیسک بی‌فایده است. **v01 هم‌زمان کشف کرد** شیء `teamyar` توابع مستندنشدهٔ بسیار مرتبطی دارد: `get_file`, `get_attachment`, `create_file_manager`, `csv_to_json`, `json_to_csv`. v02 دقیقاً همین‌ها را با چند امضای فراخوانی محتمل امتحان می‌کند (`get_file(id)`/`get_file({id=..})`، متدهای احتمالی روی شیء `create_file_manager()`، `csv_to_json` هم روی یک نمونهٔ ساختگی و هم — در صورت پیدا شدن محتوای واقعی — روی خود فایل) و نتیجهٔ هرکدام (نوع/طول/پیش‌نمایش، نه کل محتوا) را برمی‌گرداند. **باید روی سرور واقعی توسط کاربر اجرا شود** (این محیط توسعه به `erp.bimehland.com` دسترسی شبکه ندارد) — خروجی JSON آن مسیر واقعی خواندن محتوای CSV در بات نهایی را تعیین می‌کند. |
