...
⚠️ **تأیید زنده (1405/06/04، بات ۶۱۷ `voucher_signature_clear_report`) — این endpoint امضاهای سند را
باطل نمی‌کند.** در پنل، وقتی کاربر سند حسابداری را ویرایش می‌کند (مثلاً یک فاصله به شرح اضافه می‌کند)،
پلتفرم همهٔ ردیف‌های `pa_voucher_signs` آن سند را باطل می‌کند. تصور اولیه این بود که مسیر API هم همان
منطق را فعال کند. **نمی‌کند.** آزمون واقعی روی سند `5083` (ORG 8، ۱۵ ردیف امضا):

| گام | نتیجه |
|---|---|
| فراخوانی `records_update` با تغییر `CONTENT` (Toggle یک فاصله) | `success = true` |
| خواندن دوبارهٔ رکورد و مقایسه با تصویر قبلی | `CONTENT` واقعاً عوض شده بود؛ هیچ ستون دیگری تغییر نکرد |
| شمارش `pa_voucher_signs` قبل و بعد | **۱۵ → ۱۵ — هیچ امضایی باطل نشد** |

یعنی نوشتن قطعاً انجام شد (نه `no_write`)، ولی منطقِ باطل‌کنندهٔ امضا در مسیر نوشتن دیتابیس نیست —
در **کنترلر خود ماژول حسابداری** است و مسیر `/api/` آن را دور می‌زند.

**همچنین: هیچ endpoint امضا/حذف‌امضا در کل سطح API مستند وجود ندارد** — نه در ۲۴ endpoint ماژول ۱۰ و نه
در هیچ ماژول دیگری (کل این سند برای `sign`/`امضا`/`approve` جست‌وجو شد). پس «امضا کردن سند» و
«باطل کردن امضا» از طریق `teamyar.call_api` **قابل انجام نیست**.

مسیر باقی‌مانده: فراخوانی همان endpointهایی که خودِ پنل حسابداری هنگام امضا/ویرایش صدا می‌زند، از
**سمت مرورگر** (JS خودِ بات داخل نشست احراز‌هویت‌شدهٔ کاربر اجرا می‌شود، پس دسترسی‌های همان کاربر را
دارد). این مسیر هنوز آزموده نشده و نیازمند ثبت آدرس واقعی آن endpointها از تب Network است.
