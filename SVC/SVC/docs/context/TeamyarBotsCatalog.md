...
| `crm_rfm_bot.lua` (بات ۵۰۱ "RFM CRM" اصلی، مبدأ `crm_rfm_1_bot.lua`/بات ۶۰۰، `run/2/crm_rfm`، RES-framework) | HTML | همان فرم `data.js`/`data.txt` بات ۵۰۱ (`datef`,`datet`,`org_id`(ACL7)،`ctype`(ACL8)،`cat`(ACL9)) | تحلیل RFM مشتریان. **۱۴۰۵/۰۵/۲۷: فقط ۳ باگ عددی گزارش‌شده توسط کاربر مستقیماً در همین فایل (نه در بات ۶۰۰) اصلاح شد**، عیناً همان اصلاح `getData()` که قبلاً روی بات ۶۰۰ تأیید شده بود: ۱) `f_nummber`/`m_nummber` هر دو از `cdata.rnumber` می‌خواندند (کپی/پیست) → ستون‌های F/M و فرمول سگمنت (`rfm`/`segment`) عملاً فقط با وزن R حساب می‌شدند نه با اعداد واقعی ورودی F/M — اصلاح شد هرکدام از `cdata.fnumber`/`cdata.mnumber` خودش می‌خواند؛ ۲) چک `tonumber(datef/datet) == nil` هرگز true نمی‌شد (چون `data.txt` نوع را `number` declare کرده و فریم‌ورک مقدار نامعتبر را بی‌صدا به `0` تبدیل می‌کند نه `nil`) → `between 0 and {{datet}}` عملاً کل تاریخچهٔ فاکتورها (Monetary/Frequency/Days) را برمی‌گرداند نه بازهٔ انتخابی — اصلاح شد به `<= 0` + پیش‌فرض «امروز»؛ ۳) مبنای Recency/Days طبق تأیید کاربر از «امروز واقعی» به «انتهای بازهٔ فیلترشده» (`datet`) تغییر کرد (فقط جایگزینی مقدار `{{current_date}}` در Lua از `currentdate` به `datet` — متن SQL دست‌نخورده ماند چون نام placeholder همان است، فقط مقداردهی‌اش عوض شد؛ چون `datet` خالی هم پیش‌فرض «امروز» می‌گیرد، رفتار حالت بدون فیلتر همان قبل است). **عمداً دست‌نخورده ماند** (خارج از این ۳ باگ، نیاز به تأیید جداگانه): فیلتر `cat` هنوز به alias نامعتبر `c.id` ارجاع می‌دهد، فیلتر `ctype`/`crm` هنوز بدون join به `profile_user_info ui` است (هر دو کد مرده)، و در `data.txt` کلید `org_id` با نامی که `data.js` واقعاً submit می‌کند (`org`) یکی نیست — این سه مورد در بات ۶۰۰ رفع شده‌اند ولی چون کاربر فقط ۳ باگ عددی بالا را خواسته بود، اینجا لمس نشدند. |
