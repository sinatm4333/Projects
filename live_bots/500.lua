...
function SendMessageInDomainMode()
  ...
  local query = [[ select id  from chat_dialogs where topic=']].. "Saite/Admin/"..input.to..[['  and deleted=0 ]]
  admin_dialog_id = queryResultValue(query,{})
  ---------------------------------create dialog admin
  if admin_dialog_id == nil or admin_dialog_id == 0 then 
    local info_admin_dialog = {topic = "Saite/Admin/"..input.sender.number, group_id = group_id, author_id  = user_id}    
  ...
  local info_msg_admin = {message = input.status, author_id = 	73425, dialog_id  = admin_dialog_id}
...
if input.key == config_key --  and to_number.."@c.us" == input.to
  ...
  if input.sender.isGroup == true then 
    topic_str ="SaiteGroup/".. input.sender.groupName .."_"..input.from
  else
   ...
   if input.direction == "outgoing" then 
      local tt=string.match( input.to, "^(.-)@")
    	...
    	topic_str = "Saite/"..input.sender.number
    end
  end 
