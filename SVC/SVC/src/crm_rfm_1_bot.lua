...
-- Last Edit = 1405/05/22 18:20

-- botName = Sum Workdays And Sales Params
-- creator = zmo
-- date = 02/25/2025
-- version= 1.3 (اصلاح باگ‌های getData + جلوگیری از کوئری سنگین روی لود خودکار اول صفحه)
--
...
function getTableSectionOne()
  ...
  local values, total;
  -- باگ اصلاح‌شده: RES (template_report.html) هنگام باز شدن صفحه، قبل از این‌که کاربر فیلتری
  -- انتخاب کند، همیشه یک‌بار report()/getTableSectionOne() را خودکار صدا می‌زند
  -- (report[bot].onClickBtnSubmit(false) در انتهای اسکریپت خود RES، غیرقابل تغییر از این فایل).
  -- چون کوئری این بات (join دو جدول بزرگ sales_invoice/sales_invoice_product + دو window function)
  -- سنگین است، اجرای آن روی این فراخوانی خودکار (با هر بازهٔ تاریخ کش‌شده/پیش‌فرضی که باشد) دقیقاً
  -- همان «لودینگ گیر می‌کند» است. طبق درخواست شما: کوئری فقط باید بعد از کلیک واقعی «اجرا کردن»
  -- اجرا شود. RES این فراخوانی خودکار را با پارامتر page_search_first=true علامت می‌زند
  -- (send شده از getGetResponseReport؛ کلیک واقعی «اجرا کردن» آن را false می‌فرستد) — همین
  -- سیگنال را برای رد کردن کوئری سنگین روی لود خودکار استفاده می‌کنیم؛ excel()/print() این پرچم
  -- را اصلاً ست نمی‌کنند، پس دست‌نخورده می‌مانند.
